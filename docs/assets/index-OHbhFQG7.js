...
`;const k=document.querySelector("#gl-canvas"),ee=document.querySelector("#scene-list"),tn=document.querySelector("#control-list"),Fe=document.querySelector("#panel-actions"),se=document.querySelector("#key-help"),Ue=document.querySelector("#hud-title"),Yn=document.querySelector("#hud-desc"),Oe=document.querySelector("[data-action='toggle-sidebar']"),bn=document.querySelector(".stage");if(!k||!ee||!tn||!Fe||!se||!Ue||!Yn||!Oe||!bn)throw new Error("Missing required UI elements");const s=k.getContext("webgl2",{antialias:!0});if(!s)throw bn.innerHTML=`
    <div class="fallback">
      <h2>WebGL2 unavailable</h2>
      <p>Your browser or GPU does not expose WebGL2. Try a different browser.</p>
    </div>
  `,new Error("WebGL2 unavailable");const Qr=s.getExtension("EXT_color_buffer_float"),Jr=!!Qr;s.disable(s.DEPTH_TEST);s.disable(s.BLEND);const Fn=qt(s),Kn=new Map,Qn={},oe={},Jn={},rn=new Map;for(const e of ae){const n=Wt(s,mt,e.fragment),t=new Set;t.add(e.resolutionUniform),t.add(e.timeUniform),e.loopUniform&&t.add(e.loopUniform),e.stateful&&(t.add(e.passUniform??"uPass"),t.add(e.stateUniform??"uState"),t.add(e.gridUniform??"uGridSize"));for(const l of e.params)t.add(l.uniform);const o=_t(s,n,Array.from(t));Kn.set(e.id,{program:n,uniforms:o});const a={},i={};for(const l of e.params)a[l.id]=l,i[l.id]=l.type==="seed"?Math.floor(Math.random()*1e6):l.value;Qn[e.id]=a,oe[e.id]={...i},Jn[e.id]={...i}}let W=ae[0],on={},gn=performance.now(),qe=null,Ve=null,X=null,we=[],et=0,Pe=null,be=!1;function eo(){const e=["video/webm;codecs=vp9","video/webm;codecs=vp8","video/webm","video/mp4"];for(const n of e)if(MediaRecorder.isTypeSupported(n))return n;return""}function no(e){return e.startsWith("video/mp4")?"mp4":"webm"}function to(e){const n=Math.floor(e/1e3),t=String(Math.floor(n/60)).padStart(2,"0"),o=String(n%60).padStart(2,"0");return`${t}:${o}`}function ro(){const e=document.getElementById("rec-badge");!e||!be||(e.textContent=`⏺ ${to(performance.now()-et)}`)}function oo(){const e=eo();if(!e){alert("Recording is not supported in this browser.");return}const n=k.captureStream(60);we=[],X=new MediaRecorder(n,{mimeType:e,videoBitsPerSecond:16e6}),X.ondataavailable=t=>{t.data.size>0&&we.push(t.data)},X.onstop=()=>{const t=no(e),o=new Blob(we,{type:e}),a=URL.createObjectURL(o),i=document.createElement("a");i.href=a,i.download=`${W.id}-${Date.now()}.${t}`,i.click(),URL.revokeObjectURL(a),we=[]},X.start(500),be=!0,et=performance.now(),nt(),Pe=window.setInterval(ro,250)}function ao(){X&&X.state!=="inactive"&&X.stop(),be=!1,Pe!==null&&(clearInterval(Pe),Pe=null),nt()}function io(){be?ao():oo()}function nt(){const e=document.getElementById("rec-btn"),n=document.getElementById("rec-badge");!e||!n||(be?(e.textContent="Stop",e.classList.add("recording"),n.classList.remove("hidden")):(e.textContent="Record",e.classList.remove("recording"),n.classList.add("hidden"),n.textContent=""))}let Ne=!1;function an(e){Oe.classList.toggle("hidden",e)}function lo(){an(!1),Ve!==null&&window.clearTimeout(Ve),Ve=window.setTimeout(()=>{an(!0)},2500)}function tt(){const e=document.body.classList.contains("sidebar-collapsed");Oe.textContent=e?">>":"<<"}function so(){var e;(e=Ue.parentElement)==null||e.classList.remove("hidden"),qe!==null&&window.clearTimeout(qe),qe=window.setTimeout(()=>{var n;(n=Ue.parentElement)==null||n.classList.add("hidden")},1e4)}function Pn(e){const n=s.createTexture();if(!n)throw new Error("Failed to create state texture");return s.bindTexture(s.TEXTURE_2D,n),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MIN_FILTER,s.NEAREST),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MAG_FILTER,s.NEAREST),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_S,s.REPEAT),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_T,s.REPEAT),s.texImage2D(s.TEXTURE_2D,0,s.RGBA16F,e,e,0,s.RGBA,s.HALF_FLOAT,null),s.bindTexture(s.TEXTURE_2D,null),n}function uo(e){const n=Pn(e),t=Pn(e),o=s.createFramebuffer(),a=s.createFramebuffer();if(!o||!a)throw new Error("Failed to create framebuffer");return s.bindFramebuffer(s.FRAMEBUFFER,o),s.framebufferTexture2D(s.FRAMEBUFFER,s.COLOR_ATTACHMENT0,s.TEXTURE_2D,n,0),s.bindFramebuffer(s.FRAMEBUFFER,a),s.framebufferTexture2D(s.FRAMEBUFFER,s.COLOR_ATTACHMENT0,s.TEXTURE_2D,t,0),s.bindFramebuffer(s.FRAMEBUFFER,null),{size:e,textures:[n,t],fbos:[o,a],index:0,needsInit:!0}}function Sn(e){if(!e.stateful)return null;let n=rn.get(e.id);const t=e.bufferSize??192;return(!n||n.size!==t)&&(n=uo(t),rn.set(e.id,n)),n}function he(e){const n=rn.get(e);n&&(n.needsInit=!0)}function fo(e,n){let t=n;return e.min!==void 0&&(t=Math.max(e.min,t)),e.max!==void 0&&(t=Math.min(e.max,t)),e.type==="int"&&(t=Math.round(t)),t}function rt(e,n){if(e.type==="int")return String(Math.round(n));const t=e.step??.01,o=t<1?Math.min(4,Math.max(2,Math.ceil(-Math.log10(t)))):0;return n.toFixed(o)}function co(e){return e.length===1?e.toLowerCase():e}function mo(e){return e instanceof HTMLInputElement||e instanceof HTMLTextAreaElement||e instanceof HTMLSelectElement}function ve(e,n,t,o=!0){const a=Qn[e][n];if(!a)return;const i=fo(a,t);if(oe[e][n]=i,e===W.id&&o){const l=on[n];l!=null&&l.range&&(l.range.value=String(i)),l!=null&&l.number&&(l.number.value=rt(a,i))}}function po(e){var n;for(const t of((n=ae.find(o=>o.id===e))==null?void 0:n.params)??[])t.type==="seed"&&ve(e,t.id,Math.floor(Math.random()*1e6));he(e)}function ho(e){const n=Jn[e];for(const[t,o]of Object.entries(n))ve(e,t,o,!0);he(e)}function vo(){ee.innerHTML="";for(const e of ae){const n=document.createElement("option");n.value=e.id,n.textContent=e.name,ee.appendChild(n)}ee.addEventListener("change",()=>{ot(ee.value)})}function yo(e){Fe.innerHTML="";const n=document.createElement("button");if(n.className="ghost small",n.textContent="Reset",n.addEventListener("click",()=>ho(e.id)),Fe.appendChild(n),e.params.some(o=>o.type==="seed")){const o=document.createElement("button");o.className="ghost small",o.textContent="Reseed",o.addEventListener("click",()=>po(e.id)),Fe.appendChild(o)}}function bo(e){tn.innerHTML="",on={};for(const n of e.params){if(n.type==="seed")continue;const t=document.createElement("div");t.className="control";const o=document.createElement("div");o.className="control-header";const a=document.createElement("label");if(a.textContent=n.label,o.appendChild(a),n.key){const p=document.createElement("span");p.className="key-cap",p.textContent=`${n.key.inc.toUpperCase()}/${n.key.dec.toUpperCase()}`,o.appendChild(p)}const i=document.createElement("div");i.className="control-inputs";const l=document.createElement("input");l.type="range",l.min=String(n.min??0),l.max=String(n.max??1),l.step=String(n.step??(n.type==="int"?1:.01)),l.value=String(oe[e.id][n.id]),l.addEventListener("input",p=>{const y=Number(p.target.value);Number.isNaN(y)||ve(e.id,n.id,y)});const u=document.createElement("input");u.type="number",u.min=l.min,u.max=l.max,u.step=l.step,u.value=rt(n,oe[e.id][n.id]),u.addEventListener("input",p=>{const y=Number(p.target.value);Number.isNaN(y)||ve(e.id,n.id,y)}),i.appendChild(l),i.appendChild(u),t.appendChild(o),t.appendChild(i),tn.appendChild(t),on[n.id]={range:l,number:u}}}function go(e){se.innerHTML="";for(const n of e.params){if(!n.key||n.type==="seed")continue;const t=document.createElement("div");t.className="key-row",t.textContent=`${n.key.inc.toUpperCase()}/${n.key.dec.toUpperCase()}  ${n.label}`,se.appendChild(t)}se.childElementCount||(se.textContent="No mapped keys for this scene.")}function ot(e){const n=ae.find(t=>t.id===e);n&&(W=n,n.stateful&&(Sn(n),he(n.id)),Ue.textContent=n.name,Yn.textContent=n.description,so(),yo(n),bo(n),go(n),ee.value=n.id)}function So(e){if(mo(e.target))return;const n=co(e.key),t=W.params;for(const o of t){if(!o.key||o.type==="seed")continue;const a=n===o.key.inc,i=n===o.key.dec;if(!a&&!i)continue;const l=e.shiftKey&&o.key.shiftStep?o.key.shiftStep:o.key.step,p=oe[W.id][o.id]+l*(a?1:-1);ve(W.id,o.id,p),e.preventDefault();break}}function Ce(e,n,t,o,a){const i=oe[e.id],l=n.uniforms,u=l[e.resolutionUniform];u&&s.uniform2f(u,o,a);const p=l[e.timeUniform];if(p)if(e.timeMode==="phase"){const b=e.loopDuration??8,w=t%b/b;s.uniform1f(p,w)}else if(e.timeMode==="looped"){const b=e.loopDuration??8,w=t%b;if(s.uniform1f(p,w),e.loopUniform){const S=l[e.loopUniform];S&&s.uniform1f(S,b)}}else s.uniform1f(p,t);const y={};for(const b of e.params){const w=l[b.uniform],S=i[b.id];if(b.component!==void 0){const _=y[b.uniform]??[0,0,0];_[b.component]=S,y[b.uniform]=_;continue}w&&(b.type==="int"?s.uniform1i(w,Math.round(S)):s.uniform1f(w,S))}for(const[b,w]of Object.entries(y)){const S=l[b];S&&s.uniform3f(S,w[0],w[1],w[2])}}function je(e,n,t,o){const a=n.uniforms[e.passUniform??"uPass"];a&&s.uniform1i(a,o);const i=n.uniforms[e.gridUniform??"uGridSize"];i&&s.uniform2f(i,t.size,t.size);const l=n.uniforms[e.stateUniform??"uState"];l&&s.uniform1i(l,0)}function at(e,n,t,o){const a=Kn.get(e.id);if(a){if(e.stateful){if(!Jr)return;const i=Sn(e);if(!i)return;const l=()=>i.textures[i.index],u=()=>i.fbos[(i.index+1)%2];s.useProgram(a.program),s.bindVertexArray(Fn),i.needsInit&&(s.bindFramebuffer(s.FRAMEBUFFER,u()),s.viewport(0,0,i.size,i.size),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,l()),Ce(e,a,n,t,o),je(e,a,i,2),s.drawArrays(s.TRIANGLES,0,3),i.index=(i.index+1)%2,i.needsInit=!1),s.bindFramebuffer(s.FRAMEBUFFER,u()),s.viewport(0,0,i.size,i.size),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,l()),Ce(e,a,n,t,o),je(e,a,i,0),s.drawArrays(s.TRIANGLES,0,3),i.index=(i.index+1)%2,s.bindFramebuffer(s.FRAMEBUFFER,null),s.viewport(0,0,t,o),s.clearColor(0,0,0,1),s.clear(s.COLOR_BUFFER_BIT),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,l()),Ce(e,a,n,t,o),je(e,a,i,1),s.drawArrays(s.TRIANGLES,0,3);return}s.viewport(0,0,t,o),s.clearColor(0,0,0,1),s.clear(s.COLOR_BUFFER_BIT),s.useProgram(a.program),s.bindVertexArray(Fn),Ce(e,a,n,t,o),s.drawArrays(s.TRIANGLES,0,3)}}function xn(e){if(Ne)return;const n=(e-gn)/1e3,{width:t,height:o}=Ht(k);at(W,n,t,o),requestAnimationFrame(xn)}function xo(){return new Promise(e=>requestAnimationFrame(()=>e()))}async function wo(e,n,t,o,a){if(typeof VideoEncoder>"u"||typeof VideoFrame>"u"){alert("Offline rendering requires the WebCodecs API (Chrome/Edge 94+, Safari 16.4+).");return}const i={codec:"avc1.640028",width:o,height:a,bitrate:16e6,framerate:t};if(!(await VideoEncoder.isConfigSupported(i)).supported){alert("H.264 video encoding is not supported on this device.");return}let u;try{u=await window.showSaveFilePicker({suggestedName:`${e.id}-${o}x${a}-${t}fps-${n}s.mp4`,types:[{description:"MP4 Video",accept:{"video/mp4":[".mp4"]}}]})}catch{return}const p=await u.createWritable(),y=document.getElementById("offline-btn"),b=document.getElementById("offline-progress"),w=document.getElementById("offline-bar"),S=document.getElementById("offline-status");y&&(y.disabled=!0),b==null||b.classList.remove("hidden"),S==null||S.classList.remove("hidden"),Ne=!0,e.stateful&&(Sn(e),he(e.id));const _=k.width,N=k.height,D=k.style.width,Y=k.style.height;k.width=o,k.height=a,k.style.width=`${o}px`,k.style.height=`${a}px`;const it=new Ln(p),wn=new Kr({target:it,video:{codec:"avc",width:o,height:a},fastStart:!1});let ge=null;const K=new VideoEncoder({output:(C,_e)=>wn.addVideoChunk(C,_e??void 0),error:C=>{ge=C}});K.configure(i);const Se=Math.ceil(n*t),Cn=Math.round(1e6/t),lt=t*2,st=t*30,ut=performance.now();let xe=!1;const Tn=C=>{C.preventDefault(),xe=!0};k.addEventListener("webglcontextlost",Tn);for(let C=0;C<Se&&!(ge||xe);C++){const _e=C/t;at(e,_e,o,a),s.finish();const kn=new VideoFrame(k,{timestamp:C*Cn,duration:Cn});K.encode(kn,{keyFrame:C%lt===0}),kn.close(),C>0&&C%st===0&&await K.flush();const ft=(C+1)/Se*100;if(w&&(w.style.width=`${ft}%`),C%30===0){const ct=(performance.now()-ut)/1e3/(C+1)*(Se-C-1);S&&(S.textContent=`Frame ${C+1}/${Se}  —  ~${Math.ceil(ct)}s left`)}K.encodeQueueSize>10&&await new Promise(Rn=>setTimeout(Rn,1)),await xo()}k.removeEventListener("webglcontextlost",Tn);try{await K.flush(),K.close(),wn.finalize(),await p.close()}catch(C){console.error("Finalization failed:",C);try{await p.close()}catch{}}k.style.width=D,k.style.height=Y,k.width=_,k.height=N,e.stateful&&he(e.id),Ne=!1,gn=performance.now(),requestAnimationFrame(xn),y&&(y.disabled=!1);const We=ge?ge.message:null;S&&(xe?S.textContent="Context lost — partial video saved.":We?S.textContent=`Error: ${We}`:S.textContent="Done!"),setTimeout(()=>{b==null||b.classList.add("hidden"),S==null||S.classList.add("hidden"),w&&(w.style.width="0%")},xe||!!We?8e3:3e3)}Oe.addEventListener("click",()=>{document.body.classList.toggle("sidebar-collapsed"),tt()});bn.addEventListener("mousemove",()=>{lo()});document.addEventListener("keydown",So);document.addEventListener("visibilitychange",()=>{document.hidden||(gn=performance.now())});var Mn;(Mn=document.getElementById("rec-btn"))==null||Mn.addEventListener("click",io);var zn;(zn=document.getElementById("offline-btn"))==null||zn.addEventListener("click",()=>{if(Ne)return;const e=document.getElementById("offline-duration"),n=document.getElementById("offline-fps"),t=document.getElementById("offline-res"),o=Math.max(1,Math.min(3600,Number((e==null?void 0:e.value)??10))),a=Number((n==null?void 0:n.value)??60),[i,l]=((t==null?void 0:t.value)??"1920x1080").split("x").map(Number);wo(W,o,a,i,l)});vo();ot(W.id);tt();an(!0);requestAnimationFrame(xn);

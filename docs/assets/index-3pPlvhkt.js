...
\r
// Smoke / advection controls\r
uniform float uSmokeDensity;\r
uniform float uSmokeSteps;\r
uniform float uTurbulence;\r
uniform float uAdvectionRate;\r
uniform float uDissipation;\r
\r
// Lighting\r
uniform float uLightAngle;\r
uniform float uLightHeight;\r
uniform float uLightIntensity;\r
uniform float uAbsorption;\r
uniform float uScatterPower;\r
\r
// Color palette\r
...
// ──────────────────────────────────────────────────────────────\r
\r
const float PI  = 3.14159265;\r
const float TAU = 6.28318530;\r
\r
mat2 rot2(float a) {\r
    float s = sin(a), c = cos(a);\r
    return mat2(c, -s, s, c);\r
...
\r
// Hash for seeded noise\r
float hash13(vec3 p3, float seed) {\r
    p3 = fract(p3 * 0.1031 + seed * 0.117);\r
    p3 += dot(p3, p3.zyx + 31.32);\r
    return fract((p3.x + p3.y) * p3.z);\r
}\r
\r
float hash12(vec2 p, float seed) {\r
    vec3 p3 = fract(vec3(p.xyx) * 0.1031 + seed * 0.117);\r
    p3 += dot(p3, p3.yzx + 33.33);\r
    return fract((p3.x + p3.y) * p3.z);\r
}\r
\r
// Smooth 3D value noise\r
float noise3(vec3 p, float seed) {\r
    vec3 i = floor(p);\r
    vec3 f = fract(p);\r
    f = f * f * (3.0 - 2.0 * f);\r
\r
    float a = hash13(i + vec3(0, 0, 0), seed);\r
    float b = hash13(i + vec3(1, 0, 0), seed);\r
    float c = hash13(i + vec3(0, 1, 0), seed);\r
    float d = hash13(i + vec3(1, 1, 0), seed);\r
    float e = hash13(i + vec3(0, 0, 1), seed);\r
    float g = hash13(i + vec3(1, 0, 1), seed);\r
    float h = hash13(i + vec3(0, 1, 1), seed);\r
    float k = hash13(i + vec3(1, 1, 1), seed);\r
\r
    return mix(\r
        mix(mix(a, b, f.x), mix(c, d, f.x), f.y),\r
        mix(mix(e, g, f.x), mix(h, k, f.x), f.y),\r
        f.z\r
    );\r
}\r
\r
// FBM (fractal Brownian motion) — 4 octaves max for performance\r
float fbm(vec3 p, float seed, float turbulence) {\r
    float val = 0.0;\r
    float amp = 0.5;\r
    float freq = 1.0;\r
    float lacunarity = 2.0 + turbulence * 0.3;\r
    float gain = 0.5 - turbulence * 0.05;\r
    for (int i = 0; i < 4; i++) {\r
        val += amp * noise3(p * freq, seed + float(i) * 1.731);\r
        freq *= lacunarity;\r
        amp *= gain;\r
    }\r
    return val;\r
...
\r
// ──────────────────────────────────────────────────────────────\r
// FRACTAL SDF — Mandelbox-style folding\r
// ──────────────────────────────────────────────────────────────\r
\r
// Precomputed rotation matrices (set once per frame in main)\r
mat2 fracRotXY, fracRotYZ;\r
\r
float fractalSDF(vec3 p) {\r
    float scale = uFoldScale;\r
    float offset = uFoldOffset;\r
    vec3 orig = p;\r
    float dr = 1.0;\r
\r
    for (int i = 0; i < 8; i++) {\r
        if (i >= uFractalIters) break;\r
\r
        // Box fold\r
        p = clamp(p, -1.0, 1.0) * 2.0 - p;\r
\r
        // Sphere fold\r
        float r2 = dot(p, p);\r
        if (r2 < 0.25) {\r
            float t = 4.0; // fixedR2 / minR2 = 1.0/0.25\r
            p *= t;\r
            dr *= t;\r
        } else if (r2 < 1.0) {\r
            float t = 1.0 / r2;\r
...
\r
        // Rotate (precomputed matrices)\r
        p.xy *= fracRotXY;\r
        p.yz *= fracRotYZ;\r
\r
        // Scale & translate\r
        p = p * scale + orig * offset;\r
        dr = dr * abs(scale) + 1.0;\r
    }\r
\r
    return length(p) / abs(dr) - 0.002;\r
}\r
\r
// ──────────────────────────────────────────────────────────────\r
// SMOKE DENSITY — fractal surface perturbs a volumetric field\r
// ──────────────────────────────────────────────────────────────\r
\r
// Returns density in .x, fractal distance in .y (avoids recomputing SDF)\r
vec2 smokeDensityAndDist(vec3 p, float time, float seed) {\r
    float dist = fractalSDF(p);\r
\r
    // Near-surface density — sharper falloff so smoke hugs the fractal\r
    float falloff = 6.0 / max(uSmokeDensity, 0.01);\r
    float surfaceMask = exp(-abs(dist) * falloff);\r
\r
    // Quick exit for points far from the surface\r
    if (surfaceMask < 0.01) return vec2(0.0, dist);\r
\r
    // FBM turbulence advected by time\r
    vec3 advected = p + vec3(\r
        sin(time * 0.3 + p.y * 0.7) * uAdvectionRate,\r
        cos(time * 0.2 + p.z * 0.6) * uAdvectionRate * 0.8,\r
        sin(time * 0.4 + p.x * 0.5) * uAdvectionRate * 0.6\r
    );\r
    float turb = fbm(advected * 1.5, seed, uTurbulence);\r
\r
    // Turbulence-driven wisps with low base — keeps gaps visible\r
    float density = surfaceMask * max(turb * 0.9 - 0.05, 0.0);\r
    density *= exp(-length(p) * uDissipation * 0.2);\r
\r
    return vec2(max(density, 0.0), dist);\r
}\r
\r
// Cheap density for light sampling — fractal SDF only, no FBM\r
float smokeDensityCheap(vec3 p) {\r
    float dist = fractalSDF(p);\r
    float falloff = 6.0 / max(uSmokeDensity, 0.01);\r
    float surfaceMask = exp(-abs(dist) * falloff);\r
    return surfaceMask * 0.4 * exp(-length(p) * uDissipation * 0.2);\r
}\r
\r
// ──────────────────────────────────────────────────────────────\r
// COLOR PALETTE (IQ cosine gradient)\r
// ──────────────────────────────────────────────────────────────\r
\r
vec3 palette(float t) {\r
    vec3 a = vec3(0.5, 0.5, 0.5);\r
    vec3 b = vec3(0.5, 0.5, 0.5) * uSaturation;\r
    vec3 c = vec3(1.0, 1.0, 1.0);\r
    vec3 d = vec3(uHueShift, uHueShift + 0.33, uHueShift + 0.67);\r
    return a + b * cos(TAU * (c * t + d));\r
}\r
\r
vec3 emissionPalette(float t) {\r
    vec3 a = vec3(0.5, 0.5, 0.5);\r
    vec3 b = vec3(0.6, 0.6, 0.6);\r
    vec3 c = vec3(1.0, 0.7, 0.4);\r
    vec3 d = vec3(uHueShift + 0.1, uHueShift + 0.45, uHueShift + 0.8);\r
    return a + b * cos(TAU * (c * t + d));\r
}\r
\r
// ──────────────────────────────────────────────────────────────\r
...
`;const k=document.querySelector("#gl-canvas"),ee=document.querySelector("#scene-list"),nt=document.querySelector("#control-list"),Fe=document.querySelector("#panel-actions"),le=document.querySelector("#key-help"),Ue=document.querySelector("#hud-title"),Zt=document.querySelector("#hud-desc"),Le=document.querySelector("[data-action='toggle-sidebar']"),yt=document.querySelector(".stage");if(!k||!ee||!nt||!Fe||!le||!Ue||!Zt||!Le||!yt)throw new Error("Missing required UI elements");const s=k.getContext("webgl2",{antialias:!0});if(!s)throw yt.innerHTML=`
    <div class="fallback">
      <h2>WebGL2 unavailable</h2>
      <p>Your browser or GPU does not expose WebGL2. Try a different browser.</p>
    </div>
  `,new Error("WebGL2 unavailable");const qi=s.getExtension("EXT_color_buffer_float"),Vi=!!qi;s.disable(s.DEPTH_TEST);s.disable(s.BLEND);const Bt=Nn(s),Yt=new Map,Kt={},ae={},Qt={},it=new Map;for(const e of ve){const t=Dn(s,mn,e.fragment),n=new Set;n.add(e.resolutionUniform),n.add(e.timeUniform),e.loopUniform&&n.add(e.loopUniform),e.stateful&&(n.add(e.passUniform??"uPass"),n.add(e.stateUniform??"uState"),n.add(e.gridUniform??"uGridSize"));for(const l of e.params)n.add(l.uniform);const a=En(s,t,Array.from(n));Yt.set(e.id,{program:t,uniforms:a});const o={},r={};for(const l of e.params)o[l.id]=l,r[l.id]=l.type==="seed"?Math.floor(Math.random()*1e6):l.value;Kt[e.id]=o,ae[e.id]={...r},Qt[e.id]={...r}}let O=ve[0],at={},gt=performance.now(),qe=null,Ve=null,X=null,we=[],Jt=0,Ie=null,ge=!1;function ji(){const e=["video/webm;codecs=vp9","video/webm;codecs=vp8","video/webm","video/mp4"];for(const t of e)if(MediaRecorder.isTypeSupported(t))return t;return""}function Xi(e){return e.startsWith("video/mp4")?"mp4":"webm"}function $i(e){const t=Math.floor(e/1e3),n=String(Math.floor(t/60)).padStart(2,"0"),a=String(t%60).padStart(2,"0");return`${n}:${a}`}function Zi(){const e=document.getElementById("rec-badge");!e||!ge||(e.textContent=`⏺ ${$i(performance.now()-Jt)}`)}function Yi(){const e=ji();if(!e){alert("Recording is not supported in this browser.");return}const t=k.captureStream(60);we=[],X=new MediaRecorder(t,{mimeType:e,videoBitsPerSecond:16e6}),X.ondataavailable=n=>{n.data.size>0&&we.push(n.data)},X.onstop=()=>{const n=Xi(e),a=new Blob(we,{type:e}),o=URL.createObjectURL(a),r=document.createElement("a");r.href=o,r.download=`${O.id}-${Date.now()}.${n}`,r.click(),URL.revokeObjectURL(o),we=[]},X.start(500),ge=!0,Jt=performance.now(),en(),Ie=window.setInterval(Zi,250)}function Ki(){X&&X.state!=="inactive"&&X.stop(),ge=!1,Ie!==null&&(clearInterval(Ie),Ie=null),en()}function Qi(){ge?Ki():Yi()}function en(){const e=document.getElementById("rec-btn"),t=document.getElementById("rec-badge");!e||!t||(ge?(e.textContent="Stop",e.classList.add("recording"),t.classList.remove("hidden")):(e.textContent="Record",e.classList.remove("recording"),t.classList.add("hidden"),t.textContent=""))}let Ge=!1;function ot(e){Le.classList.toggle("hidden",e)}function Ji(){ot(!1),Ve!==null&&window.clearTimeout(Ve),Ve=window.setTimeout(()=>{ot(!0)},2500)}function tn(){const e=document.body.classList.contains("sidebar-collapsed");Le.textContent=e?">>":"<<"}function ea(){var e;(e=Ue.parentElement)==null||e.classList.remove("hidden"),qe!==null&&window.clearTimeout(qe),qe=window.setTimeout(()=>{var t;(t=Ue.parentElement)==null||t.classList.add("hidden")},1e4)}function Ft(e){const t=s.createTexture();if(!t)throw new Error("Failed to create state texture");return s.bindTexture(s.TEXTURE_2D,t),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MIN_FILTER,s.NEAREST),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MAG_FILTER,s.NEAREST),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_S,s.REPEAT),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_T,s.REPEAT),s.texImage2D(s.TEXTURE_2D,0,s.RGBA16F,e,e,0,s.RGBA,s.HALF_FLOAT,null),s.bindTexture(s.TEXTURE_2D,null),t}function ta(e){const t=Ft(e),n=Ft(e),a=s.createFramebuffer(),o=s.createFramebuffer();if(!a||!o)throw new Error("Failed to create framebuffer");return s.bindFramebuffer(s.FRAMEBUFFER,a),s.framebufferTexture2D(s.FRAMEBUFFER,s.COLOR_ATTACHMENT0,s.TEXTURE_2D,t,0),s.bindFramebuffer(s.FRAMEBUFFER,o),s.framebufferTexture2D(s.FRAMEBUFFER,s.COLOR_ATTACHMENT0,s.TEXTURE_2D,n,0),s.bindFramebuffer(s.FRAMEBUFFER,null),{size:e,textures:[t,n],fbos:[a,o],index:0,needsInit:!0}}function bt(e){if(!e.stateful)return null;let t=it.get(e.id);const n=e.bufferSize??192;return(!t||t.size!==n)&&(t=ta(n),it.set(e.id,t)),t}function de(e){const t=it.get(e);t&&(t.needsInit=!0)}function na(e,t){let n=t;return e.min!==void 0&&(n=Math.max(e.min,n)),e.max!==void 0&&(n=Math.min(e.max,n)),e.type==="int"&&(n=Math.round(n)),n}function nn(e,t){if(e.type==="int")return String(Math.round(t));const n=e.step??.01,a=n<1?Math.min(4,Math.max(2,Math.ceil(-Math.log10(n)))):0;return t.toFixed(a)}function ia(e){return e.length===1?e.toLowerCase():e}function aa(e){return e instanceof HTMLInputElement||e instanceof HTMLTextAreaElement||e instanceof HTMLSelectElement}function he(e,t,n,a=!0){const o=Kt[e][t];if(!o)return;const r=na(o,n);if(ae[e][t]=r,e===O.id&&a){const l=at[t];l!=null&&l.range&&(l.range.value=String(r)),l!=null&&l.number&&(l.number.value=nn(o,r))}}function oa(e){var t;for(const n of((t=ve.find(a=>a.id===e))==null?void 0:t.params)??[])n.type==="seed"&&he(e,n.id,Math.floor(Math.random()*1e6));de(e)}function ra(e){const t=Qt[e];for(const[n,a]of Object.entries(t))he(e,n,a,!0);de(e)}function la(){ee.innerHTML="";for(const e of ve){const t=document.createElement("option");t.value=e.id,t.textContent=e.name,ee.appendChild(t)}ee.addEventListener("change",()=>{an(ee.value)})}function sa(e){Fe.innerHTML="";const t=document.createElement("button");if(t.className="ghost small",t.textContent="Reset",t.addEventListener("click",()=>ra(e.id)),Fe.appendChild(t),e.params.some(a=>a.type==="seed")){const a=document.createElement("button");a.className="ghost small",a.textContent="Reseed",a.addEventListener("click",()=>oa(e.id)),Fe.appendChild(a)}}function ua(e){nt.innerHTML="",at={};for(const t of e.params){if(t.type==="seed")continue;const n=document.createElement("div");n.className="control";const a=document.createElement("div");a.className="control-header";const o=document.createElement("label");if(o.textContent=t.label,a.appendChild(o),t.key){const p=document.createElement("span");p.className="key-cap",p.textContent=`${t.key.inc.toUpperCase()}/${t.key.dec.toUpperCase()}`,a.appendChild(p)}const r=document.createElement("div");r.className="control-inputs";const l=document.createElement("input");l.type="range",l.min=String(t.min??0),l.max=String(t.max??1),l.step=String(t.step??(t.type==="int"?1:.01)),l.value=String(ae[e.id][t.id]),l.addEventListener("input",p=>{const y=Number(p.target.value);Number.isNaN(y)||he(e.id,t.id,y)});const u=document.createElement("input");u.type="number",u.min=l.min,u.max=l.max,u.step=l.step,u.value=nn(t,ae[e.id][t.id]),u.addEventListener("input",p=>{const y=Number(p.target.value);Number.isNaN(y)||he(e.id,t.id,y)}),r.appendChild(l),r.appendChild(u),n.appendChild(a),n.appendChild(r),nt.appendChild(n),at[t.id]={range:l,number:u}}}function fa(e){le.innerHTML="";for(const t of e.params){if(!t.key||t.type==="seed")continue;const n=document.createElement("div");n.className="key-row",n.textContent=`${t.key.inc.toUpperCase()}/${t.key.dec.toUpperCase()}  ${t.label}`,le.appendChild(n)}le.childElementCount||(le.textContent="No mapped keys for this scene.")}function an(e){const t=ve.find(n=>n.id===e);t&&(O=t,t.stateful&&(bt(t),de(t.id)),Ue.textContent=t.name,Zt.textContent=t.description,ea(),sa(t),ua(t),fa(t),ee.value=t.id)}function ca(e){if(aa(e.target))return;const t=ia(e.key),n=O.params;for(const a of n){if(!a.key||a.type==="seed")continue;const o=t===a.key.inc,r=t===a.key.dec;if(!o&&!r)continue;const l=e.shiftKey&&a.key.shiftStep?a.key.shiftStep:a.key.step,p=ae[O.id][a.id]+l*(o?1:-1);he(O.id,a.id,p),e.preventDefault();break}}function Ce(e,t,n,a,o){const r=ae[e.id],l=t.uniforms,u=l[e.resolutionUniform];u&&s.uniform2f(u,a,o);const p=l[e.timeUniform];if(p)if(e.timeMode==="phase"){const g=e.loopDuration??8,w=n%g/g;s.uniform1f(p,w)}else if(e.timeMode==="looped"){const g=e.loopDuration??8,w=n%g;if(s.uniform1f(p,w),e.loopUniform){const S=l[e.loopUniform];S&&s.uniform1f(S,g)}}else s.uniform1f(p,n);const y={};for(const g of e.params){const w=l[g.uniform],S=r[g.id];if(g.component!==void 0){const W=y[g.uniform]??[0,0,0];W[g.component]=S,y[g.uniform]=W;continue}w&&(g.type==="int"?s.uniform1i(w,Math.round(S)):s.uniform1f(w,S))}for(const[g,w]of Object.entries(y)){const S=l[g];S&&s.uniform3f(S,w[0],w[1],w[2])}}function je(e,t,n,a){const o=t.uniforms[e.passUniform??"uPass"];o&&s.uniform1i(o,a);const r=t.uniforms[e.gridUniform??"uGridSize"];r&&s.uniform2f(r,n.size,n.size);const l=t.uniforms[e.stateUniform??"uState"];l&&s.uniform1i(l,0)}function on(e,t,n,a){const o=Yt.get(e.id);if(o){if(e.stateful){if(!Vi)return;const r=bt(e);if(!r)return;const l=()=>r.textures[r.index],u=()=>r.fbos[(r.index+1)%2];s.useProgram(o.program),s.bindVertexArray(Bt),r.needsInit&&(s.bindFramebuffer(s.FRAMEBUFFER,u()),s.viewport(0,0,r.size,r.size),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,l()),Ce(e,o,t,n,a),je(e,o,r,2),s.drawArrays(s.TRIANGLES,0,3),r.index=(r.index+1)%2,r.needsInit=!1),s.bindFramebuffer(s.FRAMEBUFFER,u()),s.viewport(0,0,r.size,r.size),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,l()),Ce(e,o,t,n,a),je(e,o,r,0),s.drawArrays(s.TRIANGLES,0,3),r.index=(r.index+1)%2,s.bindFramebuffer(s.FRAMEBUFFER,null),s.viewport(0,0,n,a),s.clearColor(0,0,0,1),s.clear(s.COLOR_BUFFER_BIT),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,l()),Ce(e,o,t,n,a),je(e,o,r,1),s.drawArrays(s.TRIANGLES,0,3);return}s.viewport(0,0,n,a),s.clearColor(0,0,0,1),s.clear(s.COLOR_BUFFER_BIT),s.useProgram(o.program),s.bindVertexArray(Bt),Ce(e,o,t,n,a),s.drawArrays(s.TRIANGLES,0,3)}}function St(e){if(Ge)return;const t=(e-gt)/1e3,{width:n,height:a}=Pn(k);on(O,t,n,a),requestAnimationFrame(St)}function ma(){return new Promise(e=>requestAnimationFrame(()=>e()))}async function pa(e,t,n,a,o){if(typeof VideoEncoder>"u"||typeof VideoFrame>"u"){alert("Offline rendering requires the WebCodecs API (Chrome/Edge 94+, Safari 16.4+).");return}const r={codec:"avc1.640028",width:a,height:o,bitrate:16e6,framerate:n};if(!(await VideoEncoder.isConfigSupported(r)).supported){alert("H.264 video encoding is not supported on this device.");return}let u;try{u=await window.showSaveFilePicker({suggestedName:`${e.id}-${a}x${o}-${n}fps-${t}s.mp4`,types:[{description:"MP4 Video",accept:{"video/mp4":[".mp4"]}}]})}catch{return}const p=await u.createWritable(),y=document.getElementById("offline-btn"),g=document.getElementById("offline-progress"),w=document.getElementById("offline-bar"),S=document.getElementById("offline-status");y&&(y.disabled=!0),g==null||g.classList.remove("hidden"),S==null||S.classList.remove("hidden"),Ge=!0,e.stateful&&(bt(e),de(e.id));const W=k.width,G=k.height,D=k.style.width,Y=k.style.height;k.width=a,k.height=o,k.style.width=`${a}px`,k.style.height=`${o}px`;const rn=new Gt(p),xt=new Hi({target:rn,video:{codec:"avc",width:a,height:o},fastStart:!1});let be=null;const K=new VideoEncoder({output:(C,We)=>xt.addVideoChunk(C,We??void 0),error:C=>{be=C}});K.configure(r);const Se=Math.ceil(t*n),wt=Math.round(1e6/n),ln=n*2,sn=n*30,un=performance.now();let xe=!1;const Ct=C=>{C.preventDefault(),xe=!0};k.addEventListener("webglcontextlost",Ct);for(let C=0;C<Se&&!(be||xe);C++){const We=C/n;on(e,We,a,o),s.finish();const Tt=new VideoFrame(k,{timestamp:C*wt,duration:wt});K.encode(Tt,{keyFrame:C%ln===0}),Tt.close(),C>0&&C%sn===0&&await K.flush();const fn=(C+1)/Se*100;if(w&&(w.style.width=`${fn}%`),C%30===0){const cn=(performance.now()-un)/1e3/(C+1)*(Se-C-1);S&&(S.textContent=`Frame ${C+1}/${Se}  —  ~${Math.ceil(cn)}s left`)}K.encodeQueueSize>10&&await new Promise(kt=>setTimeout(kt,1)),await ma()}k.removeEventListener("webglcontextlost",Ct);try{await K.flush(),K.close(),xt.finalize(),await p.close()}catch(C){console.error("Finalization failed:",C);try{await p.close()}catch{}}k.style.width=D,k.style.height=Y,k.width=W,k.height=G,e.stateful&&de(e.id),Ge=!1,gt=performance.now(),requestAnimationFrame(St),y&&(y.disabled=!1);const Oe=be?be.message:null;S&&(xe?S.textContent="Context lost — partial video saved.":Oe?S.textContent=`Error: ${Oe}`:S.textContent="Done!"),setTimeout(()=>{g==null||g.classList.add("hidden"),S==null||S.classList.add("hidden"),w&&(w.style.width="0%")},xe||!!Oe?8e3:3e3)}Le.addEventListener("click",()=>{document.body.classList.toggle("sidebar-collapsed"),tn()});yt.addEventListener("mousemove",()=>{Ji()});document.addEventListener("keydown",ca);document.addEventListener("visibilitychange",()=>{document.hidden||(gt=performance.now())});var It;(It=document.getElementById("rec-btn"))==null||It.addEventListener("click",Qi);var zt;(zt=document.getElementById("offline-btn"))==null||zt.addEventListener("click",()=>{if(Ge)return;const e=document.getElementById("offline-duration"),t=document.getElementById("offline-fps"),n=document.getElementById("offline-res"),a=Math.max(1,Math.min(3600,Number((e==null?void 0:e.value)??10))),o=Number((t==null?void 0:t.value)??60),[r,l]=((n==null?void 0:n.value)??"1920x1080").split("x").map(Number);pa(O,a,o,r,l)});la();an(O.id);tn();ot(!0);requestAnimationFrame(St);

...
`;const I=document.querySelector("#gl-canvas"),v=document.querySelector("#scene-list"),A=document.querySelector("#control-list"),C=document.querySelector("#panel-actions"),h=document.querySelector("#key-help"),T=document.querySelector("#hud-title"),M=document.querySelector("#hud-desc"),w=document.querySelector("[data-action='toggle-sidebar']"),N=document.querySelector(".stage");if(!I||!v||!A||!C||!h||!T||!M||!w||!N)throw new Error("Missing required UI elements");const t=I.getContext("webgl2",{antialias:!0});if(!t)throw N.innerHTML=`
    <div class="fallback">
      <h2>WebGL2 unavailable</h2>
      <p>Your browser or GPU does not expose WebGL2. Try a different browser.</p>
    </div>
  `,new Error("WebGL2 unavailable");const xe=t.getExtension("EXT_color_buffer_float"),be=!!xe;t.disable(t.DEPTH_TEST);t.disable(t.BLEND);const U=he(t),_=new Map,H={},y={},q={},F=new Map;for(const e of S){const n=de(t,Z,e.fragment),o=new Set;o.add(e.resolutionUniform),o.add(e.timeUniform),e.loopUniform&&o.add(e.loopUniform),e.stateful&&(o.add(e.passUniform??"uPass"),o.add(e.stateUniform??"uState"),o.add(e.gridUniform??"uGridSize"));for(const r of e.params)o.add(r.uniform);const a=ve(t,n,Array.from(o));_.set(e.id,{program:n,uniforms:a});const i={},l={};for(const r of e.params)i[r.id]=r,l[r.id]=r.type==="seed"?Math.floor(Math.random()*1e6):r.value;H[e.id]=i,y[e.id]={...l},q[e.id]={...l}}let u=S[0],P={},O=performance.now(),R=null,k=null;function G(e){w.classList.toggle("hidden",e)}function Se(){G(!1),k!==null&&window.clearTimeout(k),k=window.setTimeout(()=>{G(!0)},2500)}function V(){const e=document.body.classList.contains("sidebar-collapsed");w.textContent=e?">>":"<<"}function ge(){var e;(e=T.parentElement)==null||e.classList.remove("hidden"),R!==null&&window.clearTimeout(R),R=window.setTimeout(()=>{var n;(n=T.parentElement)==null||n.classList.add("hidden")},1e4)}function L(e){const n=t.createTexture();if(!n)throw new Error("Failed to create state texture");return t.bindTexture(t.TEXTURE_2D,n),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_MIN_FILTER,t.NEAREST),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_MAG_FILTER,t.NEAREST),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_WRAP_S,t.REPEAT),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_WRAP_T,t.REPEAT),t.texImage2D(t.TEXTURE_2D,0,t.RGBA16F,e,e,0,t.RGBA,t.HALF_FLOAT,null),t.bindTexture(t.TEXTURE_2D,null),n}function Ce(e){const n=L(e),o=L(e),a=t.createFramebuffer(),i=t.createFramebuffer();if(!a||!i)throw new Error("Failed to create framebuffer");return t.bindFramebuffer(t.FRAMEBUFFER,a),t.framebufferTexture2D(t.FRAMEBUFFER,t.COLOR_ATTACHMENT0,t.TEXTURE_2D,n,0),t.bindFramebuffer(t.FRAMEBUFFER,i),t.framebufferTexture2D(t.FRAMEBUFFER,t.COLOR_ATTACHMENT0,t.TEXTURE_2D,o,0),t.bindFramebuffer(t.FRAMEBUFFER,null),{size:e,textures:[n,o],fbos:[a,i],index:0,needsInit:!0}}function X(e){if(!e.stateful)return null;let n=F.get(e.id);const o=e.bufferSize??192;return(!n||n.size!==o)&&(n=Ce(o),F.set(e.id,n)),n}function z(e){const n=F.get(e);n&&(n.needsInit=!0)}function Te(e,n){let o=n;return e.min!==void 0&&(o=Math.max(e.min,o)),e.max!==void 0&&(o=Math.min(e.max,o)),e.type==="int"&&(o=Math.round(o)),o}function j(e,n){if(e.type==="int")return String(Math.round(n));const o=e.step??.01,a=o<1?Math.min(4,Math.max(2,Math.ceil(-Math.log10(o)))):0;return n.toFixed(a)}function we(e){return e.length===1?e.toLowerCase():e}function Re(e){return e instanceof HTMLInputElement||e instanceof HTMLTextAreaElement||e instanceof HTMLSelectElement}function b(e,n,o,a=!0){const i=H[e][n];if(!i)return;const l=Te(i,o);if(y[e][n]=l,e===u.id&&a){const r=P[n];r!=null&&r.range&&(r.range.value=String(l)),r!=null&&r.number&&(r.number.value=j(i,l))}}function ke(e){var n;for(const o of((n=S.find(a=>a.id===e))==null?void 0:n.params)??[])o.type==="seed"&&b(e,o.id,Math.floor(Math.random()*1e6));z(e)}function Be(e){const n=q[e];for(const[o,a]of Object.entries(n))b(e,o,a,!0);z(e)}function Ae(){v.innerHTML="";for(const e of S){const n=document.createElement("option");n.value=e.id,n.textContent=e.name,v.appendChild(n)}v.addEventListener("change",()=>{Y(v.value)})}function Fe(e){C.innerHTML="";const n=document.createElement("button");if(n.className="ghost small",n.textContent="Reset",n.addEventListener("click",()=>Be(e.id)),C.appendChild(n),e.params.some(a=>a.type==="seed")){const a=document.createElement("button");a.className="ghost small",a.textContent="Reseed",a.addEventListener("click",()=>ke(e.id)),C.appendChild(a)}}function Pe(e){A.innerHTML="",P={};for(const n of e.params){if(n.type==="seed")continue;const o=document.createElement("div");o.className="control";const a=document.createElement("div");a.className="control-header";const i=document.createElement("label");if(i.textContent=n.label,a.appendChild(i),n.key){const f=document.createElement("span");f.className="key-cap",f.textContent=`${n.key.inc.toUpperCase()}/${n.key.dec.toUpperCase()}`,a.appendChild(f)}const l=document.createElement("div");l.className="control-inputs";const r=document.createElement("input");r.type="range",r.min=String(n.min??0),r.max=String(n.max??1),r.step=String(n.step??(n.type==="int"?1:.01)),r.value=String(y[e.id][n.id]),r.addEventListener("input",f=>{const p=Number(f.target.value);Number.isNaN(p)||b(e.id,n.id,p)});const s=document.createElement("input");s.type="number",s.min=r.min,s.max=r.max,s.step=r.step,s.value=j(n,y[e.id][n.id]),s.addEventListener("input",f=>{const p=Number(f.target.value);Number.isNaN(p)||b(e.id,n.id,p)}),l.appendChild(r),l.appendChild(s),o.appendChild(a),o.appendChild(l),A.appendChild(o),P[n.id]={range:r,number:s}}}function Ge(e){h.innerHTML="";for(const n of e.params){if(!n.key||n.type==="seed")continue;const o=document.createElement("div");o.className="key-row",o.textContent=`${n.key.inc.toUpperCase()}/${n.key.dec.toUpperCase()}  ${n.label}`,h.appendChild(o)}h.childElementCount||(h.textContent="No mapped keys for this scene.")}function Y(e){const n=S.find(o=>o.id===e);n&&(u=n,n.stateful&&(X(n),z(n.id)),T.textContent=n.name,M.textContent=n.description,ge(),Fe(n),Pe(n),Ge(n),v.value=n.id)}function Ie(e){if(Re(e.target))return;const n=we(e.key),o=u.params;for(const a of o){if(!a.key||a.type==="seed")continue;const i=n===a.key.inc,l=n===a.key.dec;if(!i&&!l)continue;const r=e.shiftKey&&a.key.shiftStep?a.key.shiftStep:a.key.step,f=y[u.id][a.id]+r*(i?1:-1);b(u.id,a.id,f),e.preventDefault();break}}function g(e,n,o,a,i){const l=y[e.id],r=n.uniforms,s=r[e.resolutionUniform];s&&t.uniform2f(s,a,i);const f=r[e.timeUniform];if(f)if(e.timeMode==="phase"){const c=e.loopDuration??8,m=o%c/c;t.uniform1f(f,m)}else if(e.timeMode==="looped"){const c=e.loopDuration??8,m=o%c;if(t.uniform1f(f,m),e.loopUniform){const d=r[e.loopUniform];d&&t.uniform1f(d,c)}}else t.uniform1f(f,o);const p={};for(const c of e.params){const m=r[c.uniform],d=l[c.id];if(c.component!==void 0){const D=p[c.uniform]??[0,0,0];D[c.component]=d,p[c.uniform]=D;continue}m&&(c.type==="int"?t.uniform1i(m,Math.round(d)):t.uniform1f(m,d))}for(const[c,m]of Object.entries(p)){const d=r[c];d&&t.uniform3f(d,m[0],m[1],m[2])}}function B(e,n,o,a){const i=n.uniforms[e.passUniform??"uPass"];i&&t.uniform1i(i,a);const l=n.uniforms[e.gridUniform??"uGridSize"];l&&t.uniform2f(l,o.size,o.size);const r=n.uniforms[e.stateUniform??"uState"];r&&t.uniform1i(r,0)}function x(e){const n=(e-O)/1e3,{width:o,height:a}=ye(I),i=_.get(u.id);if(i){if(u.stateful){if(!be){M.textContent="Stateful scenes require float render targets (EXT_color_buffer_float).",requestAnimationFrame(x);return}const l=X(u);if(!l){requestAnimationFrame(x);return}const r=()=>l.textures[l.index],s=()=>l.fbos[(l.index+1)%2];t.useProgram(i.program),t.bindVertexArray(U),l.needsInit&&(t.bindFramebuffer(t.FRAMEBUFFER,s()),t.viewport(0,0,l.size,l.size),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,r()),g(u,i,n,o,a),B(u,i,l,2),t.drawArrays(t.TRIANGLES,0,3),l.index=(l.index+1)%2,l.needsInit=!1),t.bindFramebuffer(t.FRAMEBUFFER,s()),t.viewport(0,0,l.size,l.size),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,r()),g(u,i,n,o,a),B(u,i,l,0),t.drawArrays(t.TRIANGLES,0,3),l.index=(l.index+1)%2,t.bindFramebuffer(t.FRAMEBUFFER,null),t.viewport(0,0,o,a),t.clearColor(0,0,0,1),t.clear(t.COLOR_BUFFER_BIT),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,r()),g(u,i,n,o,a),B(u,i,l,1),t.drawArrays(t.TRIANGLES,0,3),requestAnimationFrame(x);return}t.viewport(0,0,o,a),t.clearColor(0,0,0,1),t.clear(t.COLOR_BUFFER_BIT),t.useProgram(i.program),t.bindVertexArray(U),g(u,i,n,o,a),t.drawArrays(t.TRIANGLES,0,3),requestAnimationFrame(x)}}w.addEventListener("click",()=>{document.body.classList.toggle("sidebar-collapsed"),V()});N.addEventListener("mousemove",()=>{Se()});document.addEventListener("keydown",Ie);document.addEventListener("visibilitychange",()=>{document.hidden||(O=performance.now())});Ae();Y(u.id);V();G(!0);requestAnimationFrame(x);

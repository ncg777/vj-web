...
`;const E=document.querySelector("#gl-canvas"),T=document.querySelector("#scene-list"),R=document.querySelector("#control-list"),S=document.querySelector("#panel-actions"),h=document.querySelector("#key-help"),U=document.querySelector("#hud-title"),A=document.querySelector("#hud-desc"),G=document.querySelector("[data-action='toggle-sidebar']"),D=document.querySelector(".stage");if(!E||!T||!R||!S||!h||!U||!A||!G||!D)throw new Error("Missing required UI elements");const t=E.getContext("webgl2",{antialias:!0});if(!t)throw D.innerHTML=`
    <div class="fallback">
      <h2>WebGL2 unavailable</h2>
      <p>Your browser or GPU does not expose WebGL2. Try a different browser.</p>
    </div>
  `,new Error("WebGL2 unavailable");const ue=t.getExtension("EXT_color_buffer_float"),ce=!!ue;t.disable(t.DEPTH_TEST);t.disable(t.BLEND);const M=se(t),L=new Map,W={},v={},z={},C=new Map;for(const e of g){const n=ae(t,j,e.fragment),i=new Set;i.add(e.resolutionUniform),i.add(e.timeUniform),e.loopUniform&&i.add(e.loopUniform),e.stateful&&(i.add(e.passUniform??"uPass"),i.add(e.stateUniform??"uState"),i.add(e.gridUniform??"uGridSize"));for(const l of e.params)i.add(l.uniform);const o=re(t,n,Array.from(i));L.set(e.id,{program:n,uniforms:o});const a={},r={};for(const l of e.params)a[l.id]=l,r[l.id]=l.type==="seed"?Math.floor(Math.random()*1e6):l.value;W[e.id]=a,v[e.id]={...r},z[e.id]={...r}}let s=g[0],k={},_=performance.now();function B(e){const n=t.createTexture();if(!n)throw new Error("Failed to create state texture");return t.bindTexture(t.TEXTURE_2D,n),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_MIN_FILTER,t.NEAREST),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_MAG_FILTER,t.NEAREST),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_WRAP_S,t.REPEAT),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_WRAP_T,t.REPEAT),t.texImage2D(t.TEXTURE_2D,0,t.RGBA16F,e,e,0,t.RGBA,t.HALF_FLOAT,null),t.bindTexture(t.TEXTURE_2D,null),n}function fe(e){const n=B(e),i=B(e),o=t.createFramebuffer(),a=t.createFramebuffer();if(!o||!a)throw new Error("Failed to create framebuffer");return t.bindFramebuffer(t.FRAMEBUFFER,o),t.framebufferTexture2D(t.FRAMEBUFFER,t.COLOR_ATTACHMENT0,t.TEXTURE_2D,n,0),t.bindFramebuffer(t.FRAMEBUFFER,a),t.framebufferTexture2D(t.FRAMEBUFFER,t.COLOR_ATTACHMENT0,t.TEXTURE_2D,i,0),t.bindFramebuffer(t.FRAMEBUFFER,null),{size:e,textures:[n,i],fbos:[o,a],index:0,needsInit:!0}}function q(e){if(!e.stateful)return null;let n=C.get(e.id);const i=e.bufferSize??192;return(!n||n.size!==i)&&(n=fe(i),C.set(e.id,n)),n}function P(e){const n=C.get(e);n&&(n.needsInit=!0)}function me(e,n){let i=n;return e.min!==void 0&&(i=Math.max(e.min,i)),e.max!==void 0&&(i=Math.min(e.max,i)),e.type==="int"&&(i=Math.round(i)),i}function O(e,n){if(e.type==="int")return String(Math.round(n));const i=e.step??.01,o=i<1?Math.min(4,Math.max(2,Math.ceil(-Math.log10(i)))):0;return n.toFixed(o)}function pe(e){return e.length===1?e.toLowerCase():e}function de(e){return e instanceof HTMLInputElement||e instanceof HTMLTextAreaElement||e instanceof HTMLSelectElement}function x(e,n,i,o=!0){const a=W[e][n];if(!a)return;const r=me(a,i);if(v[e][n]=r,e===s.id&&o){const l=k[n];l!=null&&l.range&&(l.range.value=String(r)),l!=null&&l.number&&(l.number.value=O(a,r))}}function ve(e){var n;for(const i of((n=g.find(o=>o.id===e))==null?void 0:n.params)??[])i.type==="seed"&&x(e,i.id,Math.floor(Math.random()*1e6));P(e)}function he(e){const n=z[e];for(const[i,o]of Object.entries(n))x(e,i,o,!0);P(e)}function ye(){T.innerHTML="";for(const e of g){const n=document.createElement("button");n.className="scene-button",n.textContent=e.name,n.dataset.scene=e.id,n.addEventListener("click",()=>H(e.id)),T.appendChild(n)}}function xe(e){S.innerHTML="";const n=document.createElement("button");if(n.className="ghost small",n.textContent="Reset",n.addEventListener("click",()=>he(e.id)),S.appendChild(n),e.params.some(o=>o.type==="seed")){const o=document.createElement("button");o.className="ghost small",o.textContent="Reseed",o.addEventListener("click",()=>ve(e.id)),S.appendChild(o)}}function ge(e){R.innerHTML="",k={};for(const n of e.params){if(n.type==="seed")continue;const i=document.createElement("div");i.className="control";const o=document.createElement("div");o.className="control-header";const a=document.createElement("label");if(a.textContent=n.label,o.appendChild(a),n.key){const f=document.createElement("span");f.className="key-cap",f.textContent=`${n.key.inc.toUpperCase()}/${n.key.dec.toUpperCase()}`,o.appendChild(f)}const r=document.createElement("div");r.className="control-inputs";const l=document.createElement("input");l.type="range",l.min=String(n.min??0),l.max=String(n.max??1),l.step=String(n.step??(n.type==="int"?1:.01)),l.value=String(v[e.id][n.id]),l.addEventListener("input",f=>{const p=Number(f.target.value);Number.isNaN(p)||x(e.id,n.id,p)});const u=document.createElement("input");u.type="number",u.min=l.min,u.max=l.max,u.step=l.step,u.value=O(n,v[e.id][n.id]),u.addEventListener("input",f=>{const p=Number(f.target.value);Number.isNaN(p)||x(e.id,n.id,p)}),r.appendChild(l),r.appendChild(u),i.appendChild(o),i.appendChild(r),R.appendChild(i),k[n.id]={range:l,number:u}}}function be(e){h.innerHTML="";for(const n of e.params){if(!n.key||n.type==="seed")continue;const i=document.createElement("div");i.className="key-row",i.textContent=`${n.key.inc.toUpperCase()}/${n.key.dec.toUpperCase()}  ${n.label}`,h.appendChild(i)}h.childElementCount||(h.textContent="No mapped keys for this scene.")}function H(e){const n=g.find(i=>i.id===e);n&&(s=n,n.stateful&&(q(n),P(n.id)),U.textContent=n.name,A.textContent=n.description,xe(n),ge(n),be(n),T.querySelectorAll(".scene-button").forEach(i=>{const o=i;o.classList.toggle("active",o.dataset.scene===n.id)}))}function Se(e){if(de(e.target))return;const n=pe(e.key),i=s.params;for(const o of i){if(!o.key||o.type==="seed")continue;const a=n===o.key.inc,r=n===o.key.dec;if(!a&&!r)continue;const l=e.shiftKey&&o.key.shiftStep?o.key.shiftStep:o.key.step,f=v[s.id][o.id]+l*(a?1:-1);x(s.id,o.id,f),e.preventDefault();break}}function b(e,n,i,o,a){const r=v[e.id],l=n.uniforms,u=l[e.resolutionUniform];u&&t.uniform2f(u,o,a);const f=l[e.timeUniform];if(f)if(e.timeMode==="phase"){const c=e.loopDuration??8,m=i%c/c;t.uniform1f(f,m)}else if(e.timeMode==="looped"){const c=e.loopDuration??8,m=i%c;if(t.uniform1f(f,m),e.loopUniform){const d=l[e.loopUniform];d&&t.uniform1f(d,c)}}else t.uniform1f(f,i);const p={};for(const c of e.params){const m=l[c.uniform],d=r[c.id];if(c.component!==void 0){const F=p[c.uniform]??[0,0,0];F[c.component]=d,p[c.uniform]=F;continue}m&&(c.type==="int"?t.uniform1i(m,Math.round(d)):t.uniform1f(m,d))}for(const[c,m]of Object.entries(p)){const d=l[c];d&&t.uniform3f(d,m[0],m[1],m[2])}}function w(e,n,i,o){const a=n.uniforms[e.passUniform??"uPass"];a&&t.uniform1i(a,o);const r=n.uniforms[e.gridUniform??"uGridSize"];r&&t.uniform2f(r,i.size,i.size);const l=n.uniforms[e.stateUniform??"uState"];l&&t.uniform1i(l,0)}function y(e){const n=(e-_)/1e3,{width:i,height:o}=le(E),a=L.get(s.id);if(a){if(s.stateful){if(!ce){A.textContent="Stateful scenes require float render targets (EXT_color_buffer_float).",requestAnimationFrame(y);return}const r=q(s);if(!r){requestAnimationFrame(y);return}const l=()=>r.textures[r.index],u=()=>r.fbos[(r.index+1)%2];t.useProgram(a.program),t.bindVertexArray(M),r.needsInit&&(t.bindFramebuffer(t.FRAMEBUFFER,u()),t.viewport(0,0,r.size,r.size),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,l()),b(s,a,n,i,o),w(s,a,r,2),t.drawArrays(t.TRIANGLES,0,3),r.index=(r.index+1)%2,r.needsInit=!1),t.bindFramebuffer(t.FRAMEBUFFER,u()),t.viewport(0,0,r.size,r.size),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,l()),b(s,a,n,i,o),w(s,a,r,0),t.drawArrays(t.TRIANGLES,0,3),r.index=(r.index+1)%2,t.bindFramebuffer(t.FRAMEBUFFER,null),t.viewport(0,0,i,o),t.clearColor(0,0,0,1),t.clear(t.COLOR_BUFFER_BIT),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,l()),b(s,a,n,i,o),w(s,a,r,1),t.drawArrays(t.TRIANGLES,0,3),requestAnimationFrame(y);return}t.viewport(0,0,i,o),t.clearColor(0,0,0,1),t.clear(t.COLOR_BUFFER_BIT),t.useProgram(a.program),t.bindVertexArray(M),b(s,a,n,i,o),t.drawArrays(t.TRIANGLES,0,3),requestAnimationFrame(y)}}G.addEventListener("click",()=>{document.body.classList.toggle("sidebar-collapsed")});document.addEventListener("keydown",Se);document.addEventListener("visibilitychange",()=>{document.hidden||(_=performance.now())});ye();H(s.id);requestAnimationFrame(y);

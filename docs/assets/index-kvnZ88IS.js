...
`;const A=document.querySelector("#gl-canvas"),w=document.querySelector("#scene-list"),C=document.querySelector("#control-list"),T=document.querySelector("#panel-actions"),y=document.querySelector("#key-help"),_=document.querySelector("#hud-title"),P=document.querySelector("#hud-desc"),F=document.querySelector("[data-action='toggle-ui']"),I=document.querySelector("[data-action='toggle-ui-floating']"),M=document.querySelector("[data-action='fullscreen']"),B=document.querySelector(".stage");if(!A||!w||!C||!T||!y||!_||!P||!F||!I||!M||!B)throw new Error("Missing required UI elements");const t=A.getContext("webgl2",{antialias:!0});if(!t)throw B.innerHTML=`
    <div class="fallback">
      <h2>WebGL2 unavailable</h2>
      <p>Your browser or GPU does not expose WebGL2. Try a different browser.</p>
    </div>
  `,new Error("WebGL2 unavailable");const me=t.getExtension("EXT_color_buffer_float"),de=!!me;t.disable(t.DEPTH_TEST);t.disable(t.BLEND);const L=fe(t),z=new Map,q={},h={},O={},k=new Map;for(const e of b){const n=se(t,K,e.fragment),o=new Set;o.add(e.resolutionUniform),o.add(e.timeUniform),e.loopUniform&&o.add(e.loopUniform),e.stateful&&(o.add(e.passUniform??"uPass"),o.add(e.stateUniform??"uState"),o.add(e.gridUniform??"uGridSize"));for(const l of e.params)o.add(l.uniform);const i=ue(t,n,Array.from(o));z.set(e.id,{program:n,uniforms:i});const a={},r={};for(const l of e.params)a[l.id]=l,r[l.id]=l.type==="seed"?Math.floor(Math.random()*1e6):l.value;q[e.id]=a,h[e.id]={...r},O[e.id]={...r}}let s=b[0],E={},v=!1,H=performance.now();function G(e){const n=t.createTexture();if(!n)throw new Error("Failed to create state texture");return t.bindTexture(t.TEXTURE_2D,n),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_MIN_FILTER,t.NEAREST),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_MAG_FILTER,t.NEAREST),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_WRAP_S,t.REPEAT),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_WRAP_T,t.REPEAT),t.texImage2D(t.TEXTURE_2D,0,t.RGBA16F,e,e,0,t.RGBA,t.HALF_FLOAT,null),t.bindTexture(t.TEXTURE_2D,null),n}function pe(e){const n=G(e),o=G(e),i=t.createFramebuffer(),a=t.createFramebuffer();if(!i||!a)throw new Error("Failed to create framebuffer");return t.bindFramebuffer(t.FRAMEBUFFER,i),t.framebufferTexture2D(t.FRAMEBUFFER,t.COLOR_ATTACHMENT0,t.TEXTURE_2D,n,0),t.bindFramebuffer(t.FRAMEBUFFER,a),t.framebufferTexture2D(t.FRAMEBUFFER,t.COLOR_ATTACHMENT0,t.TEXTURE_2D,o,0),t.bindFramebuffer(t.FRAMEBUFFER,null),{size:e,textures:[n,o],fbos:[i,a],index:0,needsInit:!0}}function X(e){if(!e.stateful)return null;let n=k.get(e.id);const o=e.bufferSize??192;return(!n||n.size!==o)&&(n=pe(o),k.set(e.id,n)),n}function U(e){const n=k.get(e);n&&(n.needsInit=!0)}function ve(e,n){let o=n;return e.min!==void 0&&(o=Math.max(e.min,o)),e.max!==void 0&&(o=Math.min(e.max,o)),e.type==="int"&&(o=Math.round(o)),o}function j(e,n){if(e.type==="int")return String(Math.round(n));const o=e.step??.01,i=o<1?Math.min(4,Math.max(2,Math.ceil(-Math.log10(o)))):0;return n.toFixed(i)}function he(e){return e.length===1?e.toLowerCase():e}function ye(e){return e instanceof HTMLInputElement||e instanceof HTMLTextAreaElement||e instanceof HTMLSelectElement}function x(e,n,o,i=!0){const a=q[e][n];if(!a)return;const r=ve(a,o);if(h[e][n]=r,e===s.id&&i){const l=E[n];l!=null&&l.range&&(l.range.value=String(r)),l!=null&&l.number&&(l.number.value=j(a,r))}}function ge(e){var n;for(const o of((n=b.find(i=>i.id===e))==null?void 0:n.params)??[])o.type==="seed"&&x(e,o.id,Math.floor(Math.random()*1e6));U(e)}function xe(e){const n=O[e];for(const[o,i]of Object.entries(n))x(e,o,i,!0);U(e)}function be(){w.innerHTML="";for(const e of b){const n=document.createElement("button");n.className="scene-button",n.textContent=e.name,n.dataset.scene=e.id,n.addEventListener("click",()=>V(e.id)),w.appendChild(n)}}function Se(e){T.innerHTML="";const n=document.createElement("button");if(n.className="ghost small",n.textContent="Reset",n.addEventListener("click",()=>xe(e.id)),T.appendChild(n),e.params.some(i=>i.type==="seed")){const i=document.createElement("button");i.className="ghost small",i.textContent="Reseed",i.addEventListener("click",()=>ge(e.id)),T.appendChild(i)}}function Te(e){C.innerHTML="",E={};for(const n of e.params){if(n.type==="seed")continue;const o=document.createElement("div");o.className="control";const i=document.createElement("div");i.className="control-header";const a=document.createElement("label");if(a.textContent=n.label,i.appendChild(a),n.key){const f=document.createElement("span");f.className="key-cap",f.textContent=`${n.key.inc.toUpperCase()}/${n.key.dec.toUpperCase()}`,i.appendChild(f)}const r=document.createElement("div");r.className="control-inputs";const l=document.createElement("input");l.type="range",l.min=String(n.min??0),l.max=String(n.max??1),l.step=String(n.step??(n.type==="int"?1:.01)),l.value=String(h[e.id][n.id]),l.addEventListener("input",f=>{const d=Number(f.target.value);Number.isNaN(d)||x(e.id,n.id,d)});const u=document.createElement("input");u.type="number",u.min=l.min,u.max=l.max,u.step=l.step,u.value=j(n,h[e.id][n.id]),u.addEventListener("input",f=>{const d=Number(f.target.value);Number.isNaN(d)||x(e.id,n.id,d)}),r.appendChild(l),r.appendChild(u),o.appendChild(i),o.appendChild(r),C.appendChild(o),E[n.id]={range:l,number:u}}}function we(e){y.innerHTML="";for(const n of e.params){if(!n.key||n.type==="seed")continue;const o=document.createElement("div");o.className="key-row",o.textContent=`${n.key.inc.toUpperCase()}/${n.key.dec.toUpperCase()}  ${n.label}`,y.appendChild(o)}y.childElementCount||(y.textContent="No mapped keys for this scene.")}function V(e){const n=b.find(o=>o.id===e);n&&(s=n,n.stateful&&(X(n),U(n.id)),_.textContent=n.name,P.textContent=n.description,Se(n),Te(n),we(n),w.querySelectorAll(".scene-button").forEach(o=>{const i=o;i.classList.toggle("active",i.dataset.scene===n.id)}))}function Re(e){if(ye(e.target))return;const n=he(e.key),o=s.params;for(const i of o){if(!i.key||i.type==="seed")continue;const a=n===i.key.inc,r=n===i.key.dec;if(!a&&!r)continue;const l=e.shiftKey&&i.key.shiftStep?i.key.shiftStep:i.key.step,f=h[s.id][i.id]+l*(a?1:-1);x(s.id,i.id,f),e.preventDefault();break}}function S(e,n,o,i,a){const r=h[e.id],l=n.uniforms,u=l[e.resolutionUniform];u&&t.uniform2f(u,i,a);const f=l[e.timeUniform];if(f)if(e.timeMode==="phase"){const c=e.loopDuration??8,m=o%c/c;t.uniform1f(f,m)}else if(e.timeMode==="looped"){const c=e.loopDuration??8,m=o%c;if(t.uniform1f(f,m),e.loopUniform){const p=l[e.loopUniform];p&&t.uniform1f(p,c)}}else t.uniform1f(f,o);const d={};for(const c of e.params){const m=l[c.uniform],p=r[c.id];if(c.component!==void 0){const N=d[c.uniform]??[0,0,0];N[c.component]=p,d[c.uniform]=N;continue}m&&(c.type==="int"?t.uniform1i(m,Math.round(p)):t.uniform1f(m,p))}for(const[c,m]of Object.entries(d)){const p=l[c];p&&t.uniform3f(p,m[0],m[1],m[2])}}function R(e,n,o,i){const a=n.uniforms[e.passUniform??"uPass"];a&&t.uniform1i(a,i);const r=n.uniforms[e.gridUniform??"uGridSize"];r&&t.uniform2f(r,o.size,o.size);const l=n.uniforms[e.stateUniform??"uState"];l&&t.uniform1i(l,0)}function g(e){const n=(e-H)/1e3,{width:o,height:i}=ce(A),a=z.get(s.id);if(a){if(s.stateful){if(!de){P.textContent="Stateful scenes require float render targets (EXT_color_buffer_float).",requestAnimationFrame(g);return}const r=X(s);if(!r){requestAnimationFrame(g);return}const l=()=>r.textures[r.index],u=()=>r.fbos[(r.index+1)%2];t.useProgram(a.program),t.bindVertexArray(L),r.needsInit&&(t.bindFramebuffer(t.FRAMEBUFFER,u()),t.viewport(0,0,r.size,r.size),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,l()),S(s,a,n,o,i),R(s,a,r,2),t.drawArrays(t.TRIANGLES,0,3),r.index=(r.index+1)%2,r.needsInit=!1),t.bindFramebuffer(t.FRAMEBUFFER,u()),t.viewport(0,0,r.size,r.size),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,l()),S(s,a,n,o,i),R(s,a,r,0),t.drawArrays(t.TRIANGLES,0,3),r.index=(r.index+1)%2,t.bindFramebuffer(t.FRAMEBUFFER,null),t.viewport(0,0,o,i),t.clearColor(0,0,0,1),t.clear(t.COLOR_BUFFER_BIT),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,l()),S(s,a,n,o,i),R(s,a,r,1),t.drawArrays(t.TRIANGLES,0,3),requestAnimationFrame(g);return}t.viewport(0,0,o,i),t.clearColor(0,0,0,1),t.clear(t.COLOR_BUFFER_BIT),t.useProgram(a.program),t.bindVertexArray(L),S(s,a,n,o,i),t.drawArrays(t.TRIANGLES,0,3),requestAnimationFrame(g)}}function Y(e){v=e,document.body.classList.toggle("ui-hidden",v),F.textContent=v?"Show UI":"Hide UI",I.textContent=v?"Show UI":"Hide UI"}F.addEventListener("click",()=>Y(!v));I.addEventListener("click",()=>Y(!v));M.addEventListener("click",async()=>{document.fullscreenElement?await document.exitFullscreen():await B.requestFullscreen()});document.addEventListener("keydown",Re);document.addEventListener("fullscreenchange",()=>{M.textContent=document.fullscreenElement?"Exit Fullscreen":"Fullscreen"});document.addEventListener("visibilitychange",()=>{document.hidden||(H=performance.now())});be();V(s.id);requestAnimationFrame(g);

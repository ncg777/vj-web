(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))i(a);new MutationObserver(a=>{for(const l of a)if(l.type==="childList")for(const r of l.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&i(r)}).observe(document,{childList:!0,subtree:!0});function o(a){const l={};return a.integrity&&(l.integrity=a.integrity),a.referrerPolicy&&(l.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?l.credentials="include":a.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function i(a){if(a.ep)return;a.ep=!0;const l=o(a);fetch(a.href,l)}})();const Y=`#version 300 es
...
`;const I=document.querySelector("#gl-canvas"),T=document.querySelector("#scene-list"),B=document.querySelector("#control-list"),g=document.querySelector("#panel-actions"),h=document.querySelector("#key-help"),w=document.querySelector("#hud-title"),N=document.querySelector("#hud-desc"),C=document.querySelector("[data-action='toggle-sidebar']"),G=document.querySelector(".stage");if(!I||!T||!B||!g||!h||!w||!N||!C||!G)throw new Error("Missing required UI elements");const t=I.getContext("webgl2",{antialias:!0});if(!t)throw G.innerHTML=`
    <div class="fallback">
      <h2>WebGL2 unavailable</h2>
      <p>Your browser or GPU does not expose WebGL2. Try a different browser.</p>
    </div>
  `,new Error("WebGL2 unavailable");const ve=t.getExtension("EXT_color_buffer_float"),he=!!ve;t.disable(t.DEPTH_TEST);t.disable(t.BLEND);const D=de(t),_=new Map,q={},v={},O={},P=new Map;for(const e of S){const n=fe(t,Y,e.fragment),o=new Set;o.add(e.resolutionUniform),o.add(e.timeUniform),e.loopUniform&&o.add(e.loopUniform),e.stateful&&(o.add(e.passUniform??"uPass"),o.add(e.stateUniform??"uState"),o.add(e.gridUniform??"uGridSize"));for(const r of e.params)o.add(r.uniform);const i=me(t,n,Array.from(o));_.set(e.id,{program:n,uniforms:i});const a={},l={};for(const r of e.params)a[r.id]=r,l[r.id]=r.type==="seed"?Math.floor(Math.random()*1e6):r.value;q[e.id]=a,v[e.id]={...l},O[e.id]={...l}}let u=S[0],F={},H=performance.now(),R=null,k=null;function E(e){C.classList.toggle("hidden",e)}function ye(){E(!1),k!==null&&window.clearTimeout(k),k=window.setTimeout(()=>{E(!0)},2500)}function j(){const e=document.body.classList.contains("sidebar-collapsed");C.textContent=e?">>":"<<"}function xe(){var e;(e=w.parentElement)==null||e.classList.remove("hidden"),R!==null&&window.clearTimeout(R),R=window.setTimeout(()=>{var n;(n=w.parentElement)==null||n.classList.add("hidden")},1e4)}function W(e){const n=t.createTexture();if(!n)throw new Error("Failed to create state texture");return t.bindTexture(t.TEXTURE_2D,n),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_MIN_FILTER,t.NEAREST),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_MAG_FILTER,t.NEAREST),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_WRAP_S,t.REPEAT),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_WRAP_T,t.REPEAT),t.texImage2D(t.TEXTURE_2D,0,t.RGBA16F,e,e,0,t.RGBA,t.HALF_FLOAT,null),t.bindTexture(t.TEXTURE_2D,null),n}function Se(e){const n=W(e),o=W(e),i=t.createFramebuffer(),a=t.createFramebuffer();if(!i||!a)throw new Error("Failed to create framebuffer");return t.bindFramebuffer(t.FRAMEBUFFER,i),t.framebufferTexture2D(t.FRAMEBUFFER,t.COLOR_ATTACHMENT0,t.TEXTURE_2D,n,0),t.bindFramebuffer(t.FRAMEBUFFER,a),t.framebufferTexture2D(t.FRAMEBUFFER,t.COLOR_ATTACHMENT0,t.TEXTURE_2D,o,0),t.bindFramebuffer(t.FRAMEBUFFER,null),{size:e,textures:[n,o],fbos:[i,a],index:0,needsInit:!0}}function X(e){if(!e.stateful)return null;let n=P.get(e.id);const o=e.bufferSize??192;return(!n||n.size!==o)&&(n=Se(o),P.set(e.id,n)),n}function M(e){const n=P.get(e);n&&(n.needsInit=!0)}function be(e,n){let o=n;return e.min!==void 0&&(o=Math.max(e.min,o)),e.max!==void 0&&(o=Math.min(e.max,o)),e.type==="int"&&(o=Math.round(o)),o}function V(e,n){if(e.type==="int")return String(Math.round(n));const o=e.step??.01,i=o<1?Math.min(4,Math.max(2,Math.ceil(-Math.log10(o)))):0;return n.toFixed(i)}function ge(e){return e.length===1?e.toLowerCase():e}function Te(e){return e instanceof HTMLInputElement||e instanceof HTMLTextAreaElement||e instanceof HTMLSelectElement}function x(e,n,o,i=!0){const a=q[e][n];if(!a)return;const l=be(a,o);if(v[e][n]=l,e===u.id&&i){const r=F[n];r!=null&&r.range&&(r.range.value=String(l)),r!=null&&r.number&&(r.number.value=V(a,l))}}function we(e){var n;for(const o of((n=S.find(i=>i.id===e))==null?void 0:n.params)??[])o.type==="seed"&&x(e,o.id,Math.floor(Math.random()*1e6));M(e)}function Ce(e){const n=O[e];for(const[o,i]of Object.entries(n))x(e,o,i,!0);M(e)}function Re(){T.innerHTML="";for(const e of S){const n=document.createElement("button");n.className="scene-button",n.textContent=e.name,n.dataset.scene=e.id,n.addEventListener("click",()=>{Z(e.id)}),T.appendChild(n)}}function ke(e){g.innerHTML="";const n=document.createElement("button");if(n.className="ghost small",n.textContent="Reset",n.addEventListener("click",()=>Ce(e.id)),g.appendChild(n),e.params.some(i=>i.type==="seed")){const i=document.createElement("button");i.className="ghost small",i.textContent="Reseed",i.addEventListener("click",()=>we(e.id)),g.appendChild(i)}}function Ae(e){B.innerHTML="",F={};for(const n of e.params){if(n.type==="seed")continue;const o=document.createElement("div");o.className="control";const i=document.createElement("div");i.className="control-header";const a=document.createElement("label");if(a.textContent=n.label,i.appendChild(a),n.key){const f=document.createElement("span");f.className="key-cap",f.textContent=`${n.key.inc.toUpperCase()}/${n.key.dec.toUpperCase()}`,i.appendChild(f)}const l=document.createElement("div");l.className="control-inputs";const r=document.createElement("input");r.type="range",r.min=String(n.min??0),r.max=String(n.max??1),r.step=String(n.step??(n.type==="int"?1:.01)),r.value=String(v[e.id][n.id]),r.addEventListener("input",f=>{const p=Number(f.target.value);Number.isNaN(p)||x(e.id,n.id,p)});const s=document.createElement("input");s.type="number",s.min=r.min,s.max=r.max,s.step=r.step,s.value=V(n,v[e.id][n.id]),s.addEventListener("input",f=>{const p=Number(f.target.value);Number.isNaN(p)||x(e.id,n.id,p)}),l.appendChild(r),l.appendChild(s),o.appendChild(i),o.appendChild(l),B.appendChild(o),F[n.id]={range:r,number:s}}}function Be(e){h.innerHTML="";for(const n of e.params){if(!n.key||n.type==="seed")continue;const o=document.createElement("div");o.className="key-row",o.textContent=`${n.key.inc.toUpperCase()}/${n.key.dec.toUpperCase()}  ${n.label}`,h.appendChild(o)}h.childElementCount||(h.textContent="No mapped keys for this scene.")}function Z(e){const n=S.find(o=>o.id===e);n&&(u=n,n.stateful&&(X(n),M(n.id)),w.textContent=n.name,N.textContent=n.description,xe(),ke(n),Ae(n),Be(n),T.querySelectorAll(".scene-button").forEach(o=>{const i=o;i.classList.toggle("active",i.dataset.scene===n.id)}))}function Pe(e){if(Te(e.target))return;const n=ge(e.key),o=u.params;for(const i of o){if(!i.key||i.type==="seed")continue;const a=n===i.key.inc,l=n===i.key.dec;if(!a&&!l)continue;const r=e.shiftKey&&i.key.shiftStep?i.key.shiftStep:i.key.step,f=v[u.id][i.id]+r*(a?1:-1);x(u.id,i.id,f),e.preventDefault();break}}function b(e,n,o,i,a){const l=v[e.id],r=n.uniforms,s=r[e.resolutionUniform];s&&t.uniform2f(s,i,a);const f=r[e.timeUniform];if(f)if(e.timeMode==="phase"){const c=e.loopDuration??8,m=o%c/c;t.uniform1f(f,m)}else if(e.timeMode==="looped"){const c=e.loopDuration??8,m=o%c;if(t.uniform1f(f,m),e.loopUniform){const d=r[e.loopUniform];d&&t.uniform1f(d,c)}}else t.uniform1f(f,o);const p={};for(const c of e.params){const m=r[c.uniform],d=l[c.id];if(c.component!==void 0){const U=p[c.uniform]??[0,0,0];U[c.component]=d,p[c.uniform]=U;continue}m&&(c.type==="int"?t.uniform1i(m,Math.round(d)):t.uniform1f(m,d))}for(const[c,m]of Object.entries(p)){const d=r[c];d&&t.uniform3f(d,m[0],m[1],m[2])}}function A(e,n,o,i){const a=n.uniforms[e.passUniform??"uPass"];a&&t.uniform1i(a,i);const l=n.uniforms[e.gridUniform??"uGridSize"];l&&t.uniform2f(l,o.size,o.size);const r=n.uniforms[e.stateUniform??"uState"];r&&t.uniform1i(r,0)}function y(e){const n=(e-H)/1e3,{width:o,height:i}=pe(I),a=_.get(u.id);if(a){if(u.stateful){if(!he){N.textContent="Stateful scenes require float render targets (EXT_color_buffer_float).",requestAnimationFrame(y);return}const l=X(u);if(!l){requestAnimationFrame(y);return}const r=()=>l.textures[l.index],s=()=>l.fbos[(l.index+1)%2];t.useProgram(a.program),t.bindVertexArray(D),l.needsInit&&(t.bindFramebuffer(t.FRAMEBUFFER,s()),t.viewport(0,0,l.size,l.size),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,r()),b(u,a,n,o,i),A(u,a,l,2),t.drawArrays(t.TRIANGLES,0,3),l.index=(l.index+1)%2,l.needsInit=!1),t.bindFramebuffer(t.FRAMEBUFFER,s()),t.viewport(0,0,l.size,l.size),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,r()),b(u,a,n,o,i),A(u,a,l,0),t.drawArrays(t.TRIANGLES,0,3),l.index=(l.index+1)%2,t.bindFramebuffer(t.FRAMEBUFFER,null),t.viewport(0,0,o,i),t.clearColor(0,0,0,1),t.clear(t.COLOR_BUFFER_BIT),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,r()),b(u,a,n,o,i),A(u,a,l,1),t.drawArrays(t.TRIANGLES,0,3),requestAnimationFrame(y);return}t.viewport(0,0,o,i),t.clearColor(0,0,0,1),t.clear(t.COLOR_BUFFER_BIT),t.useProgram(a.program),t.bindVertexArray(D),b(u,a,n,o,i),t.drawArrays(t.TRIANGLES,0,3),requestAnimationFrame(y)}}C.addEventListener("click",()=>{document.body.classList.toggle("sidebar-collapsed"),j()});G.addEventListener("mousemove",()=>{ye()});document.addEventListener("keydown",Pe);document.addEventListener("visibilitychange",()=>{document.hidden||(H=performance.now())});Re();Z(u.id);j();E(!0);requestAnimationFrame(y);

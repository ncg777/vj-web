...
`;const E=document.querySelector("#gl-canvas"),w=document.querySelector("#scene-list"),k=document.querySelector("#control-list"),T=document.querySelector("#panel-actions"),g=document.querySelector("#key-help"),_=document.querySelector("#hud-title"),F=document.querySelector("#hud-desc"),U=document.querySelector("[data-action='toggle-ui']"),N=document.querySelector("[data-action='toggle-ui-floating']"),P=document.querySelector("[data-action='fullscreen']"),I=document.querySelector(".stage");if(!E||!w||!k||!T||!g||!_||!F||!U||!N||!P||!I)throw new Error("Missing required UI elements");const t=E.getContext("webgl2",{antialias:!0});if(!t)throw I.innerHTML=`
    <div class="fallback">
      <h2>WebGL2 unavailable</h2>
      <p>Your browser or GPU does not expose WebGL2. Try a different browser.</p>
    </div>
  `,new Error("WebGL2 unavailable");const ce=t.getExtension("EXT_color_buffer_float"),fe=!!ce;t.disable(t.DEPTH_TEST);t.disable(t.BLEND);const L=ue(t),q=new Map,z={},h={},O={},C=new Map;for(const e of y){const n=re(t,$,e.fragment),i=new Set;i.add(e.resolutionUniform),i.add(e.timeUniform),e.loopUniform&&i.add(e.loopUniform),e.stateful&&(i.add(e.passUniform??"uPass"),i.add(e.stateUniform??"uState"),i.add(e.gridUniform??"uGridSize"));for(const l of e.params)i.add(l.uniform);const o=le(t,n,Array.from(i));q.set(e.id,{program:n,uniforms:o});const a={},r={};for(const l of e.params)a[l.id]=l,r[l.id]=l.type==="seed"?Math.floor(Math.random()*1e6):l.value;z[e.id]=a,h[e.id]={...r},O[e.id]={...r}}let s=y[0],A={},v=!1,H=performance.now();function W(e){const n=t.createTexture();if(!n)throw new Error("Failed to create state texture");return t.bindTexture(t.TEXTURE_2D,n),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_MIN_FILTER,t.NEAREST),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_MAG_FILTER,t.NEAREST),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_WRAP_S,t.REPEAT),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_WRAP_T,t.REPEAT),t.texImage2D(t.TEXTURE_2D,0,t.RGBA16F,e,e,0,t.RGBA,t.HALF_FLOAT,null),t.bindTexture(t.TEXTURE_2D,null),n}function me(e){const n=W(e),i=W(e),o=t.createFramebuffer(),a=t.createFramebuffer();if(!o||!a)throw new Error("Failed to create framebuffer");return t.bindFramebuffer(t.FRAMEBUFFER,o),t.framebufferTexture2D(t.FRAMEBUFFER,t.COLOR_ATTACHMENT0,t.TEXTURE_2D,n,0),t.bindFramebuffer(t.FRAMEBUFFER,a),t.framebufferTexture2D(t.FRAMEBUFFER,t.COLOR_ATTACHMENT0,t.TEXTURE_2D,i,0),t.bindFramebuffer(t.FRAMEBUFFER,null),{size:e,textures:[n,i],fbos:[o,a],index:0,needsInit:!0}}function j(e){if(!e.stateful)return null;let n=C.get(e.id);const i=e.bufferSize??192;return(!n||n.size!==i)&&(n=me(i),C.set(e.id,n)),n}function D(e){const n=C.get(e);n&&(n.needsInit=!0)}function de(e,n){let i=n;return e.min!==void 0&&(i=Math.max(e.min,i)),e.max!==void 0&&(i=Math.min(e.max,i)),e.type==="int"&&(i=Math.round(i)),i}function V(e,n){if(e.type==="int")return String(Math.round(n));const i=e.step??.01,o=i<1?Math.min(4,Math.max(2,Math.ceil(-Math.log10(i)))):0;return n.toFixed(o)}function pe(e){return e.length===1?e.toLowerCase():e}function ve(e){return e instanceof HTMLInputElement||e instanceof HTMLTextAreaElement||e instanceof HTMLSelectElement}function x(e,n,i,o=!0){const a=z[e][n];if(!a)return;const r=de(a,i);if(h[e][n]=r,e===s.id&&o){const l=A[n];l!=null&&l.range&&(l.range.value=String(r)),l!=null&&l.number&&(l.number.value=V(a,r))}}function he(e){var n;for(const i of((n=y.find(o=>o.id===e))==null?void 0:n.params)??[])i.type==="seed"&&x(e,i.id,Math.floor(Math.random()*1e6));D(e)}function ge(e){const n=O[e];for(const[i,o]of Object.entries(n))x(e,i,o,!0);D(e)}function be(){w.innerHTML="";for(const e of y){const n=document.createElement("button");n.className="scene-button",n.textContent=e.name,n.dataset.scene=e.id,n.addEventListener("click",()=>X(e.id)),w.appendChild(n)}}function xe(e){T.innerHTML="";const n=document.createElement("button");if(n.className="ghost small",n.textContent="Reset",n.addEventListener("click",()=>ge(e.id)),T.appendChild(n),e.params.some(o=>o.type==="seed")){const o=document.createElement("button");o.className="ghost small",o.textContent="Reseed",o.addEventListener("click",()=>he(e.id)),T.appendChild(o)}}function ye(e){k.innerHTML="",A={};for(const n of e.params){if(n.type==="seed")continue;const i=document.createElement("div");i.className="control";const o=document.createElement("div");o.className="control-header";const a=document.createElement("label");if(a.textContent=n.label,o.appendChild(a),n.key){const f=document.createElement("span");f.className="key-cap",f.textContent=`${n.key.inc.toUpperCase()}/${n.key.dec.toUpperCase()}`,o.appendChild(f)}const r=document.createElement("div");r.className="control-inputs";const l=document.createElement("input");l.type="range",l.min=String(n.min??0),l.max=String(n.max??1),l.step=String(n.step??(n.type==="int"?1:.01)),l.value=String(h[e.id][n.id]),l.addEventListener("input",f=>{const d=Number(f.target.value);Number.isNaN(d)||x(e.id,n.id,d)});const u=document.createElement("input");u.type="number",u.min=l.min,u.max=l.max,u.step=l.step,u.value=V(n,h[e.id][n.id]),u.addEventListener("input",f=>{const d=Number(f.target.value);Number.isNaN(d)||x(e.id,n.id,d)}),r.appendChild(l),r.appendChild(u),i.appendChild(o),i.appendChild(r),k.appendChild(i),A[n.id]={range:l,number:u}}}function Se(e){g.innerHTML="";for(const n of e.params){if(!n.key||n.type==="seed")continue;const i=document.createElement("div");i.className="key-row",i.textContent=`${n.key.inc.toUpperCase()}/${n.key.dec.toUpperCase()}  ${n.label}`,g.appendChild(i)}g.childElementCount||(g.textContent="No mapped keys for this scene.")}function X(e){const n=y.find(i=>i.id===e);n&&(s=n,n.stateful&&(j(n),D(n.id)),_.textContent=n.name,F.textContent=n.description,xe(n),ye(n),Se(n),w.querySelectorAll(".scene-button").forEach(i=>{const o=i;o.classList.toggle("active",o.dataset.scene===n.id)}))}function Te(e){if(ve(e.target))return;const n=pe(e.key),i=s.params;for(const o of i){if(!o.key||o.type==="seed")continue;const a=n===o.key.inc,r=n===o.key.dec;if(!a&&!r)continue;const l=e.shiftKey&&o.key.shiftStep?o.key.shiftStep:o.key.step,f=h[s.id][o.id]+l*(a?1:-1);x(s.id,o.id,f),e.preventDefault();break}}function S(e,n,i,o,a){const r=h[e.id],l=n.uniforms,u=l[e.resolutionUniform];u&&t.uniform2f(u,o,a);const f=l[e.timeUniform];if(f)if(e.timeMode==="phase"){const c=e.loopDuration??8,m=i%c/c;t.uniform1f(f,m)}else if(e.timeMode==="looped"){const c=e.loopDuration??8,m=i%c;if(t.uniform1f(f,m),e.loopUniform){const p=l[e.loopUniform];p&&t.uniform1f(p,c)}}else t.uniform1f(f,i);const d={};for(const c of e.params){const m=l[c.uniform],p=r[c.id];if(c.component!==void 0){const M=d[c.uniform]??[0,0,0];M[c.component]=p,d[c.uniform]=M;continue}m&&(c.type==="int"?t.uniform1i(m,Math.round(p)):t.uniform1f(m,p))}for(const[c,m]of Object.entries(d)){const p=l[c];p&&t.uniform3f(p,m[0],m[1],m[2])}}function R(e,n,i,o){const a=n.uniforms[e.passUniform??"uPass"];a&&t.uniform1i(a,o);const r=n.uniforms[e.gridUniform??"uGridSize"];r&&t.uniform2f(r,i.size,i.size);const l=n.uniforms[e.stateUniform??"uState"];l&&t.uniform1i(l,0)}function b(e){const n=(e-H)/1e3,{width:i,height:o}=se(E),a=q.get(s.id);if(a){if(s.stateful){if(!fe){F.textContent="Stateful scenes require float render targets (EXT_color_buffer_float).",requestAnimationFrame(b);return}const r=j(s);if(!r){requestAnimationFrame(b);return}const l=()=>r.textures[r.index],u=()=>r.fbos[(r.index+1)%2];t.useProgram(a.program),t.bindVertexArray(L),r.needsInit&&(t.bindFramebuffer(t.FRAMEBUFFER,u()),t.viewport(0,0,r.size,r.size),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,l()),S(s,a,n,i,o),R(s,a,r,2),t.drawArrays(t.TRIANGLES,0,3),r.index=(r.index+1)%2,r.needsInit=!1),t.bindFramebuffer(t.FRAMEBUFFER,u()),t.viewport(0,0,r.size,r.size),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,l()),S(s,a,n,i,o),R(s,a,r,0),t.drawArrays(t.TRIANGLES,0,3),r.index=(r.index+1)%2,t.bindFramebuffer(t.FRAMEBUFFER,null),t.viewport(0,0,i,o),t.clearColor(0,0,0,1),t.clear(t.COLOR_BUFFER_BIT),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,l()),S(s,a,n,i,o),R(s,a,r,1),t.drawArrays(t.TRIANGLES,0,3),requestAnimationFrame(b);return}t.viewport(0,0,i,o),t.clearColor(0,0,0,1),t.clear(t.COLOR_BUFFER_BIT),t.useProgram(a.program),t.bindVertexArray(L),S(s,a,n,i,o),t.drawArrays(t.TRIANGLES,0,3),requestAnimationFrame(b)}}function K(e){v=e,document.body.classList.toggle("ui-hidden",v),U.textContent=v?"Show UI":"Hide UI",N.textContent=v?"Show UI":"Hide UI"}U.addEventListener("click",()=>K(!v));N.addEventListener("click",()=>K(!v));P.addEventListener("click",async()=>{document.fullscreenElement?await document.exitFullscreen():await I.requestFullscreen()});document.addEventListener("keydown",Te);document.addEventListener("fullscreenchange",()=>{P.textContent=document.fullscreenElement?"Exit Fullscreen":"Fullscreen"});document.addEventListener("visibilitychange",()=>{document.hidden||(H=performance.now())});be();X(s.id);requestAnimationFrame(b);

...
uniform float uTime;\r
\r
uniform int uDripCount;\r
uniform float uCycleMin;\r
uniform float uCycleMax;\r
uniform float uTravelGammaMin;\r
uniform float uTravelGammaMax;\r
uniform float uRadiusMin;\r
uniform float uRadiusMax;\r
uniform float uTrailLengthMin;\r
uniform float uTrailLengthMax;\r
uniform float uTrailWidthMin;\r
uniform float uTrailWidthMax;\r
uniform float uLaneJitter;\r
uniform float uSwayAmplitude;\r
uniform float uSwayFrequency;\r
uniform float uDetailSwayAmplitude;\r
uniform float uDetailSwayFrequency;\r
uniform float uSwayTimeSpeed;\r
uniform float uTwistAmount;\r
uniform float uTwistFrequency;\r
uniform float uTwirlSpeed;\r
uniform float uTwirlFalloff;\r
uniform float uFlowTilt;\r
uniform float uBleedStrength;\r
uniform float uSatelliteStrength;\r
uniform int uSatelliteCount;\r
uniform float uSatelliteSpread;\r
uniform float uSatelliteSizeMin;\r
uniform float uSatelliteSizeMax;\r
...
uniform float uPaperFiberAmount;\r
...
const float MIN_FALLOFF = 0.01;\r
const float SATELLITE_TRAIL_BASE = 0.3;\r
const float SATELLITE_TRAIL_STEP = 0.35;\r
const float SATELLITE_LAG_SPAN = 0.08;\r
const int MAX_DRIP_COUNT = 36;\r
const int MAX_SATELLITES = 4;\r
\r
...
\r
float signedNoise(vec2 p) {\r
  return noise(p) * 2.0 - 1.0;\r
}\r
\r
float fbm(vec2 p) {\r
  float value = 0.0;\r
...
\r
float twirlOffset(float y, float phase) {\r
  return sin(y * uTwistFrequency * TAU + phase) * uTwistAmount * pow(clamp(y, 0.0, 1.0), max(uTwirlFalloff, MIN_FALLOFF));\r
}\r
\r
float brushSpine(float y, float fi, float lane, float sway, float detailAmp, float detailFreq, float twistPhase, float timeOffset) {\r
  float clampedY = clamp(y, 0.0, 1.0);\r
  float laneOffset = (hash11(fi * 37.1) - 0.5) * uLaneJitter;\r
  float gesture = signedNoise(vec2(clampedY * 0.92 + fi * 0.17, fi * 0.41 + 1.7));\r
  float gestureDetail = signedNoise(vec2(clampedY * 2.8 + fi * 0.13, fi * 0.83 + 6.1));\r
  float gestureAmp = mix(0.018, 0.085, hash11(fi * 61.3)) * (0.4 + uSwayAmplitude * 3.4);\r
  float arc = sway * 1.35 * sin(clampedY * (uSwayFrequency * 0.58 + 0.9) + fi * 1.9 + timeOffset * 0.35);\r
  float detailWave = detailAmp * 1.8 * sin(clampedY * (detailFreq * 0.55 + 2.4) + fi * 4.1 + timeOffset * mix(0.12, 0.28, hash11(fi * 29.3)));\r
  float inkDrag = gesture * gestureAmp + gestureDetail * detailAmp * 4.4;\r
  float twist = twirlOffset(clampedY, twistPhase);\r
  float flow = (clampedY - 0.5) * uFlowTilt;\r
  return lane + laneOffset + arc + detailWave + inkDrag + twist + flow;\r
}\r
\r
void main() {\r
...
\r
  ...
  float fibers = fbm(vec2(uv.x * 2.5 + pulp * 0.4, uv.y * 55.0));\r
...
\r
  ...
  paper += vec3(0.035, 0.032, 0.026) * smoothstep(0.52, 0.9, fibers) * uPaperFiberAmount;\r
...
\r
  vec3 color = paper;\r
\r
  int dripCount = clamp(uDripCount, 1, MAX_DRIP_COUNT);\r
  int satelliteCount = clamp(uSatelliteCount, 0, MAX_SATELLITES);\r
\r
  for (int i = 0; i < MAX_DRIP_COUNT; i++) {\r
    if (i >= dripCount) {\r
      continue;\r
    }\r
\r
    float fi = float(i) + 1.0;\r
    float lane = mix(0.08, 0.92, hash11(fi * 7.13));\r
    float cycle = mix(uCycleMin, uCycleMax, hash11(fi * 2.71));\r
    float phase = fract(uTime / max(cycle, EPSILON) + hash11(fi * 11.9));\r
    float dripActive = smoothstep(0.01, 0.08, phase) * (1.0 - smoothstep(0.8, 0.98, phase));\r
    float travel = pow(phase, mix(uTravelGammaMin, uTravelGammaMax, hash11(fi * 5.37)));\r
    float headY = mix(-0.24, 1.16, travel);\r
    float radius = mix(uRadiusMin, uRadiusMax, hash11(fi * 17.2));\r
    float trailLength = mix(uTrailLengthMin, uTrailLengthMax, hash11(fi * 19.4)) * smoothstep(0.03, 0.35, phase);\r
\r
    float sway = (hash11(fi * 23.1) - 0.5) * uSwayAmplitude;\r
    float detailAmp = uDetailSwayAmplitude * mix(0.7, 1.3, hash11(fi * 29.3));\r
    float detailFreq = uDetailSwayFrequency * mix(0.7, 1.3, hash11(fi * 31.7));\r
    float twistPhase = fi * 0.7 + uTime * uTwirlSpeed;\r
    float staticSpine = brushSpine(uv.y, fi, lane, sway, detailAmp, detailFreq, twistPhase, 0.0);\r
    float headSpine = brushSpine(headY, fi, lane, sway, detailAmp, detailFreq, twistPhase, uTime * uSwayTimeSpeed);\r
\r
    float dx = abs(uv.x - staticSpine);\r
    float trailWidth = radius * mix(uTrailWidthMin, uTrailWidthMax, hash11(fi * 31.7));\r
    float driedCore = smoothstep(trailWidth * 2.8, trailWidth * 0.55, dx);\r
    float driedMask =\r
      driedCore *\r
      smoothstep(-0.05, 0.05, uv.y) *\r
      (1.0 - smoothstep(0.96, 1.12, uv.y)) *\r
      mix(0.5, 1.0, fbm(vec2(fi * 0.71, uv.y * 6.0 + dx * 14.0)));\r
    float alongTrail = clamp((headY - uv.y) / max(trailLength, EPSILON), 0.0, 1.0);\r
    float tipBlend = smoothstep(0.02, 0.18, alongTrail);\r
    float trailSpine = mix(headSpine, staticSpine, tipBlend);\r
    float trailDx = abs(uv.x - trailSpine);\r
    float taperedWidth = trailWidth * mix(0.22, 1.0, smoothstep(-0.02, 0.18, alongTrail));\r
    float trailCore = smoothstep(taperedWidth * 2.1, taperedWidth * 0.52, trailDx);\r
    float trailMask =\r
      trailCore *\r
      smoothstep(-0.02, 0.06, alongTrail) *\r
      (1.0 - smoothstep(1.0, 1.08, alongTrail)) *\r
      dripActive;\r
\r
    float bleed =\r
      exp(-pow(trailDx / (radius * 3.6), 2.0)) *\r
      smoothstep(-0.05, headY + radius * 0.8, uv.y) *\r
      (0.45 + 0.55 * fbm(vec2(fi * 0.7, uv.y * 7.0 + trailDx * 9.0))) *\r
      uBleedStrength;\r
\r
    float soakWidth = trailWidth * mix(3.2, 5.4, 0.5 + 0.5 * grain);\r
    float soakMask =\r
      exp(-pow(trailDx / max(soakWidth, EPSILON), 2.0)) *\r
      smoothstep(headY - trailLength - 0.08, headY + radius * 0.2, uv.y) *\r
      (0.35 + 0.65 * grain + 0.18 * pulp) *\r
      (0.35 + 0.65 * dripActive) *\r
      uBleedStrength;\r
\r
    float satellites = 0.0;\r
    for (int j = 0; j < MAX_SATELLITES; j++) {\r
      if (j >= satelliteCount) {\r
        continue;\r
      }\r
      float fj = float(j) + 1.0;\r
      float lag = fract(phase + 0.23 * fj + hash11(fi * 41.3 + fj));\r
      float satY = headY - trailLength * (SATELLITE_TRAIL_BASE + SATELLITE_TRAIL_STEP * fj) - lag * SATELLITE_LAG_SPAN;\r
      float satX = headSpine + (hash11(fi * 43.7 + fj) - 0.5) * trailWidth * uSatelliteSpread;\r
      float satR = radius * mix(uSatelliteSizeMin, uSatelliteSizeMax, hash11(fi * 47.1 + fj));\r
      vec2 satOffset = vec2((uv.x - satX) / satR, (uv.y - satY) / (satR * 1.2));\r
      satellites += (1.0 - smoothstep(0.8, 1.4, length(satOffset))) * dripActive * uSatelliteStrength;\r
    }\r
\r
    float dryMask = clamp(driedMask * 0.92 + bleed * 0.18 + soakMask * 0.22, 0.0, 1.0);\r
    float wetMask = clamp(trailMask * 0.98 + bleed * 0.34 + soakMask * 0.58 + satellites * 0.18, 0.0, 1.0);\r
    wetMask *= dripActive;\r
\r
...
\r
    float pooling = clamp(trailMask * 0.5 + soakMask * 0.55 + dryMask * 0.24, 0.0, 1.0);\r
    ...
    vec3 soakWash = mix(seepTint, pigment * 0.8, clamp(soakMask * 0.72 + bleed * 0.18, 0.0, 1.0));\r
    vec3 wash = mix(edgeTint, pigment * 0.9, pooling);\r
    color = mix(color, dryWash, dryMask * absorb * 0.5 * uDryMix);\r
    color = mix(color, soakWash, clamp(soakMask, 0.0, 1.0) * absorb * 0.32 * uWetMix);\r
...
`;const k=document.querySelector("#gl-canvas"),ee=document.querySelector("#scene-list"),nt=document.querySelector("#control-list"),Me=document.querySelector("#panel-actions"),se=document.querySelector("#key-help"),Ge=document.querySelector("#hud-title"),$t=document.querySelector("#hud-desc"),Ne=document.querySelector("[data-action='toggle-sidebar']"),yt=document.querySelector(".stage");if(!k||!ee||!nt||!Me||!se||!Ge||!$t||!Ne||!yt)throw new Error("Missing required UI elements");const s=k.getContext("webgl2",{antialias:!0});if(!s)throw yt.innerHTML=`
    <div class="fallback">
      <h2>WebGL2 unavailable</h2>
      <p>Your browser or GPU does not expose WebGL2. Try a different browser.</p>
    </div>
  `,new Error("WebGL2 unavailable");const Xi=s.getExtension("EXT_color_buffer_float"),Yi=!!Xi;s.disable(s.DEPTH_TEST);s.disable(s.BLEND);const Bt=_n(s),Zt=new Map,Kt={},ae={},Qt={},it=new Map;for(const e of oe){const t=Ln(s,cn,e.fragment),n=new Set;n.add(e.resolutionUniform),n.add(e.timeUniform),e.loopUniform&&n.add(e.loopUniform),e.stateful&&(n.add(e.passUniform??"uPass"),n.add(e.stateUniform??"uState"),n.add(e.gridUniform??"uGridSize"));for(const l of e.params)n.add(l.uniform);const a=Gn(s,t,Array.from(n));Zt.set(e.id,{program:t,uniforms:a});const o={},r={};for(const l of e.params)o[l.id]=l,r[l.id]=l.type==="seed"?Math.floor(Math.random()*1e6):l.value;Kt[e.id]=o,ae[e.id]={...r},Qt[e.id]={...r}}let W=oe[0],at={},bt=performance.now(),He=null,Ve=null,X=null,we=[],Jt=0,Fe=null,be=!1;function $i(){const e=["video/webm;codecs=vp9","video/webm;codecs=vp8","video/webm","video/mp4"];for(const t of e)if(MediaRecorder.isTypeSupported(t))return t;return""}function Zi(e){return e.startsWith("video/mp4")?"mp4":"webm"}function Ki(e){const t=Math.floor(e/1e3),n=String(Math.floor(t/60)).padStart(2,"0"),a=String(t%60).padStart(2,"0");return`${n}:${a}`}function Qi(){const e=document.getElementById("rec-badge");!e||!be||(e.textContent=`⏺ ${Ki(performance.now()-Jt)}`)}function Ji(){const e=$i();if(!e){alert("Recording is not supported in this browser.");return}const t=k.captureStream(60);we=[],X=new MediaRecorder(t,{mimeType:e,videoBitsPerSecond:16e6}),X.ondataavailable=n=>{n.data.size>0&&we.push(n.data)},X.onstop=()=>{const n=Zi(e),a=new Blob(we,{type:e}),o=URL.createObjectURL(a),r=document.createElement("a");r.href=o,r.download=`${W.id}-${Date.now()}.${n}`,r.click(),URL.revokeObjectURL(o),we=[]},X.start(500),be=!0,Jt=performance.now(),en(),Fe=window.setInterval(Qi,250)}function ea(){X&&X.state!=="inactive"&&X.stop(),be=!1,Fe!==null&&(clearInterval(Fe),Fe=null),en()}function ta(){be?ea():Ji()}function en(){const e=document.getElementById("rec-btn"),t=document.getElementById("rec-badge");!e||!t||(be?(e.textContent="Stop",e.classList.add("recording"),t.classList.remove("hidden")):(e.textContent="Record",e.classList.remove("recording"),t.classList.add("hidden"),t.textContent=""))}let Ue=!1;function ot(e){Ne.classList.toggle("hidden",e)}function na(){ot(!1),Ve!==null&&window.clearTimeout(Ve),Ve=window.setTimeout(()=>{ot(!0)},2500)}function tn(){const e=document.body.classList.contains("sidebar-collapsed");Ne.textContent=e?">>":"<<"}function ia(){var e;(e=Ge.parentElement)==null||e.classList.remove("hidden"),He!==null&&window.clearTimeout(He),He=window.setTimeout(()=>{var t;(t=Ge.parentElement)==null||t.classList.add("hidden")},1e4)}function Mt(e){const t=s.createTexture();if(!t)throw new Error("Failed to create state texture");return s.bindTexture(s.TEXTURE_2D,t),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MIN_FILTER,s.NEAREST),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MAG_FILTER,s.NEAREST),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_S,s.REPEAT),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_T,s.REPEAT),s.texImage2D(s.TEXTURE_2D,0,s.RGBA16F,e,e,0,s.RGBA,s.HALF_FLOAT,null),s.bindTexture(s.TEXTURE_2D,null),t}function aa(e){const t=Mt(e),n=Mt(e),a=s.createFramebuffer(),o=s.createFramebuffer();if(!a||!o)throw new Error("Failed to create framebuffer");return s.bindFramebuffer(s.FRAMEBUFFER,a),s.framebufferTexture2D(s.FRAMEBUFFER,s.COLOR_ATTACHMENT0,s.TEXTURE_2D,t,0),s.bindFramebuffer(s.FRAMEBUFFER,o),s.framebufferTexture2D(s.FRAMEBUFFER,s.COLOR_ATTACHMENT0,s.TEXTURE_2D,n,0),s.bindFramebuffer(s.FRAMEBUFFER,null),{size:e,textures:[t,n],fbos:[a,o],index:0,needsInit:!0}}function St(e){if(!e.stateful)return null;let t=it.get(e.id);const n=e.bufferSize??192;return(!t||t.size!==n)&&(t=aa(n),it.set(e.id,t)),t}function he(e){const t=it.get(e);t&&(t.needsInit=!0)}function oa(e,t){let n=t;return e.min!==void 0&&(n=Math.max(e.min,n)),e.max!==void 0&&(n=Math.min(e.max,n)),e.type==="int"&&(n=Math.round(n)),n}function nn(e,t){if(e.type==="int")return String(Math.round(t));const n=e.step??.01,a=n<1?Math.min(4,Math.max(2,Math.ceil(-Math.log10(n)))):0;return t.toFixed(a)}function ra(e){return e.length===1?e.toLowerCase():e}function la(e){return e instanceof HTMLInputElement||e instanceof HTMLTextAreaElement||e instanceof HTMLSelectElement}function ve(e,t,n,a=!0){const o=Kt[e][t];if(!o)return;const r=oa(o,n);if(ae[e][t]=r,e===W.id&&a){const l=at[t];l!=null&&l.range&&(l.range.value=String(r)),l!=null&&l.number&&(l.number.value=nn(o,r))}}function sa(e){var t;for(const n of((t=oe.find(a=>a.id===e))==null?void 0:t.params)??[])n.type==="seed"&&ve(e,n.id,Math.floor(Math.random()*1e6));he(e)}function ua(e){const t=Qt[e];for(const[n,a]of Object.entries(t))ve(e,n,a,!0);he(e)}function fa(){ee.innerHTML="";for(const e of oe){const t=document.createElement("option");t.value=e.id,t.textContent=e.name,ee.appendChild(t)}ee.addEventListener("change",()=>{an(ee.value)})}function ma(e){Me.innerHTML="";const t=document.createElement("button");if(t.className="ghost small",t.textContent="Reset",t.addEventListener("click",()=>ua(e.id)),Me.appendChild(t),e.params.some(a=>a.type==="seed")){const a=document.createElement("button");a.className="ghost small",a.textContent="Reseed",a.addEventListener("click",()=>sa(e.id)),Me.appendChild(a)}}function ca(e){nt.innerHTML="",at={};for(const t of e.params){if(t.type==="seed")continue;const n=document.createElement("div");n.className="control";const a=document.createElement("div");a.className="control-header";const o=document.createElement("label");if(o.textContent=t.label,a.appendChild(o),t.key){const p=document.createElement("span");p.className="key-cap",p.textContent=`${t.key.inc.toUpperCase()}/${t.key.dec.toUpperCase()}`,a.appendChild(p)}const r=document.createElement("div");r.className="control-inputs";const l=document.createElement("input");l.type="range",l.min=String(t.min??0),l.max=String(t.max??1),l.step=String(t.step??(t.type==="int"?1:.01)),l.value=String(ae[e.id][t.id]),l.addEventListener("input",p=>{const y=Number(p.target.value);Number.isNaN(y)||ve(e.id,t.id,y)});const u=document.createElement("input");u.type="number",u.min=l.min,u.max=l.max,u.step=l.step,u.value=nn(t,ae[e.id][t.id]),u.addEventListener("input",p=>{const y=Number(p.target.value);Number.isNaN(y)||ve(e.id,t.id,y)}),r.appendChild(l),r.appendChild(u),n.appendChild(a),n.appendChild(r),nt.appendChild(n),at[t.id]={range:l,number:u}}}function pa(e){se.innerHTML="";for(const t of e.params){if(!t.key||t.type==="seed")continue;const n=document.createElement("div");n.className="key-row",n.textContent=`${t.key.inc.toUpperCase()}/${t.key.dec.toUpperCase()}  ${t.label}`,se.appendChild(n)}se.childElementCount||(se.textContent="No mapped keys for this scene.")}function an(e){const t=oe.find(n=>n.id===e);t&&(W=t,t.stateful&&(St(t),he(t.id)),Ge.textContent=t.name,$t.textContent=t.description,ia(),ma(t),ca(t),pa(t),ee.value=t.id)}function da(e){if(la(e.target))return;const t=ra(e.key),n=W.params;for(const a of n){if(!a.key||a.type==="seed")continue;const o=t===a.key.inc,r=t===a.key.dec;if(!o&&!r)continue;const l=e.shiftKey&&a.key.shiftStep?a.key.shiftStep:a.key.step,p=ae[W.id][a.id]+l*(o?1:-1);ve(W.id,a.id,p),e.preventDefault();break}}function Te(e,t,n,a,o){const r=ae[e.id],l=t.uniforms,u=l[e.resolutionUniform];u&&s.uniform2f(u,a,o);const p=l[e.timeUniform];if(p)if(e.timeMode==="phase"){const b=e.loopDuration??8,w=n%b/b;s.uniform1f(p,w)}else if(e.timeMode==="looped"){const b=e.loopDuration??8,w=n%b;if(s.uniform1f(p,w),e.loopUniform){const g=l[e.loopUniform];g&&s.uniform1f(g,b)}}else s.uniform1f(p,n);const y={};for(const b of e.params){const w=l[b.uniform],g=r[b.id];if(b.component!==void 0){const O=y[b.uniform]??[0,0,0];O[b.component]=g,y[b.uniform]=O;continue}w&&(b.type==="int"?s.uniform1i(w,Math.round(g)):s.uniform1f(w,g))}for(const[b,w]of Object.entries(y)){const g=l[b];g&&s.uniform3f(g,w[0],w[1],w[2])}}function je(e,t,n,a){const o=t.uniforms[e.passUniform??"uPass"];o&&s.uniform1i(o,a);const r=t.uniforms[e.gridUniform??"uGridSize"];r&&s.uniform2f(r,n.size,n.size);const l=t.uniforms[e.stateUniform??"uState"];l&&s.uniform1i(l,0)}function on(e,t,n,a){const o=Zt.get(e.id);if(o){if(e.stateful){if(!Yi)return;const r=St(e);if(!r)return;const l=()=>r.textures[r.index],u=()=>r.fbos[(r.index+1)%2];s.useProgram(o.program),s.bindVertexArray(Bt),r.needsInit&&(s.bindFramebuffer(s.FRAMEBUFFER,u()),s.viewport(0,0,r.size,r.size),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,l()),Te(e,o,t,n,a),je(e,o,r,2),s.drawArrays(s.TRIANGLES,0,3),r.index=(r.index+1)%2,r.needsInit=!1),s.bindFramebuffer(s.FRAMEBUFFER,u()),s.viewport(0,0,r.size,r.size),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,l()),Te(e,o,t,n,a),je(e,o,r,0),s.drawArrays(s.TRIANGLES,0,3),r.index=(r.index+1)%2,s.bindFramebuffer(s.FRAMEBUFFER,null),s.viewport(0,0,n,a),s.clearColor(0,0,0,1),s.clear(s.COLOR_BUFFER_BIT),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,l()),Te(e,o,t,n,a),je(e,o,r,1),s.drawArrays(s.TRIANGLES,0,3);return}s.viewport(0,0,n,a),s.clearColor(0,0,0,1),s.clear(s.COLOR_BUFFER_BIT),s.useProgram(o.program),s.bindVertexArray(Bt),Te(e,o,t,n,a),s.drawArrays(s.TRIANGLES,0,3)}}function gt(e){if(Ue)return;const t=(e-bt)/1e3,{width:n,height:a}=Un(k);on(W,t,n,a),requestAnimationFrame(gt)}function ha(){return new Promise(e=>requestAnimationFrame(()=>e()))}async function va(e,t,n,a,o){if(typeof VideoEncoder>"u"||typeof VideoFrame>"u"){alert("Offline rendering requires the WebCodecs API (Chrome/Edge 94+, Safari 16.4+).");return}const r={codec:"avc1.640028",width:a,height:o,bitrate:16e6,framerate:n};if(!(await VideoEncoder.isConfigSupported(r)).supported){alert("H.264 video encoding is not supported on this device.");return}let u;try{u=await window.showSaveFilePicker({suggestedName:`${e.id}-${a}x${o}-${n}fps-${t}s.mp4`,types:[{description:"MP4 Video",accept:{"video/mp4":[".mp4"]}}]})}catch{return}const p=await u.createWritable(),y=document.getElementById("offline-btn"),b=document.getElementById("offline-progress"),w=document.getElementById("offline-bar"),g=document.getElementById("offline-status");y&&(y.disabled=!0),b==null||b.classList.remove("hidden"),g==null||g.classList.remove("hidden"),Ue=!0,e.stateful&&(St(e),he(e.id));const O=k.width,U=k.height,P=k.style.width,Z=k.style.height;k.width=a,k.height=o,k.style.width=`${a}px`,k.style.height=`${o}px`;const rn=new Ut(p),xt=new ji({target:rn,video:{codec:"avc",width:a,height:o},fastStart:!1});let Se=null;const K=new VideoEncoder({output:(T,Oe)=>xt.addVideoChunk(T,Oe??void 0),error:T=>{Se=T}});K.configure(r);const ge=Math.ceil(t*n),wt=Math.round(1e6/n),ln=n*2,sn=n*30,un=performance.now();let xe=!1;const Tt=T=>{T.preventDefault(),xe=!0};k.addEventListener("webglcontextlost",Tt);for(let T=0;T<ge&&!(Se||xe);T++){const Oe=T/n;on(e,Oe,a,o),s.finish();const Ct=new VideoFrame(k,{timestamp:T*wt,duration:wt});K.encode(Ct,{keyFrame:T%ln===0}),Ct.close(),T>0&&T%sn===0&&await K.flush();const fn=(T+1)/ge*100;if(w&&(w.style.width=`${fn}%`),T%30===0){const mn=(performance.now()-un)/1e3/(T+1)*(ge-T-1);g&&(g.textContent=`Frame ${T+1}/${ge}  —  ~${Math.ceil(mn)}s left`)}K.encodeQueueSize>10&&await new Promise(kt=>setTimeout(kt,1)),await ha()}k.removeEventListener("webglcontextlost",Tt);try{await K.flush(),K.close(),xt.finalize(),await p.close()}catch(T){console.error("Finalization failed:",T);try{await p.close()}catch{}}k.style.width=P,k.style.height=Z,k.width=O,k.height=U,e.stateful&&he(e.id),Ue=!1,bt=performance.now(),requestAnimationFrame(gt),y&&(y.disabled=!1);const We=Se?Se.message:null;g&&(xe?g.textContent="Context lost — partial video saved.":We?g.textContent=`Error: ${We}`:g.textContent="Done!"),setTimeout(()=>{b==null||b.classList.add("hidden"),g==null||g.classList.add("hidden"),w&&(w.style.width="0%")},xe||!!We?8e3:3e3)}Ne.addEventListener("click",()=>{document.body.classList.toggle("sidebar-collapsed"),tn()});yt.addEventListener("mousemove",()=>{na()});document.addEventListener("keydown",da);document.addEventListener("visibilitychange",()=>{document.hidden||(bt=performance.now())});var Ft;(Ft=document.getElementById("rec-btn"))==null||Ft.addEventListener("click",ta);var It;(It=document.getElementById("offline-btn"))==null||It.addEventListener("click",()=>{if(Ue)return;const e=document.getElementById("offline-duration"),t=document.getElementById("offline-fps"),n=document.getElementById("offline-res"),a=Math.max(1,Math.min(3600,Number((e==null?void 0:e.value)??10))),o=Number((t==null?void 0:t.value)??60),[r,l]=((n==null?void 0:n.value)??"1920x1080").split("x").map(Number);va(W,a,o,r,l)});fa();an(W.id);tn();ot(!0);requestAnimationFrame(gt);

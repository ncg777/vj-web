...
\r
// Coverage for a growing arc on the unit circle [0,1). Soft falloff uses\r
// circular distance to the painted set so the brush head and origin never\r
// leave a hard radial cut. When arcLen reaches 1 the whole loop is covered.\r
...
\r
  // Exterior gap is the open arc (len, 1). Distance to the painted arc is the\r
  // distance to the nearer endpoint along the circle (0 when inside the arc).\r
  float inGap = step(len, t);\r
  float distOut = inGap * min(t - len, 1.0 - t);\r
\r
  // Force a fully closed ring once travel finishes; avoid tiny residual gaps.\r
  float closed = step(1.0 - EPSILON, len);\r
  float openReveal = 1.0 - smoothstep(0.0, edge, distOut);\r
  return mix(openReveal, 1.0, closed);\r
}\r
\r
...
\r
    // Progressive painting: each stroke grows as a circular arc around its\r
    // loop, lingers as a fully closed ring, then washes out before repaint.\r
    // Start angle is hashed per loop; coverage uses circular distance so the\r
    // origin and the brush head never introduce a radial seam.\r
...
`;const k=document.querySelector("#gl-canvas"),ee=document.querySelector("#scene-list"),nt=document.querySelector("#control-list"),Fe=document.querySelector("#panel-actions"),se=document.querySelector("#key-help"),Ge=document.querySelector("#hud-title"),Zt=document.querySelector("#hud-desc"),_e=document.querySelector("[data-action='toggle-sidebar']"),yt=document.querySelector(".stage");if(!k||!ee||!nt||!Fe||!se||!Ge||!Zt||!_e||!yt)throw new Error("Missing required UI elements");const s=k.getContext("webgl2",{antialias:!0});if(!s)throw yt.innerHTML=`
    <div class="fallback">
      <h2>WebGL2 unavailable</h2>
      <p>Your browser or GPU does not expose WebGL2. Try a different browser.</p>
    </div>
  `,new Error("WebGL2 unavailable");const jr=s.getExtension("EXT_color_buffer_float"),Xr=!!jr;s.disable(s.DEPTH_TEST);s.disable(s.BLEND);const Bt=Wn(s),Yt=new Map,Kt={},ie={},Qt={},rt=new Map;for(const e of oe){const t=En(s,mn,e.fragment),n=new Set;n.add(e.resolutionUniform),n.add(e.timeUniform),e.loopUniform&&n.add(e.loopUniform),e.stateful&&(n.add(e.passUniform??"uPass"),n.add(e.stateUniform??"uState"),n.add(e.gridUniform??"uGridSize"));for(const l of e.params)n.add(l.uniform);const i=Un(s,t,Array.from(n));Yt.set(e.id,{program:t,uniforms:i});const o={},a={};for(const l of e.params)o[l.id]=l,a[l.id]=l.type==="seed"?Math.floor(Math.random()*1e6):l.value;Kt[e.id]=o,ie[e.id]={...a},Qt[e.id]={...a}}let O=oe[0],it={},bt=performance.now(),Ve=null,qe=null,X=null,we=[],Jt=0,Me=null,be=!1;function $r(){const e=["video/webm;codecs=vp9","video/webm;codecs=vp8","video/webm","video/mp4"];for(const t of e)if(MediaRecorder.isTypeSupported(t))return t;return""}function Zr(e){return e.startsWith("video/mp4")?"mp4":"webm"}function Yr(e){const t=Math.floor(e/1e3),n=String(Math.floor(t/60)).padStart(2,"0"),i=String(t%60).padStart(2,"0");return`${n}:${i}`}function Kr(){const e=document.getElementById("rec-badge");!e||!be||(e.textContent=`⏺ ${Yr(performance.now()-Jt)}`)}function Qr(){const e=$r();if(!e){alert("Recording is not supported in this browser.");return}const t=k.captureStream(60);we=[],X=new MediaRecorder(t,{mimeType:e,videoBitsPerSecond:16e6}),X.ondataavailable=n=>{n.data.size>0&&we.push(n.data)},X.onstop=()=>{const n=Zr(e),i=new Blob(we,{type:e}),o=URL.createObjectURL(i),a=document.createElement("a");a.href=o,a.download=`${O.id}-${Date.now()}.${n}`,a.click(),URL.revokeObjectURL(o),we=[]},X.start(500),be=!0,Jt=performance.now(),en(),Me=window.setInterval(Kr,250)}function Jr(){X&&X.state!=="inactive"&&X.stop(),be=!1,Me!==null&&(clearInterval(Me),Me=null),en()}function ei(){be?Jr():Qr()}function en(){const e=document.getElementById("rec-btn"),t=document.getElementById("rec-badge");!e||!t||(be?(e.textContent="Stop",e.classList.add("recording"),t.classList.remove("hidden")):(e.textContent="Record",e.classList.remove("recording"),t.classList.add("hidden"),t.textContent=""))}let We=!1;function ot(e){_e.classList.toggle("hidden",e)}function ti(){ot(!1),qe!==null&&window.clearTimeout(qe),qe=window.setTimeout(()=>{ot(!0)},2500)}function tn(){const e=document.body.classList.contains("sidebar-collapsed");_e.textContent=e?">>":"<<"}function ni(){var e;(e=Ge.parentElement)==null||e.classList.remove("hidden"),Ve!==null&&window.clearTimeout(Ve),Ve=window.setTimeout(()=>{var t;(t=Ge.parentElement)==null||t.classList.add("hidden")},1e4)}function Ft(e){const t=s.createTexture();if(!t)throw new Error("Failed to create state texture");return s.bindTexture(s.TEXTURE_2D,t),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MIN_FILTER,s.NEAREST),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MAG_FILTER,s.NEAREST),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_S,s.REPEAT),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_T,s.REPEAT),s.texImage2D(s.TEXTURE_2D,0,s.RGBA16F,e,e,0,s.RGBA,s.HALF_FLOAT,null),s.bindTexture(s.TEXTURE_2D,null),t}function ri(e){const t=Ft(e),n=Ft(e),i=s.createFramebuffer(),o=s.createFramebuffer();if(!i||!o)throw new Error("Failed to create framebuffer");return s.bindFramebuffer(s.FRAMEBUFFER,i),s.framebufferTexture2D(s.FRAMEBUFFER,s.COLOR_ATTACHMENT0,s.TEXTURE_2D,t,0),s.bindFramebuffer(s.FRAMEBUFFER,o),s.framebufferTexture2D(s.FRAMEBUFFER,s.COLOR_ATTACHMENT0,s.TEXTURE_2D,n,0),s.bindFramebuffer(s.FRAMEBUFFER,null),{size:e,textures:[t,n],fbos:[i,o],index:0,needsInit:!0}}function gt(e){if(!e.stateful)return null;let t=rt.get(e.id);const n=e.bufferSize??192;return(!t||t.size!==n)&&(t=ri(n),rt.set(e.id,t)),t}function he(e){const t=rt.get(e);t&&(t.needsInit=!0)}function ii(e,t){let n=t;return e.min!==void 0&&(n=Math.max(e.min,n)),e.max!==void 0&&(n=Math.min(e.max,n)),e.type==="int"&&(n=Math.round(n)),n}function nn(e,t){if(e.type==="int")return String(Math.round(t));const n=e.step??.01,i=n<1?Math.min(4,Math.max(2,Math.ceil(-Math.log10(n)))):0;return t.toFixed(i)}function oi(e){return e.length===1?e.toLowerCase():e}function ai(e){return e instanceof HTMLInputElement||e instanceof HTMLTextAreaElement||e instanceof HTMLSelectElement}function ve(e,t,n,i=!0){const o=Kt[e][t];if(!o)return;const a=ii(o,n);if(ie[e][t]=a,e===O.id&&i){const l=it[t];l!=null&&l.range&&(l.range.value=String(a)),l!=null&&l.number&&(l.number.value=nn(o,a))}}function li(e){var t;for(const n of((t=oe.find(i=>i.id===e))==null?void 0:t.params)??[])n.type==="seed"&&ve(e,n.id,Math.floor(Math.random()*1e6));he(e)}function si(e){const t=Qt[e];for(const[n,i]of Object.entries(t))ve(e,n,i,!0);he(e)}function ui(){ee.innerHTML="";for(const e of oe){const t=document.createElement("option");t.value=e.id,t.textContent=e.name,ee.appendChild(t)}ee.addEventListener("change",()=>{rn(ee.value)})}function fi(e){Fe.innerHTML="";const t=document.createElement("button");if(t.className="ghost small",t.textContent="Reset",t.addEventListener("click",()=>si(e.id)),Fe.appendChild(t),e.params.some(i=>i.type==="seed")){const i=document.createElement("button");i.className="ghost small",i.textContent="Reseed",i.addEventListener("click",()=>li(e.id)),Fe.appendChild(i)}}function ci(e){nt.innerHTML="",it={};for(const t of e.params){if(t.type==="seed")continue;const n=document.createElement("div");n.className="control";const i=document.createElement("div");i.className="control-header";const o=document.createElement("label");if(o.textContent=t.label,i.appendChild(o),t.key){const p=document.createElement("span");p.className="key-cap",p.textContent=`${t.key.inc.toUpperCase()}/${t.key.dec.toUpperCase()}`,i.appendChild(p)}const a=document.createElement("div");a.className="control-inputs";const l=document.createElement("input");l.type="range",l.min=String(t.min??0),l.max=String(t.max??1),l.step=String(t.step??(t.type==="int"?1:.01)),l.value=String(ie[e.id][t.id]),l.addEventListener("input",p=>{const y=Number(p.target.value);Number.isNaN(y)||ve(e.id,t.id,y)});const u=document.createElement("input");u.type="number",u.min=l.min,u.max=l.max,u.step=l.step,u.value=nn(t,ie[e.id][t.id]),u.addEventListener("input",p=>{const y=Number(p.target.value);Number.isNaN(y)||ve(e.id,t.id,y)}),a.appendChild(l),a.appendChild(u),n.appendChild(i),n.appendChild(a),nt.appendChild(n),it[t.id]={range:l,number:u}}}function mi(e){se.innerHTML="";for(const t of e.params){if(!t.key||t.type==="seed")continue;const n=document.createElement("div");n.className="key-row",n.textContent=`${t.key.inc.toUpperCase()}/${t.key.dec.toUpperCase()}  ${t.label}`,se.appendChild(n)}se.childElementCount||(se.textContent="No mapped keys for this scene.")}function rn(e){const t=oe.find(n=>n.id===e);t&&(O=t,t.stateful&&(gt(t),he(t.id)),Ge.textContent=t.name,Zt.textContent=t.description,ni(),fi(t),ci(t),mi(t),ee.value=t.id)}function pi(e){if(ai(e.target))return;const t=oi(e.key),n=O.params;for(const i of n){if(!i.key||i.type==="seed")continue;const o=t===i.key.inc,a=t===i.key.dec;if(!o&&!a)continue;const l=e.shiftKey&&i.key.shiftStep?i.key.shiftStep:i.key.step,p=ie[O.id][i.id]+l*(o?1:-1);ve(O.id,i.id,p),e.preventDefault();break}}function Ce(e,t,n,i,o){const a=ie[e.id],l=t.uniforms,u=l[e.resolutionUniform];u&&s.uniform2f(u,i,o);const p=l[e.timeUniform];if(p)if(e.timeMode==="phase"){const b=e.loopDuration??8,w=n%b/b;s.uniform1f(p,w)}else if(e.timeMode==="looped"){const b=e.loopDuration??8,w=n%b;if(s.uniform1f(p,w),e.loopUniform){const S=l[e.loopUniform];S&&s.uniform1f(S,b)}}else s.uniform1f(p,n);const y={};for(const b of e.params){const w=l[b.uniform],S=a[b.id];if(b.component!==void 0){const L=y[b.uniform]??[0,0,0];L[b.component]=S,y[b.uniform]=L;continue}w&&(b.type==="int"?s.uniform1i(w,Math.round(S)):s.uniform1f(w,S))}for(const[b,w]of Object.entries(y)){const S=l[b];S&&s.uniform3f(S,w[0],w[1],w[2])}}function je(e,t,n,i){const o=t.uniforms[e.passUniform??"uPass"];o&&s.uniform1i(o,i);const a=t.uniforms[e.gridUniform??"uGridSize"];a&&s.uniform2f(a,n.size,n.size);const l=t.uniforms[e.stateUniform??"uState"];l&&s.uniform1i(l,0)}function on(e,t,n,i){const o=Yt.get(e.id);if(o){if(e.stateful){if(!Xr)return;const a=gt(e);if(!a)return;const l=()=>a.textures[a.index],u=()=>a.fbos[(a.index+1)%2];s.useProgram(o.program),s.bindVertexArray(Bt),a.needsInit&&(s.bindFramebuffer(s.FRAMEBUFFER,u()),s.viewport(0,0,a.size,a.size),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,l()),Ce(e,o,t,n,i),je(e,o,a,2),s.drawArrays(s.TRIANGLES,0,3),a.index=(a.index+1)%2,a.needsInit=!1),s.bindFramebuffer(s.FRAMEBUFFER,u()),s.viewport(0,0,a.size,a.size),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,l()),Ce(e,o,t,n,i),je(e,o,a,0),s.drawArrays(s.TRIANGLES,0,3),a.index=(a.index+1)%2,s.bindFramebuffer(s.FRAMEBUFFER,null),s.viewport(0,0,n,i),s.clearColor(0,0,0,1),s.clear(s.COLOR_BUFFER_BIT),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,l()),Ce(e,o,t,n,i),je(e,o,a,1),s.drawArrays(s.TRIANGLES,0,3);return}s.viewport(0,0,n,i),s.clearColor(0,0,0,1),s.clear(s.COLOR_BUFFER_BIT),s.useProgram(o.program),s.bindVertexArray(Bt),Ce(e,o,t,n,i),s.drawArrays(s.TRIANGLES,0,3)}}function St(e){if(We)return;const t=(e-bt)/1e3,{width:n,height:i}=Gn(k);on(O,t,n,i),requestAnimationFrame(St)}function di(){return new Promise(e=>requestAnimationFrame(()=>e()))}async function hi(e,t,n,i,o){if(typeof VideoEncoder>"u"||typeof VideoFrame>"u"){alert("Offline rendering requires the WebCodecs API (Chrome/Edge 94+, Safari 16.4+).");return}const a={codec:"avc1.640028",width:i,height:o,bitrate:16e6,framerate:n};if(!(await VideoEncoder.isConfigSupported(a)).supported){alert("H.264 video encoding is not supported on this device.");return}let u;try{u=await window.showSaveFilePicker({suggestedName:`${e.id}-${i}x${o}-${n}fps-${t}s.mp4`,types:[{description:"MP4 Video",accept:{"video/mp4":[".mp4"]}}]})}catch{return}const p=await u.createWritable(),y=document.getElementById("offline-btn"),b=document.getElementById("offline-progress"),w=document.getElementById("offline-bar"),S=document.getElementById("offline-status");y&&(y.disabled=!0),b==null||b.classList.remove("hidden"),S==null||S.classList.remove("hidden"),We=!0,e.stateful&&(gt(e),he(e.id));const L=k.width,W=k.height,D=k.style.width,Y=k.style.height;k.width=i,k.height=o,k.style.width=`${i}px`,k.style.height=`${o}px`;const an=new Wt(p),xt=new qr({target:an,video:{codec:"avc",width:i,height:o},fastStart:!1});let ge=null;const K=new VideoEncoder({output:(C,Le)=>xt.addVideoChunk(C,Le??void 0),error:C=>{ge=C}});K.configure(a);const Se=Math.ceil(t*n),wt=Math.round(1e6/n),ln=n*2,sn=n*30,un=performance.now();let xe=!1;const Ct=C=>{C.preventDefault(),xe=!0};k.addEventListener("webglcontextlost",Ct);for(let C=0;C<Se&&!(ge||xe);C++){const Le=C/n;on(e,Le,i,o),s.finish();const Tt=new VideoFrame(k,{timestamp:C*wt,duration:wt});K.encode(Tt,{keyFrame:C%ln===0}),Tt.close(),C>0&&C%sn===0&&await K.flush();const fn=(C+1)/Se*100;if(w&&(w.style.width=`${fn}%`),C%30===0){const cn=(performance.now()-un)/1e3/(C+1)*(Se-C-1);S&&(S.textContent=`Frame ${C+1}/${Se}  —  ~${Math.ceil(cn)}s left`)}K.encodeQueueSize>10&&await new Promise(kt=>setTimeout(kt,1)),await di()}k.removeEventListener("webglcontextlost",Ct);try{await K.flush(),K.close(),xt.finalize(),await p.close()}catch(C){console.error("Finalization failed:",C);try{await p.close()}catch{}}k.style.width=D,k.style.height=Y,k.width=L,k.height=W,e.stateful&&he(e.id),We=!1,bt=performance.now(),requestAnimationFrame(St),y&&(y.disabled=!1);const Oe=ge?ge.message:null;S&&(xe?S.textContent="Context lost — partial video saved.":Oe?S.textContent=`Error: ${Oe}`:S.textContent="Done!"),setTimeout(()=>{b==null||b.classList.add("hidden"),S==null||S.classList.add("hidden"),w&&(w.style.width="0%")},xe||!!Oe?8e3:3e3)}_e.addEventListener("click",()=>{document.body.classList.toggle("sidebar-collapsed"),tn()});yt.addEventListener("mousemove",()=>{ti()});document.addEventListener("keydown",pi);document.addEventListener("visibilitychange",()=>{document.hidden||(bt=performance.now())});var Mt;(Mt=document.getElementById("rec-btn"))==null||Mt.addEventListener("click",ei);var It;(It=document.getElementById("offline-btn"))==null||It.addEventListener("click",()=>{if(We)return;const e=document.getElementById("offline-duration"),t=document.getElementById("offline-fps"),n=document.getElementById("offline-res"),i=Math.max(1,Math.min(3600,Number((e==null?void 0:e.value)??10))),o=Number((t==null?void 0:t.value)??60),[a,l]=((n==null?void 0:n.value)??"1920x1080").split("x").map(Number);hi(O,i,o,a,l)});ui();rn(O.id);tn();ot(!0);requestAnimationFrame(St);

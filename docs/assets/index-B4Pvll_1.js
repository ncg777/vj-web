...
`;const F=document.querySelector("#gl-canvas"),w=document.querySelector("#scene-list"),C=document.querySelector("#control-list"),T=document.querySelector("#panel-actions"),y=document.querySelector("#key-help"),z=document.querySelector("#hud-title"),A=document.querySelector("#hud-desc"),P=document.querySelector("[data-action='toggle-ui']"),I=document.querySelector("[data-action='toggle-ui-floating']"),M=document.querySelector("[data-action='fullscreen']"),N=document.querySelector(".stage");if(!F||!w||!C||!T||!y||!z||!A||!P||!I||!M||!N)throw new Error("Missing required UI elements");const t=F.getContext("webgl2",{antialias:!0});if(!t)throw N.innerHTML=`
    <div class="fallback">
      <h2>WebGL2 unavailable</h2>
      <p>Your browser or GPU does not expose WebGL2. Try a different browser.</p>
    </div>
  `,new Error("WebGL2 unavailable");const de=t.getExtension("EXT_color_buffer_float"),pe=!!de;t.disable(t.DEPTH_TEST);t.disable(t.BLEND);const G=me(t),_=new Map,q={},h={},O={},k=new Map;for(const e of b){const n=ue(t,Y,e.fragment),i=new Set;i.add(e.resolutionUniform),i.add(e.timeUniform),e.loopUniform&&i.add(e.loopUniform),e.stateful&&(i.add(e.passUniform??"uPass"),i.add(e.stateUniform??"uState"),i.add(e.gridUniform??"uGridSize"));for(const l of e.params)i.add(l.uniform);const o=ce(t,n,Array.from(i));_.set(e.id,{program:n,uniforms:o});const a={},r={};for(const l of e.params)a[l.id]=l,r[l.id]=l.type==="seed"?Math.floor(Math.random()*1e6):l.value;q[e.id]=a,h[e.id]={...r},O[e.id]={...r}}let s=b[0],E={},v=!1,H=performance.now();function W(e){const n=t.createTexture();if(!n)throw new Error("Failed to create state texture");return t.bindTexture(t.TEXTURE_2D,n),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_MIN_FILTER,t.NEAREST),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_MAG_FILTER,t.NEAREST),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_WRAP_S,t.REPEAT),t.texParameteri(t.TEXTURE_2D,t.TEXTURE_WRAP_T,t.REPEAT),t.texImage2D(t.TEXTURE_2D,0,t.RGBA16F,e,e,0,t.RGBA,t.HALF_FLOAT,null),t.bindTexture(t.TEXTURE_2D,null),n}function ve(e){const n=W(e),i=W(e),o=t.createFramebuffer(),a=t.createFramebuffer();if(!o||!a)throw new Error("Failed to create framebuffer");return t.bindFramebuffer(t.FRAMEBUFFER,o),t.framebufferTexture2D(t.FRAMEBUFFER,t.COLOR_ATTACHMENT0,t.TEXTURE_2D,n,0),t.bindFramebuffer(t.FRAMEBUFFER,a),t.framebufferTexture2D(t.FRAMEBUFFER,t.COLOR_ATTACHMENT0,t.TEXTURE_2D,i,0),t.bindFramebuffer(t.FRAMEBUFFER,null),{size:e,textures:[n,i],fbos:[o,a],index:0,needsInit:!0}}function X(e){if(!e.stateful)return null;let n=k.get(e.id);const i=e.bufferSize??192;return(!n||n.size!==i)&&(n=ve(i),k.set(e.id,n)),n}function B(e){const n=k.get(e);n&&(n.needsInit=!0)}function he(e,n){let i=n;return e.min!==void 0&&(i=Math.max(e.min,i)),e.max!==void 0&&(i=Math.min(e.max,i)),e.type==="int"&&(i=Math.round(i)),i}function j(e,n){if(e.type==="int")return String(Math.round(n));const i=e.step??.01,o=i<1?Math.min(4,Math.max(2,Math.ceil(-Math.log10(i)))):0;return n.toFixed(o)}function ye(e){return e.length===1?e.toLowerCase():e}function ge(e){return e instanceof HTMLInputElement||e instanceof HTMLTextAreaElement||e instanceof HTMLSelectElement}function x(e,n,i,o=!0){const a=q[e][n];if(!a)return;const r=he(a,i);if(h[e][n]=r,e===s.id&&o){const l=E[n];l!=null&&l.range&&(l.range.value=String(r)),l!=null&&l.number&&(l.number.value=j(a,r))}}function xe(e){var n;for(const i of((n=b.find(o=>o.id===e))==null?void 0:n.params)??[])i.type==="seed"&&x(e,i.id,Math.floor(Math.random()*1e6));B(e)}function be(e){const n=O[e];for(const[i,o]of Object.entries(n))x(e,i,o,!0);B(e)}function Se(){w.innerHTML="";for(const e of b){const n=document.createElement("button");n.className="scene-button",n.textContent=e.name,n.dataset.scene=e.id,n.addEventListener("click",()=>V(e.id)),w.appendChild(n)}}function Te(e){T.innerHTML="";const n=document.createElement("button");if(n.className="ghost small",n.textContent="Reset",n.addEventListener("click",()=>be(e.id)),T.appendChild(n),e.params.some(o=>o.type==="seed")){const o=document.createElement("button");o.className="ghost small",o.textContent="Reseed",o.addEventListener("click",()=>xe(e.id)),T.appendChild(o)}}function we(e){C.innerHTML="",E={};for(const n of e.params){if(n.type==="seed")continue;const i=document.createElement("div");i.className="control";const o=document.createElement("div");o.className="control-header";const a=document.createElement("label");if(a.textContent=n.label,o.appendChild(a),n.key){const f=document.createElement("span");f.className="key-cap",f.textContent=`${n.key.inc.toUpperCase()}/${n.key.dec.toUpperCase()}`,o.appendChild(f)}const r=document.createElement("div");r.className="control-inputs";const l=document.createElement("input");l.type="range",l.min=String(n.min??0),l.max=String(n.max??1),l.step=String(n.step??(n.type==="int"?1:.01)),l.value=String(h[e.id][n.id]),l.addEventListener("input",f=>{const d=Number(f.target.value);Number.isNaN(d)||x(e.id,n.id,d)});const u=document.createElement("input");u.type="number",u.min=l.min,u.max=l.max,u.step=l.step,u.value=j(n,h[e.id][n.id]),u.addEventListener("input",f=>{const d=Number(f.target.value);Number.isNaN(d)||x(e.id,n.id,d)}),r.appendChild(l),r.appendChild(u),i.appendChild(o),i.appendChild(r),C.appendChild(i),E[n.id]={range:l,number:u}}}function Re(e){y.innerHTML="";for(const n of e.params){if(!n.key||n.type==="seed")continue;const i=document.createElement("div");i.className="key-row",i.textContent=`${n.key.inc.toUpperCase()}/${n.key.dec.toUpperCase()}  ${n.label}`,y.appendChild(i)}y.childElementCount||(y.textContent="No mapped keys for this scene.")}function V(e){const n=b.find(i=>i.id===e);n&&(s=n,n.stateful&&(X(n),B(n.id)),z.textContent=n.name,A.textContent=n.description,Te(n),we(n),Re(n),w.querySelectorAll(".scene-button").forEach(i=>{const o=i;o.classList.toggle("active",o.dataset.scene===n.id)}))}function Ce(e){if(ge(e.target))return;const n=ye(e.key),i=s.params;for(const o of i){if(!o.key||o.type==="seed")continue;const a=n===o.key.inc,r=n===o.key.dec;if(!a&&!r)continue;const l=e.shiftKey&&o.key.shiftStep?o.key.shiftStep:o.key.step,f=h[s.id][o.id]+l*(a?1:-1);x(s.id,o.id,f),e.preventDefault();break}}function S(e,n,i,o,a){const r=h[e.id],l=n.uniforms,u=l[e.resolutionUniform];u&&t.uniform2f(u,o,a);const f=l[e.timeUniform];if(f)if(e.timeMode==="phase"){const c=e.loopDuration??8,m=i%c/c;t.uniform1f(f,m)}else if(e.timeMode==="looped"){const c=e.loopDuration??8,m=i%c;if(t.uniform1f(f,m),e.loopUniform){const p=l[e.loopUniform];p&&t.uniform1f(p,c)}}else t.uniform1f(f,i);const d={};for(const c of e.params){const m=l[c.uniform],p=r[c.id];if(c.component!==void 0){const D=d[c.uniform]??[0,0,0];D[c.component]=p,d[c.uniform]=D;continue}m&&(c.type==="int"?t.uniform1i(m,Math.round(p)):t.uniform1f(m,p))}for(const[c,m]of Object.entries(d)){const p=l[c];p&&t.uniform3f(p,m[0],m[1],m[2])}}function R(e,n,i,o){const a=n.uniforms[e.passUniform??"uPass"];a&&t.uniform1i(a,o);const r=n.uniforms[e.gridUniform??"uGridSize"];r&&t.uniform2f(r,i.size,i.size);const l=n.uniforms[e.stateUniform??"uState"];l&&t.uniform1i(l,0)}function g(e){const n=(e-H)/1e3,{width:i,height:o}=fe(F),a=_.get(s.id);if(a){if(s.stateful){if(!pe){A.textContent="Stateful scenes require float render targets (EXT_color_buffer_float).",requestAnimationFrame(g);return}const r=X(s);if(!r){requestAnimationFrame(g);return}const l=()=>r.textures[r.index],u=()=>r.fbos[(r.index+1)%2];t.useProgram(a.program),t.bindVertexArray(G),r.needsInit&&(t.bindFramebuffer(t.FRAMEBUFFER,u()),t.viewport(0,0,r.size,r.size),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,l()),S(s,a,n,i,o),R(s,a,r,2),t.drawArrays(t.TRIANGLES,0,3),r.index=(r.index+1)%2,r.needsInit=!1),t.bindFramebuffer(t.FRAMEBUFFER,u()),t.viewport(0,0,r.size,r.size),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,l()),S(s,a,n,i,o),R(s,a,r,0),t.drawArrays(t.TRIANGLES,0,3),r.index=(r.index+1)%2,t.bindFramebuffer(t.FRAMEBUFFER,null),t.viewport(0,0,i,o),t.clearColor(0,0,0,1),t.clear(t.COLOR_BUFFER_BIT),t.activeTexture(t.TEXTURE0),t.bindTexture(t.TEXTURE_2D,l()),S(s,a,n,i,o),R(s,a,r,1),t.drawArrays(t.TRIANGLES,0,3),requestAnimationFrame(g);return}t.viewport(0,0,i,o),t.clearColor(0,0,0,1),t.clear(t.COLOR_BUFFER_BIT),t.useProgram(a.program),t.bindVertexArray(G),S(s,a,n,i,o),t.drawArrays(t.TRIANGLES,0,3),requestAnimationFrame(g)}}function K(e){v=e,document.body.classList.toggle("ui-hidden",v),P.textContent=v?"Show UI":"Hide UI",I.textContent=v?"Show UI":"Hide UI"}P.addEventListener("click",()=>K(!v));I.addEventListener("click",()=>K(!v));M.addEventListener("click",async()=>{document.fullscreenElement?await document.exitFullscreen():await N.requestFullscreen()});document.addEventListener("keydown",Ce);document.addEventListener("fullscreenchange",()=>{M.textContent=document.fullscreenElement?"Exit Fullscreen":"Fullscreen"});document.addEventListener("visibilitychange",()=>{document.hidden||(H=performance.now())});Se();V(s.id);requestAnimationFrame(g);

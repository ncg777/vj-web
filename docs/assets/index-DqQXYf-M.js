...
void main() {
  ...
  // ------------------------------------------------------------------
  // Large-scale structure fields, computed ONCE (not per layer): a slow
  // "zone" field that splits the nebula into distinct chromatic regions,
  // and a "void" field that carves dark pockets through the gas so the
  // cloud mass never reads as one homogeneous wash.
  float tcloud = t * uCloudSpeed;
  float zone = fbm4(uv * 0.55 + seed * 2.1 + tcloud * vec2(0.021, -0.013));
  float voids = fbm4(uv * 0.8 - seed * 4.3 + tcloud * vec2(-0.017, 0.024));
  ...
  for (int layer = 0; layer < LAYERS; layer++) {
    ...
    // Fine turbulent detail plus a ridged transform of it: the ridges
    // give wispy filaments and the voids carve dark pockets, so the gas
    // breaks into structured shapes with no extra noise fetches.
    float fine = fbm4(warped * 3.2 - tc * 0.28);
    float ridged = 1.0 - abs(2.0 * fine - 1.0);
    ridged *= ridged;
    float turb = clamp(uTurbulence, 0.0, 1.0);
    float blended = mix(f, fine, turb * 0.35);
    blended = mix(blended, ridged * 0.5 + blended * 0.5, turb * 0.6);

    float density = smoothstep(uCloudDensity - 0.22, uCloudDensity + 0.25, blended);
    // Voids eat into the gas; ridges push filaments back up through them.
    density *= mix(0.25, 1.0, voidMask) + ridged * 0.35;
    density = pow(clamp(density, 0.0, 1.0), mix(1.0, 3.0, clamp(uCloudContrast, 0.0, 1.0)));

    // Distinct chromatic regions: the slow zone field picks which of
    // three well-separated palette bands dominates locally, instead of
    // averaging every sample into a uniform wash.
    float chroma = 0.25 + 0.75 * q.y;
    vec3 gasA = palette(blended * 1.3 + radius * 0.16 + fl * 0.29);
    vec3 gasB = palette(length(q) * 1.05 + 0.34 + fl * 0.17 + zone * 0.5);
    vec3 gasC = palette(f * 0.8 + 0.66 - fl * 0.12 + chroma * 0.3 - zone * 0.45);
    float zoneSel = smoothstep(0.32, 0.58, zone + (q.x - 0.5) * 0.3);
    vec3 layerCol = mix(gasA, gasB, zoneSel);
    layerCol = mix(layerCol, gasC, smoothstep(0.45, 0.72, ridged * 0.6 + chroma * 0.5 - zone * 0.3));
    layerCol *= 0.8 + 0.45 * density;
  ...
  for (int i = 0; i < MAX_BOLTS; i++) {
    ...
    // Varying strike count: the first slots always participate so the
    // frame is never empty, while the remaining slots gate on and off
    ...
    float energy = flash * uBoltIntensity * (0.75 + 0.5 * hash1(strikeSeed + 12.3));

    float baseAng = hash1(strikeSeed + 3.1) * TAU;
    float reach = uBoltReach * (0.45 + 0.75 * hash1(strikeSeed + 4.2));

    // Strike origins: the first two slots stay anchored to the plasma
    // core; the rest scatter through the visible frame. Origins are
    // drawn from a bounded disc (never past the frame edge) and the
    ...
    if (fi >= ANCHORED_SLOTS) {
      ...
      // Aim mostly back across the frame so the bolt reach stays visible.
      baseAng = originAng + 3.14159 + (hash1(strikeSeed + 4.8) - 0.5) * (1.6 + uStrikeChaos);
      // Discrete re-strike jumps: the channel snaps to nearby positions
      // a few times within a strike (like a re-striking leader) rather
      // than gliding smoothly, which reads as electric instead of drifty.
      float restep = floor(strikePhase * (2.0 + 3.0 * clamp(uStrikeChaos, 0.0, 2.0)));
      strikeOrigin += (vec2(
        hash1(strikeSeed + restep * 7.3 + 21.4),
        hash1(strikeSeed + restep * 9.1 + 34.8)
      ) - 0.5) * 0.1;
    }

    // Rotate into the bolt frame: channel along +x from the strike
    // origin (the core for the anchored slots).
    ...
    float glow = gw / (d + gw);
    float hueOffset = hash1(strikeSeed + 13.7) * 0.35;
...
`;const k=document.querySelector("#gl-canvas"),ee=document.querySelector("#scene-list"),nt=document.querySelector("#control-list"),Me=document.querySelector("#panel-actions"),se=document.querySelector("#key-help"),Ue=document.querySelector("#hud-title"),Zt=document.querySelector("#hud-desc"),Ne=document.querySelector("[data-action='toggle-sidebar']"),yt=document.querySelector(".stage");if(!k||!ee||!nt||!Me||!se||!Ue||!Zt||!Ne||!yt)throw new Error("Missing required UI elements");const s=k.getContext("webgl2",{antialias:!0});if(!s)throw yt.innerHTML=`
    <div class="fallback">
      <h2>WebGL2 unavailable</h2>
      <p>Your browser or GPU does not expose WebGL2. Try a different browser.</p>
    </div>
  `,new Error("WebGL2 unavailable");const Xa=s.getExtension("EXT_color_buffer_float"),$a=!!Xa;s.disable(s.DEPTH_TEST);s.disable(s.BLEND);const Bt=Wn(s),Yt=new Map,Kt={},oe={},Qt={},at=new Map;for(const e of ie){const t=Gn(s,mn,e.fragment),n=new Set;n.add(e.resolutionUniform),n.add(e.timeUniform),e.loopUniform&&n.add(e.loopUniform),e.stateful&&(n.add(e.passUniform??"uPass"),n.add(e.stateUniform??"uState"),n.add(e.gridUniform??"uGridSize"));for(const r of e.params)n.add(r.uniform);const o=Un(s,t,Array.from(n));Yt.set(e.id,{program:t,uniforms:o});const i={},l={};for(const r of e.params)i[r.id]=r,l[r.id]=r.type==="seed"?Math.floor(Math.random()*1e6):r.value;Kt[e.id]=i,oe[e.id]={...l},Qt[e.id]={...l}}let _=ie[0],ot={},bt=performance.now(),qe=null,Ve=null,X=null,we=[],Jt=0,Fe=null,be=!1;function Za(){const e=["video/webm;codecs=vp9","video/webm;codecs=vp8","video/webm","video/mp4"];for(const t of e)if(MediaRecorder.isTypeSupported(t))return t;return""}function Ya(e){return e.startsWith("video/mp4")?"mp4":"webm"}function Ka(e){const t=Math.floor(e/1e3),n=String(Math.floor(t/60)).padStart(2,"0"),o=String(t%60).padStart(2,"0");return`${n}:${o}`}function Qa(){const e=document.getElementById("rec-badge");!e||!be||(e.textContent=`⏺ ${Ka(performance.now()-Jt)}`)}function Ja(){const e=Za();if(!e){alert("Recording is not supported in this browser.");return}const t=k.captureStream(60);we=[],X=new MediaRecorder(t,{mimeType:e,videoBitsPerSecond:16e6}),X.ondataavailable=n=>{n.data.size>0&&we.push(n.data)},X.onstop=()=>{const n=Ya(e),o=new Blob(we,{type:e}),i=URL.createObjectURL(o),l=document.createElement("a");l.href=i,l.download=`${_.id}-${Date.now()}.${n}`,l.click(),URL.revokeObjectURL(i),we=[]},X.start(500),be=!0,Jt=performance.now(),en(),Fe=window.setInterval(Qa,250)}function eo(){X&&X.state!=="inactive"&&X.stop(),be=!1,Fe!==null&&(clearInterval(Fe),Fe=null),en()}function to(){be?eo():Ja()}function en(){const e=document.getElementById("rec-btn"),t=document.getElementById("rec-badge");!e||!t||(be?(e.textContent="Stop",e.classList.add("recording"),t.classList.remove("hidden")):(e.textContent="Record",e.classList.remove("recording"),t.classList.add("hidden"),t.textContent=""))}let Oe=!1;function it(e){Ne.classList.toggle("hidden",e)}function no(){it(!1),Ve!==null&&window.clearTimeout(Ve),Ve=window.setTimeout(()=>{it(!0)},2500)}function tn(){const e=document.body.classList.contains("sidebar-collapsed");Ne.textContent=e?">>":"<<"}function ao(){var e;(e=Ue.parentElement)==null||e.classList.remove("hidden"),qe!==null&&window.clearTimeout(qe),qe=window.setTimeout(()=>{var t;(t=Ue.parentElement)==null||t.classList.add("hidden")},1e4)}function Mt(e){const t=s.createTexture();if(!t)throw new Error("Failed to create state texture");return s.bindTexture(s.TEXTURE_2D,t),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MIN_FILTER,s.NEAREST),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MAG_FILTER,s.NEAREST),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_S,s.REPEAT),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_T,s.REPEAT),s.texImage2D(s.TEXTURE_2D,0,s.RGBA16F,e,e,0,s.RGBA,s.HALF_FLOAT,null),s.bindTexture(s.TEXTURE_2D,null),t}function oo(e){const t=Mt(e),n=Mt(e),o=s.createFramebuffer(),i=s.createFramebuffer();if(!o||!i)throw new Error("Failed to create framebuffer");return s.bindFramebuffer(s.FRAMEBUFFER,o),s.framebufferTexture2D(s.FRAMEBUFFER,s.COLOR_ATTACHMENT0,s.TEXTURE_2D,t,0),s.bindFramebuffer(s.FRAMEBUFFER,i),s.framebufferTexture2D(s.FRAMEBUFFER,s.COLOR_ATTACHMENT0,s.TEXTURE_2D,n,0),s.bindFramebuffer(s.FRAMEBUFFER,null),{size:e,textures:[t,n],fbos:[o,i],index:0,needsInit:!0}}function gt(e){if(!e.stateful)return null;let t=at.get(e.id);const n=e.bufferSize??192;return(!t||t.size!==n)&&(t=oo(n),at.set(e.id,t)),t}function he(e){const t=at.get(e);t&&(t.needsInit=!0)}function io(e,t){let n=t;return e.min!==void 0&&(n=Math.max(e.min,n)),e.max!==void 0&&(n=Math.min(e.max,n)),e.type==="int"&&(n=Math.round(n)),n}function nn(e,t){if(e.type==="int")return String(Math.round(t));const n=e.step??.01,o=n<1?Math.min(4,Math.max(2,Math.ceil(-Math.log10(n)))):0;return t.toFixed(o)}function lo(e){return e.length===1?e.toLowerCase():e}function ro(e){return e instanceof HTMLInputElement||e instanceof HTMLTextAreaElement||e instanceof HTMLSelectElement}function ve(e,t,n,o=!0){const i=Kt[e][t];if(!i)return;const l=io(i,n);if(oe[e][t]=l,e===_.id&&o){const r=ot[t];r!=null&&r.range&&(r.range.value=String(l)),r!=null&&r.number&&(r.number.value=nn(i,l))}}function so(e){var t;for(const n of((t=ie.find(o=>o.id===e))==null?void 0:t.params)??[])n.type==="seed"&&ve(e,n.id,Math.floor(Math.random()*1e6));he(e)}function uo(e){const t=Qt[e];for(const[n,o]of Object.entries(t))ve(e,n,o,!0);he(e)}function fo(){ee.innerHTML="";for(const e of ie){const t=document.createElement("option");t.value=e.id,t.textContent=e.name,ee.appendChild(t)}ee.addEventListener("change",()=>{an(ee.value)})}function co(e){Me.innerHTML="";const t=document.createElement("button");if(t.className="ghost small",t.textContent="Reset",t.addEventListener("click",()=>uo(e.id)),Me.appendChild(t),e.params.some(o=>o.type==="seed")){const o=document.createElement("button");o.className="ghost small",o.textContent="Reseed",o.addEventListener("click",()=>so(e.id)),Me.appendChild(o)}}function mo(e){nt.innerHTML="",ot={};for(const t of e.params){if(t.type==="seed")continue;const n=document.createElement("div");n.className="control";const o=document.createElement("div");o.className="control-header";const i=document.createElement("label");if(i.textContent=t.label,o.appendChild(i),t.key){const p=document.createElement("span");p.className="key-cap",p.textContent=`${t.key.inc.toUpperCase()}/${t.key.dec.toUpperCase()}`,o.appendChild(p)}const l=document.createElement("div");l.className="control-inputs";const r=document.createElement("input");r.type="range",r.min=String(t.min??0),r.max=String(t.max??1),r.step=String(t.step??(t.type==="int"?1:.01)),r.value=String(oe[e.id][t.id]),r.addEventListener("input",p=>{const y=Number(p.target.value);Number.isNaN(y)||ve(e.id,t.id,y)});const u=document.createElement("input");u.type="number",u.min=r.min,u.max=r.max,u.step=r.step,u.value=nn(t,oe[e.id][t.id]),u.addEventListener("input",p=>{const y=Number(p.target.value);Number.isNaN(y)||ve(e.id,t.id,y)}),l.appendChild(r),l.appendChild(u),n.appendChild(o),n.appendChild(l),nt.appendChild(n),ot[t.id]={range:r,number:u}}}function po(e){se.innerHTML="";for(const t of e.params){if(!t.key||t.type==="seed")continue;const n=document.createElement("div");n.className="key-row",n.textContent=`${t.key.inc.toUpperCase()}/${t.key.dec.toUpperCase()}  ${t.label}`,se.appendChild(n)}se.childElementCount||(se.textContent="No mapped keys for this scene.")}function an(e){const t=ie.find(n=>n.id===e);t&&(_=t,t.stateful&&(gt(t),he(t.id)),Ue.textContent=t.name,Zt.textContent=t.description,ao(),co(t),mo(t),po(t),ee.value=t.id)}function ho(e){if(ro(e.target))return;const t=lo(e.key),n=_.params;for(const o of n){if(!o.key||o.type==="seed")continue;const i=t===o.key.inc,l=t===o.key.dec;if(!i&&!l)continue;const r=e.shiftKey&&o.key.shiftStep?o.key.shiftStep:o.key.step,p=oe[_.id][o.id]+r*(i?1:-1);ve(_.id,o.id,p),e.preventDefault();break}}function Ce(e,t,n,o,i){const l=oe[e.id],r=t.uniforms,u=r[e.resolutionUniform];u&&s.uniform2f(u,o,i);const p=r[e.timeUniform];if(p)if(e.timeMode==="phase"){const b=e.loopDuration??8,w=n%b/b;s.uniform1f(p,w)}else if(e.timeMode==="looped"){const b=e.loopDuration??8,w=n%b;if(s.uniform1f(p,w),e.loopUniform){const S=r[e.loopUniform];S&&s.uniform1f(S,b)}}else s.uniform1f(p,n);const y={};for(const b of e.params){const w=r[b.uniform],S=l[b.id];if(b.component!==void 0){const L=y[b.uniform]??[0,0,0];L[b.component]=S,y[b.uniform]=L;continue}w&&(b.type==="int"?s.uniform1i(w,Math.round(S)):s.uniform1f(w,S))}for(const[b,w]of Object.entries(y)){const S=r[b];S&&s.uniform3f(S,w[0],w[1],w[2])}}function je(e,t,n,o){const i=t.uniforms[e.passUniform??"uPass"];i&&s.uniform1i(i,o);const l=t.uniforms[e.gridUniform??"uGridSize"];l&&s.uniform2f(l,n.size,n.size);const r=t.uniforms[e.stateUniform??"uState"];r&&s.uniform1i(r,0)}function on(e,t,n,o){const i=Yt.get(e.id);if(i){if(e.stateful){if(!$a)return;const l=gt(e);if(!l)return;const r=()=>l.textures[l.index],u=()=>l.fbos[(l.index+1)%2];s.useProgram(i.program),s.bindVertexArray(Bt),l.needsInit&&(s.bindFramebuffer(s.FRAMEBUFFER,u()),s.viewport(0,0,l.size,l.size),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,r()),Ce(e,i,t,n,o),je(e,i,l,2),s.drawArrays(s.TRIANGLES,0,3),l.index=(l.index+1)%2,l.needsInit=!1),s.bindFramebuffer(s.FRAMEBUFFER,u()),s.viewport(0,0,l.size,l.size),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,r()),Ce(e,i,t,n,o),je(e,i,l,0),s.drawArrays(s.TRIANGLES,0,3),l.index=(l.index+1)%2,s.bindFramebuffer(s.FRAMEBUFFER,null),s.viewport(0,0,n,o),s.clearColor(0,0,0,1),s.clear(s.COLOR_BUFFER_BIT),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,r()),Ce(e,i,t,n,o),je(e,i,l,1),s.drawArrays(s.TRIANGLES,0,3);return}s.viewport(0,0,n,o),s.clearColor(0,0,0,1),s.clear(s.COLOR_BUFFER_BIT),s.useProgram(i.program),s.bindVertexArray(Bt),Ce(e,i,t,n,o),s.drawArrays(s.TRIANGLES,0,3)}}function St(e){if(Oe)return;const t=(e-bt)/1e3,{width:n,height:o}=On(k);on(_,t,n,o),requestAnimationFrame(St)}function vo(){return new Promise(e=>requestAnimationFrame(()=>e()))}async function yo(e,t,n,o,i){if(typeof VideoEncoder>"u"||typeof VideoFrame>"u"){alert("Offline rendering requires the WebCodecs API (Chrome/Edge 94+, Safari 16.4+).");return}const l={codec:"avc1.640028",width:o,height:i,bitrate:16e6,framerate:n};if(!(await VideoEncoder.isConfigSupported(l)).supported){alert("H.264 video encoding is not supported on this device.");return}let u;try{u=await window.showSaveFilePicker({suggestedName:`${e.id}-${o}x${i}-${n}fps-${t}s.mp4`,types:[{description:"MP4 Video",accept:{"video/mp4":[".mp4"]}}]})}catch{return}const p=await u.createWritable(),y=document.getElementById("offline-btn"),b=document.getElementById("offline-progress"),w=document.getElementById("offline-bar"),S=document.getElementById("offline-status");y&&(y.disabled=!0),b==null||b.classList.remove("hidden"),S==null||S.classList.remove("hidden"),Oe=!0,e.stateful&&(gt(e),he(e.id));const L=k.width,O=k.height,I=k.style.width,Y=k.style.height;k.width=o,k.height=i,k.style.width=`${o}px`,k.style.height=`${i}px`;const ln=new Ot(p),xt=new ja({target:ln,video:{codec:"avc",width:o,height:i},fastStart:!1});let ge=null;const K=new VideoEncoder({output:(C,Le)=>xt.addVideoChunk(C,Le??void 0),error:C=>{ge=C}});K.configure(l);const Se=Math.ceil(t*n),wt=Math.round(1e6/n),rn=n*2,sn=n*30,un=performance.now();let xe=!1;const Ct=C=>{C.preventDefault(),xe=!0};k.addEventListener("webglcontextlost",Ct);for(let C=0;C<Se&&!(ge||xe);C++){const Le=C/n;on(e,Le,o,i),s.finish();const Tt=new VideoFrame(k,{timestamp:C*wt,duration:wt});K.encode(Tt,{keyFrame:C%rn===0}),Tt.close(),C>0&&C%sn===0&&await K.flush();const fn=(C+1)/Se*100;if(w&&(w.style.width=`${fn}%`),C%30===0){const cn=(performance.now()-un)/1e3/(C+1)*(Se-C-1);S&&(S.textContent=`Frame ${C+1}/${Se}  —  ~${Math.ceil(cn)}s left`)}K.encodeQueueSize>10&&await new Promise(kt=>setTimeout(kt,1)),await vo()}k.removeEventListener("webglcontextlost",Ct);try{await K.flush(),K.close(),xt.finalize(),await p.close()}catch(C){console.error("Finalization failed:",C);try{await p.close()}catch{}}k.style.width=I,k.style.height=Y,k.width=L,k.height=O,e.stateful&&he(e.id),Oe=!1,bt=performance.now(),requestAnimationFrame(St),y&&(y.disabled=!1);const _e=ge?ge.message:null;S&&(xe?S.textContent="Context lost — partial video saved.":_e?S.textContent=`Error: ${_e}`:S.textContent="Done!"),setTimeout(()=>{b==null||b.classList.add("hidden"),S==null||S.classList.add("hidden"),w&&(w.style.width="0%")},xe||!!_e?8e3:3e3)}Ne.addEventListener("click",()=>{document.body.classList.toggle("sidebar-collapsed"),tn()});yt.addEventListener("mousemove",()=>{no()});document.addEventListener("keydown",ho);document.addEventListener("visibilitychange",()=>{document.hidden||(bt=performance.now())});var Ft;(Ft=document.getElementById("rec-btn"))==null||Ft.addEventListener("click",to);var Dt;(Dt=document.getElementById("offline-btn"))==null||Dt.addEventListener("click",()=>{if(Oe)return;const e=document.getElementById("offline-duration"),t=document.getElementById("offline-fps"),n=document.getElementById("offline-res"),o=Math.max(1,Math.min(3600,Number((e==null?void 0:e.value)??10))),i=Number((t==null?void 0:t.value)??60),[l,r]=((n==null?void 0:n.value)??"1920x1080").split("x").map(Number);yo(_,o,i,l,r)});fo();an(_.id);tn();it(!0);requestAnimationFrame(St);

...
`,vt=[{id:"fluid-vortex-mandala",name:"Fluid Vortex Mandala",description:"Rainbow dye swept through turbulent vortices and nested mirrored petals.",symmetry:8,detail:3,twist:.7,scale:1.4,turbulence:1.8},{id:"fluid-prismatic-ribbons",name:"Fluid Prismatic Ribbons",description:"Silky streams of prismatic ink stretched into recursive kaleidoscopic ribbons.",symmetry:6,detail:2,twist:-.5,scale:1.9,turbulence:1.2},{id:"fluid-cellular-jewel",name:"Fluid Cellular Jewel",description:"Colorful cellular eddies mixing inside an intricate fractal jewel.",symmetry:12,detail:4,twist:.2,scale:2.3,turbulence:2.5},{id:"fluid-spiral-bloom",name:"Fluid Spiral Bloom",description:"Swirling liquid pigments blooming along twisting fractal spiral arms.",symmetry:5,detail:3,twist:2.4,scale:1.2,turbulence:2}];function E(e,n,t,r,o,i,l,u="float"){return{id:e,label:n,uniform:t,type:u,value:r,min:o,max:i,step:l}}const yt=vt.map((e,n)=>({id:e.id,name:e.name,description:e.description,fragment:ht.replace("uniform int uStyle;",`const int uStyle = ${n};`),resolutionUniform:"uResolution",timeUniform:"uTime",timeMode:"seconds",stateful:!0,bufferSize:384,params:[E("speed","Flow Speed","uSpeed",1,0,3,.05),E("turbulence","Turbulence","uTurbulence",e.turbulence,0,5,.05),E("symmetry","Kaleidoscope Sectors","uSymmetry",e.symmetry,2,24,1,"int"),E("detail","Fractal Folds","uDetail",e.detail,0,5,1,"int"),E("scale","Pattern Scale","uScale",e.scale,.3,4,.05),E("twist","Spiral Twist","uTwist",e.twist,-4,4,.05),E("persistence","Dye Persistence","uPersistence",.992,.95,.999,.001),E("injection","Dye Injection","uInjection",1,0,3,.05),E("hue","Hue Shift","uHue",0,0,1,.01),E("glow","Liquid Vein Glow","uGlow",.7,0,3,.05),{id:"seed",label:"Seed",uniform:"uSeed",type:"seed",value:0}]})),gt=`#version 300 es
precision highp float;
precision highp int;
...
`,bt=[{id:"smoke-rainbow-plumes",name:"Smoke Rainbow Plumes",description:"Hot pink, cyan and golden smoke billowing upward from rainbow vents.",turbulence:1.2,buoyancy:1,lifetime:.993},{id:"gas-chromatic-collision",name:"Gas Chromatic Collision",description:"Opposing multicolored gas jets collide and roll into turbulent mixing clouds.",turbulence:1.5,buoyancy:.15,lifetime:.992},{id:"smoke-nebula-vortex",name:"Smoke Nebula Vortex",description:"Luminous violet, turquoise and orange gases spiral into a cosmic vortex.",turbulence:1,buoyancy:0,lifetime:.996},{id:"gas-neon-fog",name:"Gas Neon Fog",description:"Low banks of electric rainbow fog tumble in rolling crosswinds.",turbulence:1.8,buoyancy:.25,lifetime:.995},{id:"smoke-orbital-trails",name:"Smoke Orbital Trails",description:"Orbiting emitters leave curling ribbons of vivid smoke that disperse into wisps.",turbulence:1.3,buoyancy:.1,lifetime:.991}];function G(e,n,t,r,o,i,l){return{id:e,label:n,uniform:t,type:"float",value:r,min:o,max:i,step:l}}const St=bt.map((e,n)=>({id:e.id,name:e.name,description:e.description,fragment:gt.replace("uniform int uStyle;",`const int uStyle = ${n};`),resolutionUniform:"uResolution",timeUniform:"uTime",timeMode:"seconds",stateful:!0,bufferSize:384,params:[G("speed","Flow Speed","uSpeed",1,0,3,.05),G("turbulence","Turbulence","uTurbulence",e.turbulence,0,4,.05),G("eddyScale","Eddy Scale","uEddyScale",1,.4,3,.05),G("buoyancy","Buoyancy","uBuoyancy",e.buoyancy,-1,2,.05),G("wind","Crosswind","uWind",0,-1,1,.05),G("emission","Gas Emission","uEmission",1,0,3,.05),G("width","Emitter Width","uWidth",1,.4,2.5,.05),G("persistence","Smoke Persistence","uPersistence",e.lifetime,.97,.999,.001),G("density","Optical Density","uDensity",1.3,.2,3,.05),G("hue","Hue Shift","uHue",0,0,1,.01),G("glow","Scattering Glow","uGlow",.8,0,2,.05),{id:"seed",label:"Seed",uniform:"uSeed",type:"seed",value:0}]})),xt=`#version 300 es
precision highp float;
precision highp int;
...
\r
float hash(vec2 point) {\r
  vec3 hashed = fract(vec3(point.xyx) * 0.1031);\r
  hashed += dot(hashed, hashed.yzx + 33.33);\r
  return fract((hashed.x + hashed.y) * hashed.z);\r
}\r
\r
float noise(vec2 point) {\r
  vec2 cell = floor(point);\r
  vec2 blend = fract(point);\r
  blend = blend * blend * (3.0 - 2.0 * blend);\r
  return mix(mix(hash(cell), hash(cell + vec2(1.0, 0.0)), blend.x),\r
             mix(hash(cell + vec2(0.0, 1.0)), hash(cell + 1.0), blend.x), blend.y);\r
}\r
\r
float fbm(vec2 point, int octaves) {\r
  float value = 0.0;\r
  float amplitude = 0.5;\r
  float total = 0.0;\r
  for (int octave = 0; octave < 8; octave++) {\r
    if (octave >= octaves) break;\r
    value += noise(point) * amplitude;\r
    total += amplitude;\r
    point = mat2(1.6, 1.2, -1.2, 1.6) * point + vec2(1.7, 9.2);\r
    amplitude *= 0.5;\r
  }\r
  return value / max(total, 0.001);\r
}\r
\r
float segmentDistance(vec2 point, vec2 start, vec2 end) {\r
  vec2 direction = end - start;\r
  float along = clamp(dot(point - start, direction) / max(dot(direction, direction), 1e-10), 0.0, 1.0);\r
  return length(point - start - direction * along);\r
}\r
\r
float jaggedNoise(float position, float seed) {\r
  float cell = floor(position);\r
  return mix(hash(vec2(cell, seed)), hash(vec2(cell + 1.0, seed)), fract(position)) * 2.0 - 1.0;\r
}\r
\r
vec2 channelPoint(vec2 start, vec2 end, float progress, float seed) {\r
  vec2 direction = end - start;\r
  vec2 perpendicular = vec2(-direction.y, direction.x);\r
  float frequency = uBoltNoiseScale;\r
  float displacement = jaggedNoise(progress * 3.0, seed) * 0.55\r
                     + jaggedNoise(progress * frequency, seed + 13.0) * 0.30\r
                     + jaggedNoise(progress * frequency * 2.7, seed + 29.0) * 0.15;\r
  float envelope = pow(max(sin(progress * 3.14159265), 0.0), 0.45);\r
  return mix(start, end, progress) + perpendicular * displacement * uBoltWiggle * 4.0 * envelope;\r
}\r
\r
vec3 channelLight(float distanceToChannel, float width, float energy, float pixelSize) {\r
  float core = (1.0 - smoothstep(width * 0.35, width + pixelSize, distanceToChannel))\r
             * width / max(width, pixelSize * 0.65);\r
  float corona = exp(-distanceToChannel / max(width * 3.0, pixelSize)) * 0.32;\r
  float bloom = exp(-distanceToChannel / max(width * 18.0, pixelSize * 2.0)) * 0.035;\r
  vec3 hotColor = mix(uLightningColor, vec3(max(max(uLightningColor.r, uLightningColor.g), uLightningColor.b)), 0.88);\r
  return energy * (hotColor * core * 4.5 + uLightningColor * (corona + bloom));\r
}\r
\r
vec3 discharge(vec2 point, vec2 start, vec2 end, float seed, float growth,\r
               float energy, float branchEnergy, float pixelSize, out float channelDistance) {\r
  channelDistance = 100.0;\r
  float nearestProgress = 0.0;\r
  vec2 previous = start;\r
  for (int segment = 1; segment <= 8; segment++) {\r
    float progress = min(float(segment) / 8.0, growth);\r
    vec2 current = channelPoint(start, end, progress, seed);\r
    channelDistance = min(channelDistance, segmentDistance(point, previous, current));\r
    previous = current;\r
    if (float(segment) / 8.0 >= growth) break;\r
  }\r
\r
  vec3 light = vec3(0.0);\r
  vec2 direction = end - start;\r
  vec2 perpendicular = vec2(-direction.y, direction.x);\r
  float haloRadius = max(uBoltWidth * 120.0, pixelSize * 12.0);\r
  if (segmentDistance(point, start, end) < length(direction) * uBoltWiggle * 4.0 + haloRadius) {\r
    float coreDistance = 100.0;\r
    previous = start;\r
    for (int segment = 1; segment <= 64; segment++) {\r
      float progress = min(float(segment) / 64.0, growth);\r
      vec2 current = channelPoint(start, end, progress, seed);\r
      float distanceToSegment = segmentDistance(point, previous, current);\r
      if (distanceToSegment < coreDistance) {\r
        coreDistance = distanceToSegment;\r
        nearestProgress = progress;\r
      }\r
      previous = current;\r
      if (float(segment) / 64.0 >= growth) break;\r
    }\r
    float width = uBoltWidth * mix(1.15, 0.55, nearestProgress);\r
    light = channelLight(coreDistance, width, energy, pixelSize);\r
  }\r
\r
  for (int branch = 0; branch < 7; branch++) {\r
    float branchIndex = float(branch);\r
    float branchSeed = seed + branchIndex * 17.31 + 71.0;\r
    if (hash(vec2(branchSeed, 4.0)) >= uBoltBranching) continue;\r
    float attachment = 0.12 + branchIndex * 0.105 + hash(vec2(branchSeed, 5.0)) * 0.06;\r
    attachment = floor(attachment * 64.0) / 64.0;\r
    if (growth <= attachment) continue;\r
    float side = hash(vec2(branchSeed, 6.0)) < 0.5 ? -1.0 : 1.0;\r
    float extent = mix(0.16, 0.44, hash(vec2(branchSeed, 7.0))) * (1.0 - attachment * 0.5);\r
    vec2 branchStart = channelPoint(start, end, attachment, seed);\r
    vec2 branchEnd = branchStart + (direction + perpendicular * side * mix(0.5, 1.5, hash(vec2(branchSeed, 8.0)))) * extent;\r
    float branchGrowth = clamp((growth - attachment) / extent, 0.0, 1.0);\r
    vec2 forkStart = channelPoint(branchStart, branchEnd, 0.5, branchSeed);\r
    vec2 forkEnd = forkStart + (direction * 0.5 - perpendicular * side * 0.7) * extent * 0.5;\r
    float branchBound = length(branchEnd - branchStart) * uBoltWiggle * 4.0 + haloRadius;\r
    float forkBound = length(forkEnd - forkStart) * uBoltWiggle * 4.0 + haloRadius;\r
    if (segmentDistance(point, branchStart, branchEnd) > branchBound\r
      && segmentDistance(point, forkStart, forkEnd) > forkBound) continue;\r
    float branchDistance = 100.0;\r
    float branchProgress = 0.0;\r
    previous = branchStart;\r
    for (int segment = 1; segment <= 16; segment++) {\r
      float progress = min(float(segment) / 16.0, branchGrowth);\r
      vec2 current = channelPoint(branchStart, branchEnd, progress, branchSeed);\r
      float distanceToSegment = segmentDistance(point, previous, current);\r
      if (distanceToSegment < branchDistance) {\r
        branchDistance = distanceToSegment;\r
        branchProgress = progress;\r
      }\r
      previous = current;\r
      if (float(segment) / 16.0 >= branchGrowth) break;\r
    }\r
    float branchWidth = uBoltWidth * mix(0.55, 0.08, branchProgress);\r
    light += channelLight(branchDistance, branchWidth, branchEnergy * (1.0 - branchProgress * 0.75), pixelSize);\r
\r
    if (hash(vec2(branchSeed, 9.0)) < uBoltBranching && branchGrowth > 0.5) {\r
      float forkGrowth = clamp((branchGrowth - 0.5) * 2.5, 0.0, 1.0);\r
      float forkDistance = 100.0;\r
      previous = forkStart;\r
      for (int segment = 1; segment <= 8; segment++) {\r
        float progress = min(float(segment) / 8.0, forkGrowth);\r
        vec2 current = channelPoint(forkStart, forkEnd, progress, branchSeed + 53.0);\r
        forkDistance = min(forkDistance, segmentDistance(point, previous, current));\r
        previous = current;\r
        if (float(segment) / 8.0 >= forkGrowth) break;\r
      }\r
      light += channelLight(forkDistance, uBoltWidth * 0.18, branchEnergy * 0.22, pixelSize);\r
    }\r
  }\r
  return light;\r
}\r
\r
float strokePulse(float age, float onset, float decay) {\r
  float elapsed = age - onset;\r
  return smoothstep(0.0, 0.002, elapsed) * exp(-max(elapsed, 0.0) / decay);\r
}\r
\r
void main() {\r
  vec2 point = (gl_FragCoord.xy - 0.5 * uResolution.xy) / uResolution.y * uZoom;\r
  float pixelSize = uZoom / uResolution.y;\r
  float aspect = uResolution.x / uResolution.y;\r
  float time = uTime * uTimeScale;\r
  vec2 cloudUV = point * uCloudScale + time * uCloudSpeed * vec2(0.03, 0.02);\r
  float cloudBase = fbm(cloudUV, uNoiseOctaves);\r
  float cloudDetail = fbm(cloudUV * 3.1 + cloudBase * 2.0, max(uNoiseOctaves - 1, 1));\r
  float cloudMass = mix(cloudBase, cloudBase * 0.65 + cloudDetail * 0.35, uCloudDetail);\r
  float clouds = smoothstep(0.68 - uCloudDensity * 0.48, 0.88 - uCloudDensity * 0.30, cloudMass);\r
  float relief = fbm(cloudUV + vec2(0.07, 0.11), max(uNoiseOctaves - 1, 1)) - cloudBase;\r
  vec3 color = uCloudColor * (0.10 + clouds * 0.65) * clamp(0.75 + relief * 3.0, 0.3, 1.2);\r
  vec3 lightning = vec3(0.0);\r
\r
  float rate = max(0.08, uFlickerSpeed * 0.6);\r
  float baseEvent = floor(time * rate);\r
  for (int lane = 0; lane < 12; lane++) {\r
    if (lane >= uBoltCount) break;\r
    float laneSeed = float(lane) * 19.19 + mod(uSeed, 10000.0) * 0.137;\r
    for (int candidate = 0; candidate < 4; candidate++) {\r
      float event = baseEvent - float(candidate);\r
      float eventSeed = hash(vec2(event, laneSeed + 11.0)) * 937.0;\r
      float eventTime = (event + hash(vec2(event, laneSeed + 23.0))) / rate;\r
      float age = time - eventTime;\r
      if (age < 0.0 || age > 0.52 || hash(vec2(event, laneSeed + 37.0)) < 0.52) continue;\r
\r
      float leaderDuration = mix(0.018, 0.045, hash(vec2(eventSeed, 1.0)));\r
      float strokeAge = age - leaderDuration;\r
      float spacing = mix(0.11, 0.042, clamp(uBoltNoiseSpeed / 8.0, 0.0, 1.0));\r
      float primary = strokePulse(strokeAge, 0.0, 0.018);\r
      float secondary = strokePulse(strokeAge, spacing, 0.012) * 0.65;\r
      float tertiary = strokePulse(strokeAge, spacing * 2.3, 0.016) * 0.4;\r
      secondary *= step(0.3, hash(vec2(eventSeed, 2.0)));\r
      tertiary *= step(0.55, hash(vec2(eventSeed, 3.0)));\r
      float afterglow = strokePulse(strokeAge, 0.0, 0.065) * 0.08;\r
      float leader = strokeAge < 0.0 ? 0.045 : 0.0;\r
      float flash = primary + secondary + tertiary + afterglow;\r
      float energy = (flash + leader) * uBoltIntensity * 4.0;\r
      if (energy < 0.001) continue;\r
\r
      vec2 start = (vec2(hash(vec2(eventSeed, 21.0)), hash(vec2(eventSeed, 25.0))) - 0.5)\r
                 * vec2(aspect * 0.85, 0.85);\r
      float channelLength = mix(min(uBoltLengthMin, uBoltLengthMax), max(uBoltLengthMin, uBoltLengthMax), hash(vec2(eventSeed, 37.0)));\r
      float angle = hash(vec2(eventSeed, 29.0)) * TAU;\r
      vec2 end = start + vec2(cos(angle), sin(angle)) * channelLength;\r
      float growth = strokeAge < 0.0 ? floor(clamp(age / leaderDuration, 0.0, 1.0) * 24.0) / 24.0 : 1.0;\r
      float branchEnergy = (primary + secondary * 0.22 + tertiary * 0.12 + afterglow + leader) * uBoltIntensity * 3.2;\r
      float channelDistance;\r
      vec3 channel = discharge(point, start, end, eventSeed, growth, energy, branchEnergy, pixelSize, channelDistance);\r
      float veil = smoothstep(0.40, 0.78, cloudMass) * 0.55;\r
      lightning += channel * (1.0 - veil);\r
      float scatter = exp(-channelDistance / (0.045 + clouds * 0.12));\r
      color += mix(uLightningColor, vec3(1.0), 0.3) * scatter * flash * uCloudIllumination\r
             * uBoltIntensity * (0.12 + clouds * 1.8) * (0.65 + cloudDetail * 0.7);\r
    }\r
  }\r
\r
  color += lightning;\r
  color = vec3(1.0) - exp(-color);\r
  outColor = vec4(pow(max(color, vec3(0.0)), vec3(1.0 / 2.2)), 1.0);\r
}`,Ut=`#version 300 es
precision highp float;
precision highp int;
...
`;const k=document.querySelector("#gl-canvas"),te=document.querySelector("#scene-list"),an=document.querySelector("#control-list"),De=document.querySelector("#panel-actions"),ce=document.querySelector("#key-help"),Oe=document.querySelector("#hud-title"),Qn=document.querySelector("#hud-desc"),_e=document.querySelector("[data-action='toggle-sidebar']"),Sn=document.querySelector(".stage");if(!k||!te||!an||!De||!ce||!Oe||!Qn||!_e||!Sn)throw new Error("Missing required UI elements");const s=k.getContext("webgl2",{antialias:!0});if(!s)throw Sn.innerHTML=`
    <div class="fallback">
      <h2>WebGL2 unavailable</h2>
      <p>Your browser or GPU does not expose WebGL2. Try a different browser.</p>
    </div>
  `,new Error("WebGL2 unavailable");const sa=s.getExtension("EXT_color_buffer_float"),ua=!!sa;s.disable(s.DEPTH_TEST);s.disable(s.BLEND);const Dn=er(s),Jn=new Map,et={},ie={},nt={},on=new Map;for(const e of le){const n=Kt(s,dt,e.fragment),t=new Set;t.add(e.resolutionUniform),t.add(e.timeUniform),e.loopUniform&&t.add(e.loopUniform),e.stateful&&(t.add(e.passUniform??"uPass"),t.add(e.stateUniform??"uState"),t.add(e.gridUniform??"uGridSize"));for(const l of e.params)t.add(l.uniform);const r=Qt(s,n,Array.from(t));Jn.set(e.id,{program:n,uniforms:r});const o={},i={};for(const l of e.params)o[l.id]=l,i[l.id]=l.type==="seed"?Math.floor(Math.random()*1e6):l.value;et[e.id]=o,ie[e.id]={...i},nt[e.id]={...i}}let H=le[0],ln={},xn=performance.now(),je=null,Xe=null,$=null,Te=[],tt=0,ze=null,Se=!1;function ca(){const e=["video/webm;codecs=vp9","video/webm;codecs=vp8","video/webm","video/mp4"];for(const n of e)if(MediaRecorder.isTypeSupported(n))return n;return""}function fa(e){return e.startsWith("video/mp4")?"mp4":"webm"}function ma(e){const n=Math.floor(e/1e3),t=String(Math.floor(n/60)).padStart(2,"0"),r=String(n%60).padStart(2,"0");return`${t}:${r}`}function pa(){const e=document.getElementById("rec-badge");!e||!Se||(e.textContent=`⏺ ${ma(performance.now()-tt)}`)}function da(){const e=ca();if(!e){alert("Recording is not supported in this browser.");return}const n=k.captureStream(60);Te=[],$=new MediaRecorder(n,{mimeType:e,videoBitsPerSecond:16e6}),$.ondataavailable=t=>{t.data.size>0&&Te.push(t.data)},$.onstop=()=>{const t=fa(e),r=new Blob(Te,{type:e}),o=URL.createObjectURL(r),i=document.createElement("a");i.href=o,i.download=`${H.id}-${Date.now()}.${t}`,i.click(),URL.revokeObjectURL(o),Te=[]},$.start(500),Se=!0,tt=performance.now(),rt(),ze=window.setInterval(pa,250)}function ha(){$&&$.state!=="inactive"&&$.stop(),Se=!1,ze!==null&&(clearInterval(ze),ze=null),rt()}function va(){Se?ha():da()}function rt(){const e=document.getElementById("rec-btn"),n=document.getElementById("rec-badge");!e||!n||(Se?(e.textContent="Stop",e.classList.add("recording"),n.classList.remove("hidden")):(e.textContent="Record",e.classList.remove("recording"),n.classList.add("hidden"),n.textContent=""))}let We=!1;function sn(e){_e.classList.toggle("hidden",e)}function ya(){sn(!1),Xe!==null&&window.clearTimeout(Xe),Xe=window.setTimeout(()=>{sn(!0)},2500)}function at(){const e=document.body.classList.contains("sidebar-collapsed");_e.textContent=e?">>":"<<"}function ga(){var e;(e=Oe.parentElement)==null||e.classList.remove("hidden"),je!==null&&window.clearTimeout(je),je=window.setTimeout(()=>{var n;(n=Oe.parentElement)==null||n.classList.add("hidden")},1e4)}function zn(e){const n=s.createTexture();if(!n)throw new Error("Failed to create state texture");return s.bindTexture(s.TEXTURE_2D,n),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MIN_FILTER,s.NEAREST),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MAG_FILTER,s.NEAREST),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_S,s.REPEAT),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_T,s.REPEAT),s.texImage2D(s.TEXTURE_2D,0,s.RGBA16F,e,e,0,s.RGBA,s.HALF_FLOAT,null),s.bindTexture(s.TEXTURE_2D,null),n}function ba(e){const n=zn(e),t=zn(e),r=s.createFramebuffer(),o=s.createFramebuffer();if(!r||!o)throw new Error("Failed to create framebuffer");return s.bindFramebuffer(s.FRAMEBUFFER,r),s.framebufferTexture2D(s.FRAMEBUFFER,s.COLOR_ATTACHMENT0,s.TEXTURE_2D,n,0),s.bindFramebuffer(s.FRAMEBUFFER,o),s.framebufferTexture2D(s.FRAMEBUFFER,s.COLOR_ATTACHMENT0,s.TEXTURE_2D,t,0),s.bindFramebuffer(s.FRAMEBUFFER,null),{size:e,textures:[n,t],fbos:[r,o],index:0,needsInit:!0}}function wn(e){if(!e.stateful)return null;let n=on.get(e.id);const t=e.bufferSize??192;return(!n||n.size!==t)&&(n=ba(t),on.set(e.id,n)),n}function ye(e){const n=on.get(e);n&&(n.needsInit=!0)}function Sa(e,n){let t=n;return e.min!==void 0&&(t=Math.max(e.min,t)),e.max!==void 0&&(t=Math.min(e.max,t)),e.type==="int"&&(t=Math.round(t)),t}function ot(e,n){if(e.type==="int")return String(Math.round(n));const t=e.step??.01,r=t<1?Math.min(4,Math.max(2,Math.ceil(-Math.log10(t)))):0;return n.toFixed(r)}function xa(e){return e.length===1?e.toLowerCase():e}function wa(e){return e instanceof HTMLInputElement||e instanceof HTMLTextAreaElement||e instanceof HTMLSelectElement}function ge(e,n,t,r=!0){const o=et[e][n];if(!o)return;const i=Sa(o,t);if(ie[e][n]=i,e===H.id&&r){const l=ln[n];l!=null&&l.range&&(l.range.value=String(i)),l!=null&&l.number&&(l.number.value=ot(o,i))}}function Ca(e){var n;for(const t of((n=le.find(r=>r.id===e))==null?void 0:n.params)??[])t.type==="seed"&&ge(e,t.id,Math.floor(Math.random()*1e6));ye(e)}function Ta(e){const n=nt[e];for(const[t,r]of Object.entries(n))ge(e,t,r,!0);ye(e)}function ka(){te.innerHTML="";for(const e of le){const n=document.createElement("option");n.value=e.id,n.textContent=e.name,te.appendChild(n)}te.addEventListener("change",()=>{it(te.value)})}function Ra(e){De.innerHTML="";const n=document.createElement("button");if(n.className="ghost small",n.textContent="Reset",n.addEventListener("click",()=>Ta(e.id)),De.appendChild(n),e.params.some(r=>r.type==="seed")){const r=document.createElement("button");r.className="ghost small",r.textContent="Reseed",r.addEventListener("click",()=>Ca(e.id)),De.appendChild(r)}}function Aa(e){an.innerHTML="",ln={};for(const n of e.params){if(n.type==="seed")continue;const t=document.createElement("div");t.className="control";const r=document.createElement("div");r.className="control-header";const o=document.createElement("label");if(o.textContent=n.label,r.appendChild(o),n.key){const p=document.createElement("span");p.className="key-cap",p.textContent=`${n.key.inc.toUpperCase()}/${n.key.dec.toUpperCase()}`,r.appendChild(p)}const i=document.createElement("div");i.className="control-inputs";const l=document.createElement("input");l.type="range",l.min=String(n.min??0),l.max=String(n.max??1),l.step=String(n.step??(n.type==="int"?1:.01)),l.value=String(ie[e.id][n.id]),l.addEventListener("input",p=>{const y=Number(p.target.value);Number.isNaN(y)||ge(e.id,n.id,y)});const u=document.createElement("input");u.type="number",u.min=l.min,u.max=l.max,u.step=l.step,u.value=ot(n,ie[e.id][n.id]),u.addEventListener("input",p=>{const y=Number(p.target.value);Number.isNaN(y)||ge(e.id,n.id,y)}),i.appendChild(l),i.appendChild(u),t.appendChild(r),t.appendChild(i),an.appendChild(t),ln[n.id]={range:l,number:u}}}function Ba(e){ce.innerHTML="";for(const n of e.params){if(!n.key||n.type==="seed")continue;const t=document.createElement("div");t.className="key-row",t.textContent=`${n.key.inc.toUpperCase()}/${n.key.dec.toUpperCase()}  ${n.label}`,ce.appendChild(t)}ce.childElementCount||(ce.textContent="No mapped keys for this scene.")}function it(e){const n=le.find(t=>t.id===e);n&&(H=n,n.stateful&&(wn(n),ye(n.id)),Oe.textContent=n.name,Qn.textContent=n.description,ga(),Ra(n),Aa(n),Ba(n),te.value=n.id)}function Pa(e){if(wa(e.target))return;const n=xa(e.key),t=H.params;for(const r of t){if(!r.key||r.type==="seed")continue;const o=n===r.key.inc,i=n===r.key.dec;if(!o&&!i)continue;const l=e.shiftKey&&r.key.shiftStep?r.key.shiftStep:r.key.step,p=ie[H.id][r.id]+l*(o?1:-1);ge(H.id,r.id,p),e.preventDefault();break}}function ke(e,n,t,r,o){const i=ie[e.id],l=n.uniforms,u=l[e.resolutionUniform];u&&s.uniform2f(u,r,o);const p=l[e.timeUniform];if(p)if(e.timeMode==="phase"){const g=e.loopDuration??8,w=t%g/g;s.uniform1f(p,w)}else if(e.timeMode==="looped"){const g=e.loopDuration??8,w=t%g;if(s.uniform1f(p,w),e.loopUniform){const S=l[e.loopUniform];S&&s.uniform1f(S,g)}}else s.uniform1f(p,t);const y={};for(const g of e.params){const w=l[g.uniform],S=i[g.id];if(g.component!==void 0){const q=y[g.uniform]??[0,0,0];q[g.component]=S,y[g.uniform]=q;continue}w&&(g.type==="int"?s.uniform1i(w,Math.round(S)):s.uniform1f(w,S))}for(const[g,w]of Object.entries(y)){const S=l[g];S&&s.uniform3f(S,w[0],w[1],w[2])}}function Ze(e,n,t,r){const o=n.uniforms[e.passUniform??"uPass"];o&&s.uniform1i(o,r);const i=n.uniforms[e.gridUniform??"uGridSize"];i&&s.uniform2f(i,t.size,t.size);const l=n.uniforms[e.stateUniform??"uState"];l&&s.uniform1i(l,0)}function lt(e,n,t,r){const o=Jn.get(e.id);if(o){if(e.stateful){if(!ua)return;const i=wn(e);if(!i)return;const l=()=>i.textures[i.index],u=()=>i.fbos[(i.index+1)%2];s.useProgram(o.program),s.bindVertexArray(Dn),i.needsInit&&(s.bindFramebuffer(s.FRAMEBUFFER,u()),s.viewport(0,0,i.size,i.size),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,l()),ke(e,o,n,t,r),Ze(e,o,i,2),s.drawArrays(s.TRIANGLES,0,3),i.index=(i.index+1)%2,i.needsInit=!1),s.bindFramebuffer(s.FRAMEBUFFER,u()),s.viewport(0,0,i.size,i.size),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,l()),ke(e,o,n,t,r),Ze(e,o,i,0),s.drawArrays(s.TRIANGLES,0,3),i.index=(i.index+1)%2,s.bindFramebuffer(s.FRAMEBUFFER,null),s.viewport(0,0,t,r),s.clearColor(0,0,0,1),s.clear(s.COLOR_BUFFER_BIT),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,l()),ke(e,o,n,t,r),Ze(e,o,i,1),s.drawArrays(s.TRIANGLES,0,3);return}s.viewport(0,0,t,r),s.clearColor(0,0,0,1),s.clear(s.COLOR_BUFFER_BIT),s.useProgram(o.program),s.bindVertexArray(Dn),ke(e,o,n,t,r),s.drawArrays(s.TRIANGLES,0,3)}}function Cn(e){if(We)return;const n=(e-xn)/1e3,{width:t,height:r}=Jt(k);lt(H,n,t,r),requestAnimationFrame(Cn)}function Fa(){return new Promise(e=>requestAnimationFrame(()=>e()))}async function Da(e,n,t,r,o){if(typeof VideoEncoder>"u"||typeof VideoFrame>"u"){alert("Offline rendering requires the WebCodecs API (Chrome/Edge 94+, Safari 16.4+).");return}const i={codec:"avc1.640028",width:r,height:o,bitrate:16e6,framerate:t};if(!(await VideoEncoder.isConfigSupported(i)).supported){alert("H.264 video encoding is not supported on this device.");return}let u;try{u=await window.showSaveFilePicker({suggestedName:`${e.id}-${r}x${o}-${t}fps-${n}s.mp4`,types:[{description:"MP4 Video",accept:{"video/mp4":[".mp4"]}}]})}catch{return}const p=await u.createWritable(),y=document.getElementById("offline-btn"),g=document.getElementById("offline-progress"),w=document.getElementById("offline-bar"),S=document.getElementById("offline-status");y&&(y.disabled=!0),g==null||g.classList.remove("hidden"),S==null||S.classList.remove("hidden"),We=!0,e.stateful&&(wn(e),ye(e.id));const q=k.width,W=k.height,M=k.style.width,Q=k.style.height;k.width=r,k.height=o,k.style.width=`${r}px`,k.style.height=`${o}px`;const st=new Ln(p),Tn=new la({target:st,video:{codec:"avc",width:r,height:o},fastStart:!1});let xe=null;const J=new VideoEncoder({output:(C,qe)=>Tn.addVideoChunk(C,qe??void 0),error:C=>{xe=C}});J.configure(i);const we=Math.ceil(n*t),kn=Math.round(1e6/t),ut=t*2,ct=t*30,ft=performance.now();let Ce=!1;const Rn=C=>{C.preventDefault(),Ce=!0};k.addEventListener("webglcontextlost",Rn);for(let C=0;C<we&&!(xe||Ce);C++){const qe=C/t;lt(e,qe,r,o),s.finish();const An=new VideoFrame(k,{timestamp:C*kn,duration:kn});J.encode(An,{keyFrame:C%ut===0}),An.close(),C>0&&C%ct===0&&await J.flush();const mt=(C+1)/we*100;if(w&&(w.style.width=`${mt}%`),C%30===0){const pt=(performance.now()-ft)/1e3/(C+1)*(we-C-1);S&&(S.textContent=`Frame ${C+1}/${we}  —  ~${Math.ceil(pt)}s left`)}J.encodeQueueSize>10&&await new Promise(Bn=>setTimeout(Bn,1)),await Fa()}k.removeEventListener("webglcontextlost",Rn);try{await J.flush(),J.close(),Tn.finalize(),await p.close()}catch(C){console.error("Finalization failed:",C);try{await p.close()}catch{}}k.style.width=M,k.style.height=Q,k.width=q,k.height=W,e.stateful&&ye(e.id),We=!1,xn=performance.now(),requestAnimationFrame(Cn),y&&(y.disabled=!1);const He=xe?xe.message:null;S&&(Ce?S.textContent="Context lost — partial video saved.":He?S.textContent=`Error: ${He}`:S.textContent="Done!"),setTimeout(()=>{g==null||g.classList.add("hidden"),S==null||S.classList.add("hidden"),w&&(w.style.width="0%")},Ce||!!He?8e3:3e3)}_e.addEventListener("click",()=>{document.body.classList.toggle("sidebar-collapsed"),at()});Sn.addEventListener("mousemove",()=>{ya()});document.addEventListener("keydown",Pa);document.addEventListener("visibilitychange",()=>{document.hidden||(xn=performance.now())});var Mn;(Mn=document.getElementById("rec-btn"))==null||Mn.addEventListener("click",va);var Gn;(Gn=document.getElementById("offline-btn"))==null||Gn.addEventListener("click",()=>{if(We)return;const e=document.getElementById("offline-duration"),n=document.getElementById("offline-fps"),t=document.getElementById("offline-res"),r=Math.max(1,Math.min(3600,Number((e==null?void 0:e.value)??10))),o=Number((n==null?void 0:n.value)??60),[i,l]=((t==null?void 0:t.value)??"1920x1080").split("x").map(Number);Da(H,r,o,i,l)});ka();it(H.id);at();sn(!0);requestAnimationFrame(Cn);

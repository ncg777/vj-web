...
`;const R=document.querySelector("#gl-canvas"),ee=document.querySelector("#scene-list"),nt=document.querySelector("#control-list"),Fe=document.querySelector("#panel-actions"),se=document.querySelector("#key-help"),Ne=document.querySelector("#hud-title"),Zt=document.querySelector("#hud-desc"),Oe=document.querySelector("[data-action='toggle-sidebar']"),yt=document.querySelector(".stage");if(!R||!ee||!nt||!Fe||!se||!Ne||!Zt||!Oe||!yt)throw new Error("Missing required UI elements");const s=R.getContext("webgl2",{antialias:!0});if(!s)throw yt.innerHTML=`
    <div class="fallback">
      <h2>WebGL2 unavailable</h2>
      <p>Your browser or GPU does not expose WebGL2. Try a different browser.</p>
    </div>
  `,new Error("WebGL2 unavailable");const Xo=s.getExtension("EXT_color_buffer_float"),$o=!!Xo;s.disable(s.DEPTH_TEST);s.disable(s.BLEND);const Bt=Un(s),Yt=new Map,Kt={},ie={},Qt={},ot=new Map;for(const e of ae){const t=Dn(s,mn,e.fragment),n=new Set;n.add(e.resolutionUniform),n.add(e.timeUniform),e.loopUniform&&n.add(e.loopUniform),e.stateful&&(n.add(e.passUniform??"uPass"),n.add(e.stateUniform??"uState"),n.add(e.gridUniform??"uGridSize"));for(const r of e.params)n.add(r.uniform);const i=_n(s,t,Array.from(n));Yt.set(e.id,{program:t,uniforms:i});const a={},l={};for(const r of e.params)a[r.id]=r,l[r.id]=r.type==="seed"?Math.floor(Math.random()*1e6):r.value;Kt[e.id]=a,ie[e.id]={...l},Qt[e.id]={...l}}let L=ae[0],it={},bt=performance.now(),qe=null,Ve=null,X=null,we=[],Jt=0,Ee=null,be=!1;function Zo(){const e=["video/webm;codecs=vp9","video/webm;codecs=vp8","video/webm","video/mp4"];for(const t of e)if(MediaRecorder.isTypeSupported(t))return t;return""}function Yo(e){return e.startsWith("video/mp4")?"mp4":"webm"}function Ko(e){const t=Math.floor(e/1e3),n=String(Math.floor(t/60)).padStart(2,"0"),i=String(t%60).padStart(2,"0");return`${n}:${i}`}function Qo(){const e=document.getElementById("rec-badge");!e||!be||(e.textContent=`⏺ ${Ko(performance.now()-Jt)}`)}function Jo(){const e=Zo();if(!e){alert("Recording is not supported in this browser.");return}const t=R.captureStream(60);we=[],X=new MediaRecorder(t,{mimeType:e,videoBitsPerSecond:16e6}),X.ondataavailable=n=>{n.data.size>0&&we.push(n.data)},X.onstop=()=>{const n=Yo(e),i=new Blob(we,{type:e}),a=URL.createObjectURL(i),l=document.createElement("a");l.href=a,l.download=`${L.id}-${Date.now()}.${n}`,l.click(),URL.revokeObjectURL(a),we=[]},X.start(500),be=!0,Jt=performance.now(),en(),Ee=window.setInterval(Qo,250)}function ei(){X&&X.state!=="inactive"&&X.stop(),be=!1,Ee!==null&&(clearInterval(Ee),Ee=null),en()}function ti(){be?ei():Jo()}function en(){const e=document.getElementById("rec-btn"),t=document.getElementById("rec-badge");!e||!t||(be?(e.textContent="Stop",e.classList.add("recording"),t.classList.remove("hidden")):(e.textContent="Record",e.classList.remove("recording"),t.classList.add("hidden"),t.textContent=""))}let Ue=!1;function at(e){Oe.classList.toggle("hidden",e)}function ni(){at(!1),Ve!==null&&window.clearTimeout(Ve),Ve=window.setTimeout(()=>{at(!0)},2500)}function tn(){const e=document.body.classList.contains("sidebar-collapsed");Oe.textContent=e?">>":"<<"}function oi(){var e;(e=Ne.parentElement)==null||e.classList.remove("hidden"),qe!==null&&window.clearTimeout(qe),qe=window.setTimeout(()=>{var t;(t=Ne.parentElement)==null||t.classList.add("hidden")},1e4)}function Ft(e){const t=s.createTexture();if(!t)throw new Error("Failed to create state texture");return s.bindTexture(s.TEXTURE_2D,t),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MIN_FILTER,s.NEAREST),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MAG_FILTER,s.NEAREST),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_S,s.REPEAT),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_T,s.REPEAT),s.texImage2D(s.TEXTURE_2D,0,s.RGBA16F,e,e,0,s.RGBA,s.HALF_FLOAT,null),s.bindTexture(s.TEXTURE_2D,null),t}function ii(e){const t=Ft(e),n=Ft(e),i=s.createFramebuffer(),a=s.createFramebuffer();if(!i||!a)throw new Error("Failed to create framebuffer");return s.bindFramebuffer(s.FRAMEBUFFER,i),s.framebufferTexture2D(s.FRAMEBUFFER,s.COLOR_ATTACHMENT0,s.TEXTURE_2D,t,0),s.bindFramebuffer(s.FRAMEBUFFER,a),s.framebufferTexture2D(s.FRAMEBUFFER,s.COLOR_ATTACHMENT0,s.TEXTURE_2D,n,0),s.bindFramebuffer(s.FRAMEBUFFER,null),{size:e,textures:[t,n],fbos:[i,a],index:0,needsInit:!0}}function St(e){if(!e.stateful)return null;let t=ot.get(e.id);const n=e.bufferSize??192;return(!t||t.size!==n)&&(t=ii(n),ot.set(e.id,t)),t}function he(e){const t=ot.get(e);t&&(t.needsInit=!0)}function ai(e,t){let n=t;return e.min!==void 0&&(n=Math.max(e.min,n)),e.max!==void 0&&(n=Math.min(e.max,n)),e.type==="int"&&(n=Math.round(n)),n}function nn(e,t){if(e.type==="int")return String(Math.round(t));const n=e.step??.01,i=n<1?Math.min(4,Math.max(2,Math.ceil(-Math.log10(n)))):0;return t.toFixed(i)}function li(e){return e.length===1?e.toLowerCase():e}function ri(e){return e instanceof HTMLInputElement||e instanceof HTMLTextAreaElement||e instanceof HTMLSelectElement}function ve(e,t,n,i=!0){const a=Kt[e][t];if(!a)return;const l=ai(a,n);if(ie[e][t]=l,e===L.id&&i){const r=it[t];r!=null&&r.range&&(r.range.value=String(l)),r!=null&&r.number&&(r.number.value=nn(a,l))}}function si(e){var t;for(const n of((t=ae.find(i=>i.id===e))==null?void 0:t.params)??[])n.type==="seed"&&ve(e,n.id,Math.floor(Math.random()*1e6));he(e)}function ui(e){const t=Qt[e];for(const[n,i]of Object.entries(t))ve(e,n,i,!0);he(e)}function fi(){ee.innerHTML="";for(const e of ae){const t=document.createElement("option");t.value=e.id,t.textContent=e.name,ee.appendChild(t)}ee.addEventListener("change",()=>{on(ee.value)})}function ci(e){Fe.innerHTML="";const t=document.createElement("button");if(t.className="ghost small",t.textContent="Reset",t.addEventListener("click",()=>ui(e.id)),Fe.appendChild(t),e.params.some(i=>i.type==="seed")){const i=document.createElement("button");i.className="ghost small",i.textContent="Reseed",i.addEventListener("click",()=>si(e.id)),Fe.appendChild(i)}}function mi(e){nt.innerHTML="",it={};for(const t of e.params){if(t.type==="seed")continue;const n=document.createElement("div");n.className="control";const i=document.createElement("div");i.className="control-header";const a=document.createElement("label");if(a.textContent=t.label,i.appendChild(a),t.key){const p=document.createElement("span");p.className="key-cap",p.textContent=`${t.key.inc.toUpperCase()}/${t.key.dec.toUpperCase()}`,i.appendChild(p)}const l=document.createElement("div");l.className="control-inputs";const r=document.createElement("input");r.type="range",r.min=String(t.min??0),r.max=String(t.max??1),r.step=String(t.step??(t.type==="int"?1:.01)),r.value=String(ie[e.id][t.id]),r.addEventListener("input",p=>{const y=Number(p.target.value);Number.isNaN(y)||ve(e.id,t.id,y)});const u=document.createElement("input");u.type="number",u.min=r.min,u.max=r.max,u.step=r.step,u.value=nn(t,ie[e.id][t.id]),u.addEventListener("input",p=>{const y=Number(p.target.value);Number.isNaN(y)||ve(e.id,t.id,y)}),l.appendChild(r),l.appendChild(u),n.appendChild(i),n.appendChild(l),nt.appendChild(n),it[t.id]={range:r,number:u}}}function pi(e){se.innerHTML="";for(const t of e.params){if(!t.key||t.type==="seed")continue;const n=document.createElement("div");n.className="key-row",n.textContent=`${t.key.inc.toUpperCase()}/${t.key.dec.toUpperCase()}  ${t.label}`,se.appendChild(n)}se.childElementCount||(se.textContent="No mapped keys for this scene.")}function on(e){const t=ae.find(n=>n.id===e);t&&(L=t,t.stateful&&(St(t),he(t.id)),Ne.textContent=t.name,Zt.textContent=t.description,oi(),ci(t),mi(t),pi(t),ee.value=t.id)}function di(e){if(ri(e.target))return;const t=li(e.key),n=L.params;for(const i of n){if(!i.key||i.type==="seed")continue;const a=t===i.key.inc,l=t===i.key.dec;if(!a&&!l)continue;const r=e.shiftKey&&i.key.shiftStep?i.key.shiftStep:i.key.step,p=ie[L.id][i.id]+r*(a?1:-1);ve(L.id,i.id,p),e.preventDefault();break}}function Ce(e,t,n,i,a){const l=ie[e.id],r=t.uniforms,u=r[e.resolutionUniform];u&&s.uniform2f(u,i,a);const p=r[e.timeUniform];if(p)if(e.timeMode==="phase"){const b=e.loopDuration??8,w=n%b/b;s.uniform1f(p,w)}else if(e.timeMode==="looped"){const b=e.loopDuration??8,w=n%b;if(s.uniform1f(p,w),e.loopUniform){const g=r[e.loopUniform];g&&s.uniform1f(g,b)}}else s.uniform1f(p,n);const y={};for(const b of e.params){const w=r[b.uniform],g=l[b.id];if(b.component!==void 0){const W=y[b.uniform]??[0,0,0];W[b.component]=g,y[b.uniform]=W;continue}w&&(b.type==="int"?s.uniform1i(w,Math.round(g)):s.uniform1f(w,g))}for(const[b,w]of Object.entries(y)){const g=r[b];g&&s.uniform3f(g,w[0],w[1],w[2])}}function je(e,t,n,i){const a=t.uniforms[e.passUniform??"uPass"];a&&s.uniform1i(a,i);const l=t.uniforms[e.gridUniform??"uGridSize"];l&&s.uniform2f(l,n.size,n.size);const r=t.uniforms[e.stateUniform??"uState"];r&&s.uniform1i(r,0)}function an(e,t,n,i){const a=Yt.get(e.id);if(a){if(e.stateful){if(!$o)return;const l=St(e);if(!l)return;const r=()=>l.textures[l.index],u=()=>l.fbos[(l.index+1)%2];s.useProgram(a.program),s.bindVertexArray(Bt),l.needsInit&&(s.bindFramebuffer(s.FRAMEBUFFER,u()),s.viewport(0,0,l.size,l.size),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,r()),Ce(e,a,t,n,i),je(e,a,l,2),s.drawArrays(s.TRIANGLES,0,3),l.index=(l.index+1)%2,l.needsInit=!1),s.bindFramebuffer(s.FRAMEBUFFER,u()),s.viewport(0,0,l.size,l.size),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,r()),Ce(e,a,t,n,i),je(e,a,l,0),s.drawArrays(s.TRIANGLES,0,3),l.index=(l.index+1)%2,s.bindFramebuffer(s.FRAMEBUFFER,null),s.viewport(0,0,n,i),s.clearColor(0,0,0,1),s.clear(s.COLOR_BUFFER_BIT),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,r()),Ce(e,a,t,n,i),je(e,a,l,1),s.drawArrays(s.TRIANGLES,0,3);return}s.viewport(0,0,n,i),s.clearColor(0,0,0,1),s.clear(s.COLOR_BUFFER_BIT),s.useProgram(a.program),s.bindVertexArray(Bt),Ce(e,a,t,n,i),s.drawArrays(s.TRIANGLES,0,3)}}function gt(e){if(Ue)return;const t=(e-bt)/1e3,{width:n,height:i}=Nn(R);an(L,t,n,i),requestAnimationFrame(gt)}function hi(){return new Promise(e=>requestAnimationFrame(()=>e()))}async function vi(e,t,n,i,a){if(typeof VideoEncoder>"u"||typeof VideoFrame>"u"){alert("Offline rendering requires the WebCodecs API (Chrome/Edge 94+, Safari 16.4+).");return}const l={codec:"avc1.640028",width:i,height:a,bitrate:16e6,framerate:n};if(!(await VideoEncoder.isConfigSupported(l)).supported){alert("H.264 video encoding is not supported on this device.");return}let u;try{u=await window.showSaveFilePicker({suggestedName:`${e.id}-${i}x${a}-${n}fps-${t}s.mp4`,types:[{description:"MP4 Video",accept:{"video/mp4":[".mp4"]}}]})}catch{return}const p=await u.createWritable(),y=document.getElementById("offline-btn"),b=document.getElementById("offline-progress"),w=document.getElementById("offline-bar"),g=document.getElementById("offline-status");y&&(y.disabled=!0),b==null||b.classList.remove("hidden"),g==null||g.classList.remove("hidden"),Ue=!0,e.stateful&&(St(e),he(e.id));const W=R.width,U=R.height,P=R.style.width,Y=R.style.height;R.width=i,R.height=a,R.style.width=`${i}px`,R.style.height=`${a}px`;const ln=new Ut(p),xt=new jo({target:ln,video:{codec:"avc",width:i,height:a},fastStart:!1});let Se=null;const K=new VideoEncoder({output:(C,We)=>xt.addVideoChunk(C,We??void 0),error:C=>{Se=C}});K.configure(l);const ge=Math.ceil(t*n),wt=Math.round(1e6/n),rn=n*2,sn=n*30,un=performance.now();let xe=!1;const Ct=C=>{C.preventDefault(),xe=!0};R.addEventListener("webglcontextlost",Ct);for(let C=0;C<ge&&!(Se||xe);C++){const We=C/n;an(e,We,i,a),s.finish();const Tt=new VideoFrame(R,{timestamp:C*wt,duration:wt});K.encode(Tt,{keyFrame:C%rn===0}),Tt.close(),C>0&&C%sn===0&&await K.flush();const fn=(C+1)/ge*100;if(w&&(w.style.width=`${fn}%`),C%30===0){const cn=(performance.now()-un)/1e3/(C+1)*(ge-C-1);g&&(g.textContent=`Frame ${C+1}/${ge}  —  ~${Math.ceil(cn)}s left`)}K.encodeQueueSize>10&&await new Promise(Rt=>setTimeout(Rt,1)),await hi()}R.removeEventListener("webglcontextlost",Ct);try{await K.flush(),K.close(),xt.finalize(),await p.close()}catch(C){console.error("Finalization failed:",C);try{await p.close()}catch{}}R.style.width=P,R.style.height=Y,R.width=W,R.height=U,e.stateful&&he(e.id),Ue=!1,bt=performance.now(),requestAnimationFrame(gt),y&&(y.disabled=!1);const Le=Se?Se.message:null;g&&(xe?g.textContent="Context lost — partial video saved.":Le?g.textContent=`Error: ${Le}`:g.textContent="Done!"),setTimeout(()=>{b==null||b.classList.add("hidden"),g==null||g.classList.add("hidden"),w&&(w.style.width="0%")},xe||!!Le?8e3:3e3)}Oe.addEventListener("click",()=>{document.body.classList.toggle("sidebar-collapsed"),tn()});yt.addEventListener("mousemove",()=>{ni()});document.addEventListener("keydown",di);document.addEventListener("visibilitychange",()=>{document.hidden||(bt=performance.now())});var Et;(Et=document.getElementById("rec-btn"))==null||Et.addEventListener("click",ti);var It;(It=document.getElementById("offline-btn"))==null||It.addEventListener("click",()=>{if(Ue)return;const e=document.getElementById("offline-duration"),t=document.getElementById("offline-fps"),n=document.getElementById("offline-res"),i=Math.max(1,Math.min(3600,Number((e==null?void 0:e.value)??10))),a=Number((t==null?void 0:t.value)??60),[l,r]=((n==null?void 0:n.value)??"1920x1080").split("x").map(Number);vi(L,i,a,l,r)});fi();on(L.id);tn();at(!0);requestAnimationFrame(gt);

...
`;const k=document.querySelector("#gl-canvas"),ee=document.querySelector("#scene-list"),nt=document.querySelector("#control-list"),Me=document.querySelector("#panel-actions"),se=document.querySelector("#key-help"),Ue=document.querySelector("#hud-title"),Zt=document.querySelector("#hud-desc"),_e=document.querySelector("[data-action='toggle-sidebar']"),yt=document.querySelector(".stage");if(!k||!ee||!nt||!Me||!se||!Ue||!Zt||!_e||!yt)throw new Error("Missing required UI elements");const s=k.getContext("webgl2",{antialias:!0});if(!s)throw yt.innerHTML=`
    <div class="fallback">
      <h2>WebGL2 unavailable</h2>
      <p>Your browser or GPU does not expose WebGL2. Try a different browser.</p>
    </div>
  `,new Error("WebGL2 unavailable");const Xa=s.getExtension("EXT_color_buffer_float"),$a=!!Xa;s.disable(s.DEPTH_TEST);s.disable(s.BLEND);const Bt=On(s),Yt=new Map,Kt={},ie={},Qt={},at=new Map;for(const e of oe){const t=Gn(s,mn,e.fragment),n=new Set;n.add(e.resolutionUniform),n.add(e.timeUniform),e.loopUniform&&n.add(e.loopUniform),e.stateful&&(n.add(e.passUniform??"uPass"),n.add(e.stateUniform??"uState"),n.add(e.gridUniform??"uGridSize"));for(const r of e.params)n.add(r.uniform);const i=Un(s,t,Array.from(n));Yt.set(e.id,{program:t,uniforms:i});const o={},l={};for(const r of e.params)o[r.id]=r,l[r.id]=r.type==="seed"?Math.floor(Math.random()*1e6):r.value;Kt[e.id]=o,ie[e.id]={...l},Qt[e.id]={...l}}let N=oe[0],it={},bt=performance.now(),qe=null,Ve=null,X=null,we=[],Jt=0,Fe=null,be=!1;function Za(){const e=["video/webm;codecs=vp9","video/webm;codecs=vp8","video/webm","video/mp4"];for(const t of e)if(MediaRecorder.isTypeSupported(t))return t;return""}function Ya(e){return e.startsWith("video/mp4")?"mp4":"webm"}function Ka(e){const t=Math.floor(e/1e3),n=String(Math.floor(t/60)).padStart(2,"0"),i=String(t%60).padStart(2,"0");return`${n}:${i}`}function Qa(){const e=document.getElementById("rec-badge");!e||!be||(e.textContent=`⏺ ${Ka(performance.now()-Jt)}`)}function Ja(){const e=Za();if(!e){alert("Recording is not supported in this browser.");return}const t=k.captureStream(60);we=[],X=new MediaRecorder(t,{mimeType:e,videoBitsPerSecond:16e6}),X.ondataavailable=n=>{n.data.size>0&&we.push(n.data)},X.onstop=()=>{const n=Ya(e),i=new Blob(we,{type:e}),o=URL.createObjectURL(i),l=document.createElement("a");l.href=o,l.download=`${N.id}-${Date.now()}.${n}`,l.click(),URL.revokeObjectURL(o),we=[]},X.start(500),be=!0,Jt=performance.now(),en(),Fe=window.setInterval(Qa,250)}function ei(){X&&X.state!=="inactive"&&X.stop(),be=!1,Fe!==null&&(clearInterval(Fe),Fe=null),en()}function ti(){be?ei():Ja()}function en(){const e=document.getElementById("rec-btn"),t=document.getElementById("rec-badge");!e||!t||(be?(e.textContent="Stop",e.classList.add("recording"),t.classList.remove("hidden")):(e.textContent="Record",e.classList.remove("recording"),t.classList.add("hidden"),t.textContent=""))}let We=!1;function ot(e){_e.classList.toggle("hidden",e)}function ni(){ot(!1),Ve!==null&&window.clearTimeout(Ve),Ve=window.setTimeout(()=>{ot(!0)},2500)}function tn(){const e=document.body.classList.contains("sidebar-collapsed");_e.textContent=e?">>":"<<"}function ai(){var e;(e=Ue.parentElement)==null||e.classList.remove("hidden"),qe!==null&&window.clearTimeout(qe),qe=window.setTimeout(()=>{var t;(t=Ue.parentElement)==null||t.classList.add("hidden")},1e4)}function Mt(e){const t=s.createTexture();if(!t)throw new Error("Failed to create state texture");return s.bindTexture(s.TEXTURE_2D,t),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MIN_FILTER,s.NEAREST),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_MAG_FILTER,s.NEAREST),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_S,s.REPEAT),s.texParameteri(s.TEXTURE_2D,s.TEXTURE_WRAP_T,s.REPEAT),s.texImage2D(s.TEXTURE_2D,0,s.RGBA16F,e,e,0,s.RGBA,s.HALF_FLOAT,null),s.bindTexture(s.TEXTURE_2D,null),t}function ii(e){const t=Mt(e),n=Mt(e),i=s.createFramebuffer(),o=s.createFramebuffer();if(!i||!o)throw new Error("Failed to create framebuffer");return s.bindFramebuffer(s.FRAMEBUFFER,i),s.framebufferTexture2D(s.FRAMEBUFFER,s.COLOR_ATTACHMENT0,s.TEXTURE_2D,t,0),s.bindFramebuffer(s.FRAMEBUFFER,o),s.framebufferTexture2D(s.FRAMEBUFFER,s.COLOR_ATTACHMENT0,s.TEXTURE_2D,n,0),s.bindFramebuffer(s.FRAMEBUFFER,null),{size:e,textures:[t,n],fbos:[i,o],index:0,needsInit:!0}}function gt(e){if(!e.stateful)return null;let t=at.get(e.id);const n=e.bufferSize??192;return(!t||t.size!==n)&&(t=ii(n),at.set(e.id,t)),t}function he(e){const t=at.get(e);t&&(t.needsInit=!0)}function oi(e,t){let n=t;return e.min!==void 0&&(n=Math.max(e.min,n)),e.max!==void 0&&(n=Math.min(e.max,n)),e.type==="int"&&(n=Math.round(n)),n}function nn(e,t){if(e.type==="int")return String(Math.round(t));const n=e.step??.01,i=n<1?Math.min(4,Math.max(2,Math.ceil(-Math.log10(n)))):0;return t.toFixed(i)}function li(e){return e.length===1?e.toLowerCase():e}function ri(e){return e instanceof HTMLInputElement||e instanceof HTMLTextAreaElement||e instanceof HTMLSelectElement}function ve(e,t,n,i=!0){const o=Kt[e][t];if(!o)return;const l=oi(o,n);if(ie[e][t]=l,e===N.id&&i){const r=it[t];r!=null&&r.range&&(r.range.value=String(l)),r!=null&&r.number&&(r.number.value=nn(o,l))}}function si(e){var t;for(const n of((t=oe.find(i=>i.id===e))==null?void 0:t.params)??[])n.type==="seed"&&ve(e,n.id,Math.floor(Math.random()*1e6));he(e)}function ui(e){const t=Qt[e];for(const[n,i]of Object.entries(t))ve(e,n,i,!0);he(e)}function fi(){ee.innerHTML="";for(const e of oe){const t=document.createElement("option");t.value=e.id,t.textContent=e.name,ee.appendChild(t)}ee.addEventListener("change",()=>{an(ee.value)})}function ci(e){Me.innerHTML="";const t=document.createElement("button");if(t.className="ghost small",t.textContent="Reset",t.addEventListener("click",()=>ui(e.id)),Me.appendChild(t),e.params.some(i=>i.type==="seed")){const i=document.createElement("button");i.className="ghost small",i.textContent="Reseed",i.addEventListener("click",()=>si(e.id)),Me.appendChild(i)}}function mi(e){nt.innerHTML="",it={};for(const t of e.params){if(t.type==="seed")continue;const n=document.createElement("div");n.className="control";const i=document.createElement("div");i.className="control-header";const o=document.createElement("label");if(o.textContent=t.label,i.appendChild(o),t.key){const p=document.createElement("span");p.className="key-cap",p.textContent=`${t.key.inc.toUpperCase()}/${t.key.dec.toUpperCase()}`,i.appendChild(p)}const l=document.createElement("div");l.className="control-inputs";const r=document.createElement("input");r.type="range",r.min=String(t.min??0),r.max=String(t.max??1),r.step=String(t.step??(t.type==="int"?1:.01)),r.value=String(ie[e.id][t.id]),r.addEventListener("input",p=>{const y=Number(p.target.value);Number.isNaN(y)||ve(e.id,t.id,y)});const u=document.createElement("input");u.type="number",u.min=r.min,u.max=r.max,u.step=r.step,u.value=nn(t,ie[e.id][t.id]),u.addEventListener("input",p=>{const y=Number(p.target.value);Number.isNaN(y)||ve(e.id,t.id,y)}),l.appendChild(r),l.appendChild(u),n.appendChild(i),n.appendChild(l),nt.appendChild(n),it[t.id]={range:r,number:u}}}function pi(e){se.innerHTML="";for(const t of e.params){if(!t.key||t.type==="seed")continue;const n=document.createElement("div");n.className="key-row",n.textContent=`${t.key.inc.toUpperCase()}/${t.key.dec.toUpperCase()}  ${t.label}`,se.appendChild(n)}se.childElementCount||(se.textContent="No mapped keys for this scene.")}function an(e){const t=oe.find(n=>n.id===e);t&&(N=t,t.stateful&&(gt(t),he(t.id)),Ue.textContent=t.name,Zt.textContent=t.description,ai(),ci(t),mi(t),pi(t),ee.value=t.id)}function di(e){if(ri(e.target))return;const t=li(e.key),n=N.params;for(const i of n){if(!i.key||i.type==="seed")continue;const o=t===i.key.inc,l=t===i.key.dec;if(!o&&!l)continue;const r=e.shiftKey&&i.key.shiftStep?i.key.shiftStep:i.key.step,p=ie[N.id][i.id]+r*(o?1:-1);ve(N.id,i.id,p),e.preventDefault();break}}function Ce(e,t,n,i,o){const l=ie[e.id],r=t.uniforms,u=r[e.resolutionUniform];u&&s.uniform2f(u,i,o);const p=r[e.timeUniform];if(p)if(e.timeMode==="phase"){const b=e.loopDuration??8,w=n%b/b;s.uniform1f(p,w)}else if(e.timeMode==="looped"){const b=e.loopDuration??8,w=n%b;if(s.uniform1f(p,w),e.loopUniform){const S=r[e.loopUniform];S&&s.uniform1f(S,b)}}else s.uniform1f(p,n);const y={};for(const b of e.params){const w=r[b.uniform],S=l[b.id];if(b.component!==void 0){const L=y[b.uniform]??[0,0,0];L[b.component]=S,y[b.uniform]=L;continue}w&&(b.type==="int"?s.uniform1i(w,Math.round(S)):s.uniform1f(w,S))}for(const[b,w]of Object.entries(y)){const S=r[b];S&&s.uniform3f(S,w[0],w[1],w[2])}}function je(e,t,n,i){const o=t.uniforms[e.passUniform??"uPass"];o&&s.uniform1i(o,i);const l=t.uniforms[e.gridUniform??"uGridSize"];l&&s.uniform2f(l,n.size,n.size);const r=t.uniforms[e.stateUniform??"uState"];r&&s.uniform1i(r,0)}function on(e,t,n,i){const o=Yt.get(e.id);if(o){if(e.stateful){if(!$a)return;const l=gt(e);if(!l)return;const r=()=>l.textures[l.index],u=()=>l.fbos[(l.index+1)%2];s.useProgram(o.program),s.bindVertexArray(Bt),l.needsInit&&(s.bindFramebuffer(s.FRAMEBUFFER,u()),s.viewport(0,0,l.size,l.size),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,r()),Ce(e,o,t,n,i),je(e,o,l,2),s.drawArrays(s.TRIANGLES,0,3),l.index=(l.index+1)%2,l.needsInit=!1),s.bindFramebuffer(s.FRAMEBUFFER,u()),s.viewport(0,0,l.size,l.size),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,r()),Ce(e,o,t,n,i),je(e,o,l,0),s.drawArrays(s.TRIANGLES,0,3),l.index=(l.index+1)%2,s.bindFramebuffer(s.FRAMEBUFFER,null),s.viewport(0,0,n,i),s.clearColor(0,0,0,1),s.clear(s.COLOR_BUFFER_BIT),s.activeTexture(s.TEXTURE0),s.bindTexture(s.TEXTURE_2D,r()),Ce(e,o,t,n,i),je(e,o,l,1),s.drawArrays(s.TRIANGLES,0,3);return}s.viewport(0,0,n,i),s.clearColor(0,0,0,1),s.clear(s.COLOR_BUFFER_BIT),s.useProgram(o.program),s.bindVertexArray(Bt),Ce(e,o,t,n,i),s.drawArrays(s.TRIANGLES,0,3)}}function St(e){if(We)return;const t=(e-bt)/1e3,{width:n,height:i}=Wn(k);on(N,t,n,i),requestAnimationFrame(St)}function hi(){return new Promise(e=>requestAnimationFrame(()=>e()))}async function vi(e,t,n,i,o){if(typeof VideoEncoder>"u"||typeof VideoFrame>"u"){alert("Offline rendering requires the WebCodecs API (Chrome/Edge 94+, Safari 16.4+).");return}const l={codec:"avc1.640028",width:i,height:o,bitrate:16e6,framerate:n};if(!(await VideoEncoder.isConfigSupported(l)).supported){alert("H.264 video encoding is not supported on this device.");return}let u;try{u=await window.showSaveFilePicker({suggestedName:`${e.id}-${i}x${o}-${n}fps-${t}s.mp4`,types:[{description:"MP4 Video",accept:{"video/mp4":[".mp4"]}}]})}catch{return}const p=await u.createWritable(),y=document.getElementById("offline-btn"),b=document.getElementById("offline-progress"),w=document.getElementById("offline-bar"),S=document.getElementById("offline-status");y&&(y.disabled=!0),b==null||b.classList.remove("hidden"),S==null||S.classList.remove("hidden"),We=!0,e.stateful&&(gt(e),he(e.id));const L=k.width,W=k.height,D=k.style.width,Y=k.style.height;k.width=i,k.height=o,k.style.width=`${i}px`,k.style.height=`${o}px`;const ln=new Wt(p),xt=new ja({target:ln,video:{codec:"avc",width:i,height:o},fastStart:!1});let ge=null;const K=new VideoEncoder({output:(C,Le)=>xt.addVideoChunk(C,Le??void 0),error:C=>{ge=C}});K.configure(l);const Se=Math.ceil(t*n),wt=Math.round(1e6/n),rn=n*2,sn=n*30,un=performance.now();let xe=!1;const Ct=C=>{C.preventDefault(),xe=!0};k.addEventListener("webglcontextlost",Ct);for(let C=0;C<Se&&!(ge||xe);C++){const Le=C/n;on(e,Le,i,o),s.finish();const Tt=new VideoFrame(k,{timestamp:C*wt,duration:wt});K.encode(Tt,{keyFrame:C%rn===0}),Tt.close(),C>0&&C%sn===0&&await K.flush();const fn=(C+1)/Se*100;if(w&&(w.style.width=`${fn}%`),C%30===0){const cn=(performance.now()-un)/1e3/(C+1)*(Se-C-1);S&&(S.textContent=`Frame ${C+1}/${Se}  —  ~${Math.ceil(cn)}s left`)}K.encodeQueueSize>10&&await new Promise(kt=>setTimeout(kt,1)),await hi()}k.removeEventListener("webglcontextlost",Ct);try{await K.flush(),K.close(),xt.finalize(),await p.close()}catch(C){console.error("Finalization failed:",C);try{await p.close()}catch{}}k.style.width=D,k.style.height=Y,k.width=L,k.height=W,e.stateful&&he(e.id),We=!1,bt=performance.now(),requestAnimationFrame(St),y&&(y.disabled=!1);const Ne=ge?ge.message:null;S&&(xe?S.textContent="Context lost — partial video saved.":Ne?S.textContent=`Error: ${Ne}`:S.textContent="Done!"),setTimeout(()=>{b==null||b.classList.add("hidden"),S==null||S.classList.add("hidden"),w&&(w.style.width="0%")},xe||!!Ne?8e3:3e3)}_e.addEventListener("click",()=>{document.body.classList.toggle("sidebar-collapsed"),tn()});yt.addEventListener("mousemove",()=>{ni()});document.addEventListener("keydown",di);document.addEventListener("visibilitychange",()=>{document.hidden||(bt=performance.now())});var Ft;(Ft=document.getElementById("rec-btn"))==null||Ft.addEventListener("click",ti);var Pt;(Pt=document.getElementById("offline-btn"))==null||Pt.addEventListener("click",()=>{if(We)return;const e=document.getElementById("offline-duration"),t=document.getElementById("offline-fps"),n=document.getElementById("offline-res"),i=Math.max(1,Math.min(3600,Number((e==null?void 0:e.value)??10))),o=Number((t==null?void 0:t.value)??60),[l,r]=((n==null?void 0:n.value)??"1920x1080").split("x").map(Number);vi(N,i,o,l,r)});fi();an(N.id);tn();ot(!0);requestAnimationFrame(St);
